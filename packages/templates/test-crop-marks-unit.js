...
#align(center + horizon)[
  ...
  try {
    ...
    execSync(
      `typst compile --root . --format svg "${typstFile}" "${svgFile}"`,
      { stdio: "pipe" },
    );
...
function validateCropMarks(cropMarks, expected, testId) {
  ...
  if (cropMarks.length === 4) {
    ...
    const leftMark = cropMarks.reduce((min, mark) =>
      mark.x < min.x ? mark : min,
    );
    const rightMark = cropMarks.reduce((max, mark) =>
      mark.right > max.right ? mark : max,
    );
    const topMark = cropMarks.reduce((min, mark) =>
      mark.y < min.y ? mark : min,
    );
    const bottomMark = cropMarks.reduce((max, mark) =>
      mark.bottom > max.bottom ? mark : max,
    );
    ...
    if (expected.marksWithinSorobanBounds) {
      const allMarksReasonable = cropMarks.every(
        (mark) => mark.x > -100 && mark.x < 300 && mark.y > -50 && mark.y < 300,
      );
      results.push({
...
async function runUnitTests() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      // Validate crop marks
      const results = validateCropMarks(
        cropMarks,
      ...
      console.log(
        `   ❌ Failed to generate/test ${testCase.id}: ${error.message}`,
      );
      totalTests++;
      failedTests.push({
