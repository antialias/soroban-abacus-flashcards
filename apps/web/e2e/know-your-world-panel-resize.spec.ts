...
test.describe("Know Your World - Panel Resizing", () => {
  ...
  test("should have draggable resize handle between panels", async ({
    page,
  }) => {
  ...
  test("should resize panels when dragging handle", async ({ page }) => {
    ...
    const initialTopHeight = await topPanel
      .boundingBox()
      .then((box) => box?.height || 0);
    const initialBottomHeight = await bottomPanel
      .boundingBox()
      .then((box) => box?.height || 0);
    ...
    if (await resizeHandle.evaluate((el) => el !== null)) {
      ...
      const newTopHeight = await topPanel
        .boundingBox()
        .then((box) => box?.height || 0);
      const newBottomHeight = await bottomPanel
        .boundingBox()
        .then((box) => box?.height || 0);
  ...
  test("should respect min/max panel size constraints", async ({ page }) => {
    ...
    if (await resizeHandle.evaluate((el) => el !== null)) {
      ...
      const minTopHeight = await topPanel
        .boundingBox()
        .then((box) => box?.height || 0);
  ...
  test("should maintain map aspect ratio during resize", async ({ page }) => {
    ...
    const tolerance = 0.1;
    const diff =
      Math.abs(currentAspectRatio - expectedAspectRatio) / expectedAspectRatio;
