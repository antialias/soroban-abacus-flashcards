...
test.describe("API Authorization", () => {
  test.describe("Session Plan Authorization", () => {
    test("parent can create and modify session plan for own child", async ({
      page,
    }) => {
      ...
      const enableSkillsRes = await request.put(
        `/api/curriculum/${playerId}/skills`,
        {
          data: {
            masteredSkillIds: [
              "1a-direct-addition",
      ...
      const createPlanRes = await request.post(
      ...
      expect(
        approveRes.ok(),
        `Approve failed: ${await approveRes.text()}`,
      ).toBeTruthy();
      ...
      expect(
        startRes.ok(),
        `Start failed: ${await startRes.text()}`,
      ).toBeTruthy();
      ...
      expect(
        abandonRes.ok(),
        `Abandon failed: ${await abandonRes.text()}`,
      ).toBeTruthy();
    ...
    test("unrelated user cannot modify another user's session plan", async ({
      browser,
    }) => {
      ...
      try {
        ...
        const enableSkillsRes = await userARequest.put(
          `/api/curriculum/${playerId}/skills`,
          {
            data: {
  ...
  test.describe("Skills Endpoint Authorization", () => {
    test("parent can record skill attempts for own child", async ({ page }) => {
      ...
      expect(
        recordRes.ok(),
        `Record skill failed: ${await recordRes.text()}`,
      ).toBeTruthy();

      // PUT: Set mastered skills
      const setMasteredRes = await request.put(
        `/api/curriculum/${playerId}/skills`,
        {
      ...
      const refreshRes = await request.patch(
        `/api/curriculum/${playerId}/skills`,
        {
      ...
      expect(
        refreshRes.ok(),
        `Refresh skill failed: ${await refreshRes.text()}`,
      ).toBeTruthy();
    ...
    test("unrelated user cannot record skill attempts for another's child", async ({
      browser,
    }) => {
      const userAContext = await browser.newContext();
      const userBContext = await browser.newContext();
      ...
      try {
        ...
        const postAttackRes = await userBRequest.post(
          `/api/curriculum/${playerId}/skills`,
          {
            data: { skillId: "1a-direct-addition", isCorrect: true },
          },
        ...
        const putAttackRes = await userBRequest.put(
          `/api/curriculum/${playerId}/skills`,
          {
            data: {
              masteredSkillIds: ["1a-direct-addition", "1b-heaven-bead"],
        ...
        const patchAttackRes = await userBRequest.patch(
          `/api/curriculum/${playerId}/skills`,
          {
            data: { skillId: "1a-direct-addition" },
          },
  ...
  test.describe("Record Game Authorization", () => {
    ...
    test("parent can record game stats for own child", async ({ page }) => {
      ...
      const recordGameRes = await request.post(
        "/api/player-stats/record-game",
        {
          data: {
            gameResult: {
              ...
              playerResults: [
                { playerId, won: true, score: 100, accuracy: 0.95 },
              ],
            },
          },
        },
      ...
      expect(
        recordGameRes.ok(),
        `Record game failed: ${await recordGameRes.text()}`,
      ).toBeTruthy();
    ...
    test("unrelated user cannot record game stats for another's child", async ({
      browser,
    }) => {
      const userAContext = await browser.newContext();
      const userBContext = await browser.newContext();
    ...
    test("cannot record game stats for mixed authorized/unauthorized players", async ({
      ...
      try {
        ...
        const mixedRes = await userARequest.post(
          "/api/player-stats/record-game",
          {
            data: {
              gameResult: {
