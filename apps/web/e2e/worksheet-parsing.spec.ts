...
test.describe("Worksheet Parsing Flow", () => {
  ...
  test.describe("Attachment API", () => {
    test("can create and retrieve attachments for a session", async ({
      page,
    }) => {
      await page.goto("/");
  ...
  test.describe("Parse Review API", () => {
    test("review endpoint returns 404 for non-existent attachment", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Authorization", () => {
    test("unrelated user cannot access another user's attachments", async ({
      browser,
    }) => {
  ...
  test.describe("Parse API Validation", () => {
    test("parse stream endpoint validates attachment ownership", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
    ...
    test("parse-selected endpoint validates problem indices", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Approve/Unapprove Flow", () => {
    test("approve endpoint returns 404 for non-existent attachment", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
    ...
    test("unapprove endpoint returns 404 for non-existent attachment", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
