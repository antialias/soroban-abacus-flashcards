...
const OUTPUT_PATH = path.join(
  process.cwd(),
  "public/data/skill-difficulty-report.json",
);
...
function parseSnapshotFile(content: string): SnapshotData {
  ...
  const learningTrajectory = extractSnapshot(
    "learning-trajectory-by-category",
  ) as {
    exposuresToMastery: Record<string, number>;
    categoryAverages: Record<string, number>;
  };
  ...
  const learningExpectations = extractSnapshot(
    "learning-expectations-validation",
  ) as {
    at20Exposures: Record<string, string>;
    gapBetweenEasiestAndHardest: string;
  };
...
function generateReport(data: SnapshotData) {
  ...
  return {
    ...
    summary: {
      ...
      fiveCompAvgExposures:
        data.learningTrajectory?.categoryAverages?.fiveComplement ?? 24,
      tenCompAvgExposures:
        data.learningTrajectory?.categoryAverages?.tenComplement ?? 36,
      gapAt20Exposures:
        data.learningExpectations?.gapBetweenEasiestAndHardest ??
      ...
      skills: [
        {
          ...
          category: "basic",
          color: "#22c55e", // green
          data: data.abComparison?.withDifficulty?.[
            "basic.directAddition"
          ]?.map((v) => v * 100) ?? [28, 61, 78, 86, 91, 93, 96, 98],
        },
        {
          ...
          ]?.map((v) => v * 100) ?? [15, 41, 61, 74, 81, 86, 92, 95],
        },
        {
          ...
          ]?.map((v) => v * 100) ?? [9, 28, 47, 61, 71, 78, 86, 91],
        },
        {
          ...
          ]?.map((v) => v * 100) ?? [6, 20, 36, 50, 61, 69, 80, 86],
        },
      ],
    },
...
async function main() {
  ...
  console.log("\n--- Summary ---");
  console.log(
    `Basic skills avg: ${report.summary.basicAvgExposures} exposures to 80%`,
  );
  console.log(
    `Five-complements avg: ${report.summary.fiveCompAvgExposures} exposures to 80%`,
  );
  console.log(
    `Ten-complements avg: ${report.summary.tenCompAvgExposures} exposures to 80%`,
  );
  console.log(`Gap at 20 exposures: ${report.summary.gapAt20Exposures}`);
  console.log(
    `Exposure ratio (ten-comp/basic): ${report.summary.exposureRatioForEqualMastery}x`,
  );
}
