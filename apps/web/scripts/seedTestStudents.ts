...
import {
  createClassroom,
  getTeacherClassroom,
} from "../src/lib/classroom/classroom-manager";
...
async function createTestStudent(
  ...
  if (existing) {
    ...
    await db
      .delete(schema.parentChild)
      .where(eq(schema.parentChild.childPlayerId, existing.id));
 ...
 */
async function generateGameResults(
  playerId: string,
  profile: TestStudentProfile,
): Promise<number> {
  ...
  for (const gameConfig of profile.gameHistory) {
    ...
    for (let i = 0; i < gameConfig.gameCount; i++) {
      ...
      const normalizedScore = Math.max(
        0,
        Math.min(100, gameConfig.targetScore + scoreVariation),
      );
...
async function main() {
  ...
  if (!classroom) {
    ...
    if (result.success) {
      ...
      console.log(
        `   Created classroom: ${classroom.name} (code: ${classroom.code})`,
      );
    } else {
    ...
    console.log(
      `   Using existing classroom: ${classroom.name} (code: ${classroom.code})`,
    );
  }
