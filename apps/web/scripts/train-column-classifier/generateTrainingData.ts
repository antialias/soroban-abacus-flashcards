...
async function generateDigitSamples(
  ...
  for (const style of ABACUS_STYLE_VARIANTS) {
    ...
    const samplesForThisStyle = Math.min(
      samplesPerStyle,
      config.samplesPerDigit - sampleIndex,
    );
      ...
      try {
        ...
        if (progress.completed % 100 === 0) {
          ...
          process.stdout.write(
            `\rProgress: ${pct}% (${progress.completed}/${progress.total})`,
          );
        }
      } catch (error) {
...
async function generateAllTrainingData(): Promise<void> {
  ...
  if (values["dry-run"]) {
    ...
    ABACUS_STYLE_VARIANTS.forEach((style) => {
      console.log(
        `  - ${style.name}: ${style.beadShape}, ${style.colorScheme}`,
      );
    });
    return;
  }
  ...
  const metadata = {
    ...
    samplesPerDigit: Object.fromEntries(
      Array.from({ length: 10 }, (_, i) => [
        i,
        allSamples.filter((s) => s.digit === i).length,
      ]),
    ),
