...
describe("Session Share API", () => {
  ...
  afterEach(async () => {
    ...
    await db
      .delete(schema.sessionPlans)
      .where(eq(schema.sessionPlans.playerId, testPlayerId));
  ...
  describe("createSessionShare", () => {
    ...
    it("generates unique tokens for each share", async () => {
      const share1 = await createSessionShare(
        testSessionId,
        testPlayerId,
        testUserId,
        "1h",
      );
      const share2 = await createSessionShare(
        testSessionId,
        testPlayerId,
        testUserId,
        "1h",
      );
      const share3 = await createSessionShare(
        testSessionId,
        testPlayerId,
        testUserId,
        "1h",
