...
const ALL_TEST_SKILLS = [
  ...BASIC_SKILLS,
  ...FIVE_COMPLEMENT_SKILLS,
  ...TEN_COMPLEMENT_SKILLS,
];

/**
 * Convert skill ID array to SkillSet format for constraints
 */
function skillIdsToSkillSet(
  skillIds: string[],
    ...
    try {
      ...
      if (problem && problem.skillsRequired) {
        ...
        if (targetSkills) {
          ...
          targetSkillOccurrences += countTargetSkills(
  ...
  return {
    ...
    avgTargetSkillsPerProblem:
      successfulGenerations > 0
        ? targetSkillOccurrences / successfulGenerations
        : 0,
    targetHitRate:
      successfulGenerations > 0
        ? problemsWithTarget / successfulGenerations
        : 0,
    skillDistribution,
  };
}
...
describe("Problem Generator Targeting", () => {
  describe("Basic Targeting Verification", () => {
    it("should generate problems with five complement skills when targeted", () => {
      ...
      // Run with targeting
      const withTarget = runTargetingBatch(
        allowedSkills,
        targetSkills,
        100,
        12345,
      ...
      console.log(
        `With targeting:    ${(withTarget.targetHitRate * 100).toFixed(1)}% hit rate`,
      );
      console.log(
        `Without targeting: ${(withoutTarget.targetHitRate * 100).toFixed(1)}% hit rate`,
      );
      console.log(
      ...
      expect(withTarget.targetHitRate).toBeGreaterThan(
        withoutTarget.targetHitRate,
      );
    });
  ...
  describe("Large-Scale A/B Testing", () => {
    it("should show statistically significant targeting improvement across many seeds", () => {
      ...
      console.log(
        `Seeds: ${seeds.length}, Iterations per seed: ${iterationsPerSeed}`,
      );
      ...
      for (const seed of seeds) {
        ...
        const withoutTarget = runTargetingBatch(
          allowedSkills,
          null,
          iterationsPerSeed,
          seed,
        ...
        const withoutRate = (
          (withoutTargetHits / withoutTarget.totalProblems) *
          100
        ).toFixed(1);
        const diff = (
      ...
      console.log(
        `Total problems generated: ${totalWithTarget + totalWithoutTarget}`,
      );
    ...
    it("should compare targeting effectiveness for different skill combinations", () => {
      ...
      console.log(
        "Scenario                                    With      Without   Δ       Effect",
      );
      console.log("-".repeat(80));
      ...
      for (const scenario of scenarios) {
        ...
        const withoutProblemsWithTarget = Object.entries(
          withoutTarget.skillDistribution,
        )
  ...
  describe("Targeting Reliability by Seed", () => {
    it("should show consistent targeting across 100 different seeds", () => {
      ...
      const avgWithout =
        withoutTargetRates.reduce((a, b) => a + b, 0) / numSeeds;
      ...
      const stdWith = Math.sqrt(
        withTargetRates.reduce((sum, r) => sum + (r - avgWith) ** 2, 0) /
          numSeeds,
      );
      const stdWithout = Math.sqrt(
        withoutTargetRates.reduce((sum, r) => sum + (r - avgWithout) ** 2, 0) /
          numSeeds,
      );
      ...
      console.log(`\nDifference:`);
      console.log(
        `  Mean improvement: ${((avgWith - avgWithout) * 100).toFixed(1)}pp`,
      );
      console.log(
        `  Effect size: ${((avgWith - avgWithout) / stdWithout).toFixed(2)} std devs`,
      );

      // Count wins
      const targetingWins = withTargetRates.filter(
        (r, i) => r > withoutTargetRates[i],
      ).length;
      const targetingLosses = withTargetRates.filter(
        (r, i) => r < withoutTargetRates[i],
      ).length;
      ...
      console.log(
        `  Win rate:        ${((targetingWins / numSeeds) * 100).toFixed(1)}%`,
      );
