  ...
  try {
    while (attempts < MAX_GENERATION_ATTEMPTS) {
      ...
      if (attempts % 10000 === 0) {
        ...
        console.log(
          `[${attempts}] Generated ${allProblems.length} problems. Lowest coverage:`,
        );
    ...
    stats: {
      ...
      averageSkillsPerProblem:
        successfulGenerations > 0
          ? totalSkillsGenerated / successfulGenerations
          : 0,
    },
  };
}
...
describe("BKT Comprehensive Skill Identification", () => {
  ...
  beforeAll(() => {
    ...
    console.log(
      `Avg skills per problem: ${corpus.stats.averageSkillsPerProblem.toFixed(1)}`,
    );
    ...
    if (corpus.underrepresentedSkills.length > 0) {
      ...
      for (const skillId of corpus.underrepresentedSkills) {
        console.log(
          `  ${skillId}: ${corpus.bySkill.get(skillId)?.length || 0} problems`,
        );
      }
    } else {
  ...
  describe("Phase 1: Problem Corpus Coverage", () => {
    ...
    it("should have multi-skill problems (3+ skills)", () => {
      const multiSkillProblems = corpus.allProblems.filter(
        (p) => p.skillsUsed.length >= 3,
      );
      const percentage =
        (multiSkillProblems.length / corpus.allProblems.length) * 100;
        ...
        for (const skillId of skills) {
          it(`should identify ${skillId} as weak when 50% incorrect`, () => {
            ...
            if (problems.length < 20) {
              console.log(
                `⚠ Skipping ${skillId} - only ${problems.length} problems`,
              );
              return;
            }
            ...
            for (let i = 0; i < shuffled.length; i++) {
              ...
              const timestamp = new Date(
                baseTime - (shuffled.length - i) * 5000,
              );

              results.push(
                createResultFromProblem(problem, isCorrect, timestamp),
              );
            }
            ...
            const testedSkillBkt = bkt.skills.find(
              (s) => s.skillId === skillId,
            );
            ...
            const testedSkillRank = confidentSkills.findIndex(
              (s) => s.skillId === skillId,
            );
            ...
            console.log(
              `  BKT P(known): ${testedSkillBkt?.pKnown.toFixed(3) || "N/A"}`,
            );
            console.log(
              `  BKT confidence: ${testedSkillBkt?.confidence.toFixed(3) || "N/A"}`,
            );
            console.log(
            ...
            if (testedSkillBkt) {
              ...
              if (!isIdentifiedAsWeak) {
                ...
                console.log(
                  `    This is expected due to conjunctive model blame distribution`,
                );
              }
  ...
  describe("Phase 3: Summary Statistics", () => {
    it("should print overall BKT accuracy summary", () => {
      ...
      for (const r of results) {
        const shortSkill =
          r.skill.length > 25 ? `${r.skill.slice(0, 22)}...` : r.skill;
      ...
      if (notIdentified > 0) {
        ...
        console.log(
          "  These skills co-occur with other skills that receive more blame.",
        );
        console.log(
          "  This is CORRECT behavior - BKT identifies the most likely culprits.",
        );
      }
