...
export class JourneyRunner {
  ...
  async run(): Promise<JourneyResult> {
    ...
    try {
      for (
        let sessionNum = 1;
        sessionNum <= this.config.sessionCount;
        sessionNum++
      ) {
  ...
  private async runSession(sessionNumber: number): Promise<SessionSnapshot> {
    ...
    for (const part of startedPlan.parts) {
      for (const slot of part.slots) {
        ...
        for (const skillId of answer.skillsChallenged) {
          sessionExposures.set(
            skillId,
            (sessionExposures.get(skillId) ?? 0) + 1,
          );
        }
        ...
        await recordSlotResult(plan.id, {
          ...
          studentAnswer: answer.isCorrect
            ? slot.problem.answer
            : slot.problem.answer + 1,
    ...
    console.log(
      `[DEBUG] getRecentSessionResults returned ${problemHistory.length} results`,
    );
  ...
  private calculateFinalMetrics(snapshots: SessionSnapshot[]): JourneyMetrics {
    ...
    const totalFatigue = snapshots.reduce(
      (sum, s) => sum + s.sessionFatigue,
      0,
    );
    const avgFatiguePerSession =
      snapshots.length > 0 ? totalFatigue / snapshots.length : 0;
  ...
  private calculateWeakSkillSurfacing(snapshots: SessionSnapshot[]): number {
    ...
    const expectedRatio =
      weakSkills.length / this.config.practicingSkills.length;
   ...
   */
  private buildSkillTrajectories(
    snapshots: SessionSnapshot[],
  ): Map<string, SkillTrajectory> {
