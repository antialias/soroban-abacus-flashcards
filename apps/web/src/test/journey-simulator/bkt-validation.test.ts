...
async function runValidationScenario(
  ...
  const adaptiveAccuracy =
    adaptiveResults.snapshots[adaptiveResults.snapshots.length - 1]?.accuracy ?? 0
  const classicAccuracy =
    classicResults.snapshots[classicResults.snapshots.length - 1]?.accuracy ?? 0
...
describe('BKT Validation Suite', () => {
  ...
  describe('BKT Correlation Validation', () => {
    it('should show BKT pKnown correlates with true performance over time', async () => {
      ...
      const lastSnapshot = result.adaptive.snapshots[result.adaptive.snapshots.length - 1]

      console.log('Skill                     True P    BKT P(known)  Confidence  Exposures')
  ...
  describe('Confidence Growth Validation', () => {
    it('should show confidence increases with more opportunities', async () => {
      ...
      console.log('Session  ' + skills.map((s) => s.split('.')[1].slice(0, 10).padEnd(12)).join(''))
      console.log('-'.repeat(80))
      ...
      for (const skillId of skills) {
        const firstConf = firstSnapshot?.bktEstimates.get(skillId)?.confidence ?? 0
        const lastConf = lastSnapshot?.bktEstimates.get(skillId)?.confidence ?? 0
...
function printSessionDetails(results: JourneyResult): void {
  ...
  for (let i = 0; i < results.snapshots.length; i++) {
    ...
    console.log(`   ${i + 1}       ${accuracy}      N/A        ${weakCount} skills`)
  }
}
...
function printValidationSummary(results: ValidationResult[]): void {
  ...
  for (const r of results) {
    ...
    const adaptAcc = (r.metrics.adaptiveAccuracy * 100).toFixed(0).padStart(5) + '%'
    const classAcc = (r.metrics.classicAccuracy * 100).toFixed(0).padStart(5) + '%'
    const diff =
  ...
  // Aggregate stats
  const avgAdaptive =
    results.reduce((sum, r) => sum + r.metrics.adaptiveAccuracy, 0) / results.length
  const avgClassic = results.reduce((sum, r) => sum + r.metrics.classicAccuracy, 0) / results.length
  const adaptiveWins = results.filter((r) => r.metrics.accuracyDifference > 0).length
  const classicWins = results.filter((r) => r.metrics.accuracyDifference < 0).length
  const ties = results.filter((r) => r.metrics.accuracyDifference === 0).length

  console.log('\n' + '-'.repeat(80))
  console.log(`\nAGGREGATE RESULTS:`)
  console.log(`  Average final accuracy - Adaptive: ${(avgAdaptive * 100).toFixed(1)}%`)
  console.log(`  Average final accuracy - Classic:  ${(avgClassic * 100).toFixed(1)}%`)
  console.log(`  Adaptive wins: ${adaptiveWins} | Classic wins: ${classicWins} | Ties: ${ties}`)
  console.log(
...
function validateBktAssumptions(results: ValidationResult[]): void {
  ...
  const adaptiveMatchOrBetter = results.filter((r) => r.metrics.accuracyDifference >= -0.05).length
  console.log(
  ...
  const improvedAdaptive = results.filter((r) => r.metrics.adaptiveImprovement > 0).length
  const improvedClassic = results.filter((r) => r.metrics.classicImprovement > 0).length
  console.log(
  ...
  expect(improvedAdaptive + improvedClassic).toBeGreaterThanOrEqual(results.length)
