...
describe("Blame Attribution: Unit Tests", () => {
  ...
  it("should converge when all skills have equal mastery", () => {
    ...
    expect(bayesianResult[0].blameWeight).toBeCloseTo(
      bayesianResult[1].blameWeight,
      2,
    );
  ...
  it("should show extreme divergence at mastery cliff", () => {
    ...
    console.log(
      `Heuristic blame to new skill: ${(heuristicBlameToNew * 100).toFixed(1)}%`,
    );
    console.log(
      `Bayesian blame to new skill:  ${(bayesianBlameToNew * 100).toFixed(1)}%`,
    );
    console.log(
...
describe("Blame Attribution: Journey Simulation A/B", () => {
  ...
  it("should compare heuristic vs bayesian over full learning journey", async () => {
    ...
    const studentHeuristic = new SimulatedStudent(
      baseConfig.profile,
      rngHeuristic,
    );
    ...
    const studentBayesian = new SimulatedStudent(
      baseConfig.profile,
      rngBayesian,
    );
    ...
    const heuristicAccuracies = resultHeuristic.snapshots.map(
      (s) => s.accuracy,
    );
    ...
    console.log(
      `Heuristic: ${resultHeuristic.finalMetrics.weakSkillSurfacing.toFixed(2)}x`,
    );
    console.log(
      `Bayesian:  ${resultBayesian.finalMetrics.weakSkillSurfacing.toFixed(2)}x`,
    );

    // Check BKT correlation
    console.log("\n--- BKT-TRUE CORRELATION ---");
    console.log(
      `Heuristic: ${resultHeuristic.finalMetrics.bktCorrelation.toFixed(3)}`,
    );
    console.log(
      `Bayesian:  ${resultBayesian.finalMetrics.bktCorrelation.toFixed(3)}`,
    );
...
describe("Blame Attribution: Convergence Speed Results", () => {
  ...
  it("Summary: Compare blame methods across all learner types", async () => {
    ...
    for (const { name, profile } of profiles) {
      ...
      const { playerId: pH } = await createTestStudent(
        dbH.db,
        `${name}-heuristic`,
      );
      ...
      const { playerId: pB } = await createTestStudent(
        dbB.db,
        `${name}-bayesian`,
      );
      ...
      results.push({
        name,
        heuristicFinal:
          resultH.snapshots[resultH.snapshots.length - 1].accuracy,
    ...
    console.log(`\n${"=".repeat(85)}`);
    console.log(
      "   CONVERGENCE SPEED: HEURISTIC vs BAYESIAN BLAME ATTRIBUTION",
    );
    ...
    const avgBayesianCorr =
      results.reduce((s, r) => s + r.bayesianCorrelation, 0) / results.length;
    const avgHeuristicImpr =
      results.reduce((s, r) => s + r.heuristicImprovement, 0) / results.length;
    const avgBayesianImpr =
      results.reduce((s, r) => s + r.bayesianImprovement, 0) / results.length;

    console.log(`\nAverage BKT Correlation:`);
    console.log(`  Heuristic: ${avgHeuristicCorr.toFixed(3)}`);
    console.log(`  Bayesian:  ${avgBayesianCorr.toFixed(3)}`);
    console.log(
      `  Winner:    ${avgHeuristicCorr > avgBayesianCorr ? "Heuristic" : "Bayesian"}`,
    );
  ...
  it("Multi-seed validation: Fast learner heuristic vs bayesian", async () => {
    ...
    console.log(`\n${"=".repeat(70)}`);
    console.log(
      "   MULTI-SEED VALIDATION: FAST LEARNER - HEURISTIC vs BAYESIAN",
    );
    console.log("=".repeat(70));
    ...
    const diffs = results.map(
      (r) => r.bayesianCorrelation - r.heuristicCorrelation,
    );
    ...
    expect({
      seeds: results.map((r) => ({
        ...
        bayesianCorrelation: Math.round(r.bayesianCorrelation * 1000) / 1000,
        difference:
          Math.round((r.bayesianCorrelation - r.heuristicCorrelation) * 1000) /
          1000,
      })),
      statistics: {
