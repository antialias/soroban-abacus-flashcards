...
describe("Journey Simulator", () => {
  ...
  describe("SimulatedStudent", () => {
    it("should initialize with profile exposure values", () => {
      ...
      expect(
        student.getTrueProbability("basic.directAddition"),
      ).toBeGreaterThan(0.8);
...
describe("Journey Simulator Integration", () => {
  ...
  it("should show accuracy improvement over sessions", async () => {
    ...
    expect(result.finalMetrics.accuracyImprovement).toBeGreaterThanOrEqual(
      -0.1,
    );
  }, 90000);
});
    ...
    for (const snapshot of result.snapshots) {
      ...
      for (const skillId of testSkills) {
        ...
        const trueProbability =
          snapshot.trueSkillProbabilities.get(skillId) ?? 0;
        const cumulativeExposure =
          snapshot.cumulativeExposures.get(skillId) ?? 0;
        console.log(
...
describe("A/B Comparison: Adaptive vs Classic", () => {
  it("should show different behavior between adaptive and classic modes", async () => {
    ...
    const studentAdaptive = new SimulatedStudent(
      baseConfig.profile,
      rngAdaptive,
    );
    ...
    const adaptiveFinalAccuracy =
      adaptiveAccuracies[adaptiveAccuracies.length - 1];
    const classicFinalAccuracy =
      classicAccuracies[classicAccuracies.length - 1];

    console.log("\n--- KEY METRICS ---");
    console.log(
      `Adaptive final accuracy: ${(adaptiveFinalAccuracy * 100).toFixed(1)}%`,
    );
    console.log(
      `Classic final accuracy:  ${(classicFinalAccuracy * 100).toFixed(1)}%`,
    );
    console.log(
    ...
    if (accuraciesIdentical) {
      ...
      console.log(
        "    This suggests BKT may not be affecting problem generation.",
      );
    } else {
...
describe("A/B Test: Missed Lesson Learners", () => {
  ...
  for (const { name, profile, expectedWeakSkills } of profiles) {
    it(`${name}: BKT identifies weak skills and adaptive outperforms classic`, async () => {
      ...
      const rngClassic = new SeededRandom(baseConfig.seed);
      const studentClassic = new SimulatedStudent(
        baseConfig.profile,
        rngClassic,
      );
      const runnerClassic = new JourneyRunner(
      ...
      // ============ ANALYZE RESULTS ============
      const adaptiveAccuracies = resultAdaptive.snapshots.map(
        (s) => s.accuracy,
      );
      const classicAccuracies = resultClassic.snapshots.map((s) => s.accuracy);
      ...
      console.log(
        `Profile: K=${profile.halfMaxExposure}, n=${profile.hillCoefficient}`,
      );
      ...
      // Check overlap
      const correctlyIdentified = expectedWeakSkills.filter((s) =>
        bktIdentifiedWeak.includes(s),
      );
      const missedWeakSkills = expectedWeakSkills.filter(
        (s) => !bktIdentifiedWeak.includes(s),
      );
      const falsePositives = bktIdentifiedWeak.filter(
        (s) => !expectedWeakSkills.includes(s),
      );
  ...
  it("Summary: Compare all learner types", async () => {
    ...
    const adaptiveWins = results.filter(
      (r) => r.adaptiveFinal > r.classicFinal,
    ).length;
...
describe("Stark Contrast: Weak Skill Detection", () => {
  it("should identify weak skills and show adaptive vs classic difference", async () => {
    ...
    const lastAdaptiveSnapshot =
      resultAdaptive.snapshots[resultAdaptive.snapshots.length - 1];
    const lastClassicSnapshot =
      resultClassic.snapshots[resultClassic.snapshots.length - 1];
