...
import {
  ...
  useComplementRace,
} from "../../context/ComplementRaceContext";
...
describe("useSteamJourney - Passenger Boarding", () => {
  ...
  test("passenger boards when train reaches their origin station", () => {
    ...
    const boardedPassenger = result.current.race.state.passengers.find(
      (p) => p.id === "p1",
    );
  ...
  test("multiple passengers can board at the same station on different cars", () => {
    ...
    const boardedCount = result.current.race.state.passengers.filter(
      (p) => p.isBoarded,
    ).length;
  ...
  test("passenger is delivered when their car reaches destination", () => {
    ...
    const deliveredPassenger = result.current.race.state.passengers.find(
      (p) => p.id === "p1",
    );
