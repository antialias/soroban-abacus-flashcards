...
export function calculateFinalScore(
  ...
  const timeBonus = Math.max(
    0,
    Math.round((1000 * difficulty) / timeInMinutes),
  );
  ...
  const finalScore = Math.round(
    (baseScore + efficiencyBonus + timeBonus) *
...
export function getPerformanceAnalysis(state: MemoryPairsState): {
  ...
  if (efficiency >= 85) {
    ...
    improvements.push(
      "Try to reduce random guessing and use memory strategies",
    );
  }
  ...
  if (improvements.length === 0) {
    improvements.push(
      "Great job! Continue challenging yourself with harder difficulties.",
    );
  }
...
export function getMultiplayerWinner(
  ...
  const maxScore = Math.max(
    ...activePlayers.map((playerId) => scores[playerId] || 0),
  );

  // Find all players with the highest score
  const winners = activePlayers.filter(
    (playerId) => (scores[playerId] || 0) === maxScore,
  );
