  ...
  switch (action.type) {
    ...
    case "START_QUIZ":
      return {
        ...
        foundNumbers: [],
        guessesRemaining:
          action.quizCards.length + Math.floor(action.quizCards.length / 2),
        gamePhase: "display",
      };
    case "NEXT_CARD":
          ...
          >
            {Object.entries(DIFFICULTY_LEVELS).map(([key, level]) => (
              <button
                ...
                style={{
                  background:
                    state.selectedDifficulty === key ? "#3b82f6" : "white",
                  color: state.selectedDifficulty === key ? "white" : "#1f2937",
                  border: "2px solid",
                  borderColor:
                    state.selectedDifficulty === key ? "#3b82f6" : "#d1d5db",
                  borderRadius: "8px",
                  padding: "8px 12px",
                  cursor: "pointer",
                  textAlign: "center",
          ...
          >
            {[2, 5, 8, 12, 15].map((count) => (
              <button
                ...
                style={{
                  background:
                    state.selectedCount === count ? "#3b82f6" : "white",
                  color: state.selectedCount === count ? "white" : "#1f2937",
                  border: "2px solid",
                  borderColor:
                    state.selectedCount === count ? "#3b82f6" : "#d1d5db",
                  borderRadius: "8px",
                  padding: "8px 16px",
                  cursor: "pointer",
                  fontSize: "14px",
...
function DisplayPhase({
  ...
  const progressPercentage =
    (state.currentCardIndex / state.quizCards.length) * 100;
  ...
  useEffect(() => {
    ...
    const showNextCard = async () => {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, displayTimeMs - transitionPause),
      );

      // Don't hide the abacus - just advance to next card for smooth transition
      console.log(
        `DisplayPhase: Card ${state.currentCardIndex + 1} transitioning to next`,
      );
    ...
    <div
      style={{
        ...
        animation: isTransitioning
          ? `subtlePageFlash ${flashDuration}s ease-out`
          : undefined,
      }}
    >
      <div
  ...
  const handleKeyboardInput = useCallback(
    (key: string) => {
      ...
      if (/^[0-9]$/.test(key)) {
        ...
        if (
          state.correctAnswers.includes(number) &&
          !state.foundNumbers.includes(number)
        ) {
          ...
          const couldBePrefix = state.correctAnswers.some((n) =>
            n.toString().startsWith(newInput),
          );
          const isCompleteWrongNumber =
            !state.correctAnswers.includes(number) && !couldBePrefix;
          ...
          if (
            (newInput.length >= 2 || isCompleteWrongNumber) &&
  ...
  return (
    <div
      style={{
        ...
        padding: "12px",
        paddingBottom:
          (hasPhysicalKeyboard === false || testingMode) &&
          state.guessesRemaining > 0
            ? "100px"
          ...
          >
            ...
            <button
              style={{
                ...
                borderRadius: "8px",
                background:
                  state.currentInput.length > 0 ? "#fef2f2" : "#f9fafb",
                fontSize: "14px",
                fontWeight: "bold",
                color: state.currentInput.length > 0 ? "#dc2626" : "#9ca3af",
                cursor:
                  state.currentInput.length > 0 ? "pointer" : "not-allowed",
                minWidth: "70px",
                minHeight: "50px",
                display: "flex",
                alignItems: "center",
        ...
        >
          ...
          >
            ...
            >
              ...
              {state.gamePhase === "display" && (
                <DisplayPhase state={state} dispatch={dispatch} />
              )}
              ...
              {state.gamePhase === "results" && (
                <ResultsPhase state={state} dispatch={dispatch} />
              )}
            </div>
          </div>
        </div>
