 ...
 */
function InferenceSampleDisplay({
  sample,
  index,
}: {
...
export function TrainingCard({
  ...
  const chartOption = useMemo(() => {
    ...
    const hasPixelError = epochHistory.some(
      (e) => e.val_pixel_error !== undefined,
    );
  ...
  if (!currentEpoch) {
    return (
      <div className={css({ textAlign: "center", py: 6 })}>
        ...
        <div
          className={css({ color: "gray.300", fontWeight: "medium", mb: 1 })}
        >
          Preparing to Train
        </div>
  ...
  const progressPercent = Math.round(
    (currentEpoch.epoch / currentEpoch.total_epochs) * 100,
  );
    ...
    <div className={css({ textAlign: "center" })}>
      ...
      {isBoundaryDetector ? (
        <>
          ...
          {currentEpoch.inference_samples &&
            currentEpoch.inference_samples.length > 0 && (
              <div className={css({ mb: 3 })}>
                ...
                >
                  ...
                  Predicted{" "}
                  <span className={css({ color: "purple.400" })}>●</span> Ground
                  Truth
                </div>
                <div
                  className={css({
