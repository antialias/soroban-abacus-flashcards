...
function BeadPositionDisplay({
  ...
  const heavenColor =
    heavenConfidence > 0.8 ? 'green.400' : heavenConfidence > 0.5 ? 'yellow.400' : 'red.400'
  const earthColor =
    earthConfidence > 0.8 ? 'green.400' : earthConfidence > 0.5 ? 'yellow.400' : 'red.400'
  ...
  return (
    ...
    >
      ...
      <div className={css({ fontSize: '2xs', color: 'gray.500', textAlign: 'center' })}>
        ...
        <span className={css({ color: earthColor })}>E:{(earthConfidence * 100).toFixed(0)}%</span>
      </div>
    </div>
  )
}
...
export function ModelTester({ columnCount = 4 }: ModelTesterProps) {
  ...
  const runInference = useCallback(async () => {
    ...
    try {
      ...
      const columnImages = processImageFrame(imageElement, useCalibration, columnCount)
      ...
      // Run classification
      console.log('[ModelTester] Running classification on', columnImages.length, 'columns')
  ...
  return (
    ...
    >
      ...
      <div className={css({ display: 'flex', alignItems: 'center', gap: 2, mb: 3 })}>
        ...
        <span className={css({ fontSize: 'xs', ml: 'auto' })}>
          {classifier.isModelLoaded ? (
            ...
            <span className={css({ color: 'red.400' })}>Model incompatible</span>
      ...
      {classifier.isModelUnavailable && (
        ...
        >
          <div className={css({ fontWeight: 'medium', mb: 1 })}>Model needs retraining</div>
          <div className={css({ fontSize: 'xs', color: 'gray.400' })}>
            The existing model uses the old architecture. Train a new model with the two-head bead
            position detection to use this feature.
          </div>
        </div>
      )}
          ...
          >
            {columnDebug.dataUrls.map((url, idx) => (
              ...
              >
                ...
                />
                <span className={css({ fontSize: '2xs', color: 'gray.500', mt: 1 })}>
        ...
        >
          ...
          >
            {results.beadPositions.map((beadPos, idx) => (
              <BeadPositionDisplay key={idx} beadPosition={beadPos} digit={results.digits[idx]} />
            ))}
          </div>
