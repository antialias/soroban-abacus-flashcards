 ...
 */
export function isBoundaryDataItem(
  item: AnyDataItem,
): item is BoundaryDataItem {
 ...
 */
export function applyFilters<T extends AnyDataItem>(
  items: T[],
  ...
  return items.filter((item) => {
    ...
    if (filters.timeRangeMode !== "all" && item.capturedAt) {
      ...
      if (
        ...
        if (
          itemTime <= filters.afterTimestamp ||
          itemTime >= filters.beforeTimestamp
        ) {
          return false;
        }
      }
