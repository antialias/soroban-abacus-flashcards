...
const TrainingDiagnosticsContext = createContext<TrainingDiagnostics | null>(
  null,
);
...
export function useTrainingDiagnostics(): TrainingDiagnostics {
  ...
  if (!ctx) {
    throw new Error(
      "useTrainingDiagnostics must be used within TrainingDiagnosticsProvider",
    );
  }
  return ctx;
}
...
export function TrainingDiagnosticsProvider({
  ...
  const diagnostics = useMemo(() => {
    const reasons = analyzeDiagnostics(
      result,
      samples,
      datasetInfo,
      epochHistory,
    ...
    return {
      ...
      shouldShowRemediation:
        accuracy < 0.5 || (accuracy < 0.7 && reasons.length > 0),
      reasons,
    };
