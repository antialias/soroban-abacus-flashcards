...
function QuadTestContent() {
  ...
  const [staticImageCanvas, setStaticImageCanvas] =
  ...
  const [preprocessingStrategy, setPreprocessingStrategy] =
    useState<PreprocessingStrategy>(PREPROCESSING_DEFAULTS.strategy);
  const [enableHistogramEqualization, setEnableHistogramEqualization] =
    useState(PREPROCESSING_DEFAULTS.histogramEqualization);
  ...
  const [adaptiveBlockSize, setAdaptiveBlockSize] = useState(
    PREPROCESSING_DEFAULTS.blockSize,
  );
  ...
  const [enableHoughLines, setEnableHoughLines] = useState(
    PREPROCESSING_DEFAULTS.houghLines,
  );
  const [houghThreshold, setHoughThreshold] = useState(
    PREPROCESSING_DEFAULTS.houghThreshold,
  );
  const [houghMinLineLength, setHoughMinLineLength] = useState(
    PREPROCESSING_DEFAULTS.houghMinLineLength,
  );
  const [houghMaxLineGap, setHoughMaxLineGap] = useState(
    PREPROCESSING_DEFAULTS.houghMaxLineGap,
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        if (settings.morphGradient !== undefined)
          setEnableMorphGradient(settings.morphGradient);
        if (settings.blockSize !== undefined)
          setAdaptiveBlockSize(settings.blockSize);
        ...
        // Hough line settings
        if (settings.houghLines !== undefined)
          setEnableHoughLines(settings.houghLines);
        if (settings.houghThreshold !== undefined)
          setHoughThreshold(settings.houghThreshold);
        if (settings.houghMinLineLength !== undefined)
          setHoughMinLineLength(settings.houghMinLineLength);
        if (settings.houghMaxLineGap !== undefined)
          setHoughMaxLineGap(settings.houghMaxLineGap);
      }
    } catch {
      // Ignore parse errors
    }
          ...
          try {
            ...
            if (errorCount === 1 || errorCount === MAX_CONSECUTIVE_ERRORS) {
              console.error(
                ...
                typeof err === "number"
                  ? `OpenCV error code: ${err} (0x${err.toString(16)})`
                  : err,
              );
            }
            ...
            if (errorCount >= MAX_CONSECUTIVE_ERRORS) {
              console.warn(
                "Too many detection errors, pausing for 1 second...",
              );
              setTimeout(() => {
                errorCount = 0;
  ...
  return (
    ...
    >
      ...
      >
        <StatusBadge
          label="opencv-react"
          status={opencvLoaded ? "ready" : "loading"}
        />
        <StatusBadge
      ...
      >
        ...
        >
          <div>
            ...
            {opencvLoaded && (
              <span className={css({ color: "green.400", ml: 2 })}>
                ✓ cv available
              </span>
            )}
          </div>
          ...
          {isCameraActive && (
            <>
              ...
              <div>
                Camera detection: {cameraDebugInfo.detectionTimeMs.toFixed(1)}ms
              </div>
              <div>Tracked quads: {cameraDebugInfo.trackedCount}</div>
              <div>
                Best stability:{" "}
                {(cameraDebugInfo.bestStability * 100).toFixed(0)}%
              </div>
          ...
          {staticDetectionResult && (
            <>
              ...
              <div>
                Static detection:{" "}
                {staticDetectionResult.detectionTimeMs.toFixed(1)}ms
              </div>
            </>
          )}
        </div>
      ...
      >
        ...
        <div className={css({ bg: "gray.800", borderRadius: "lg", p: 4 })}>
          ...
          >
                ...
                {Object.entries(DETECTION_PROFILES).map(([key, profile]) => (
                  <button
                    ...
                    className={css({
                      ...
                      borderColor:
                        activeProfile === key ? "blue.400" : "transparent",
                      cursor: "pointer",
            ...
            >
              Select a profile above for quick setup, or adjust individual
              settings below.
            </div>
            ...
            <div className={css({ mb: 3 })}>
              ...
              >
                ...
                <option value="enhanced">
                  Enhanced - Better for low contrast
                </option>
                <option value="adaptive">
                  Adaptive - Best for uneven lighting
                </option>
                <option value="multi">
                  Multi - Tries everything (recommended)
                </option>
              </select>
              <div
                className={css({
            ...
            <div className={css({ mb: 3 })}>
              ...
              >
                <div>
                  ...
                  >
                    Spreads out brightness levels. Turn ON for low-contrast
                    images.
                  </div>
                </div>
                <div>
                  ...
                  >
                    Detects edges locally. Turn ON when lighting is uneven
                    across the document.
                  </div>
                </div>
                <div>
                  ...
                  >
                    Finds edges by dilation-erosion. Turn ON for thick or blurry
                    edges.
                  </div>
                </div>
              </div>
            ...
            <div className={css({ mb: 2 })}>
              ...
              >
                <div>
                  ...
                  >
                    Neighborhood size for local thresholding. Larger = smoother
                    edges, smaller = more detail.
                  </div>
                </div>
                <div>
                  <SliderControl
                  ...
                  >
                    Threshold offset. Higher = fewer edges detected, lower =
                    more noise.
                  </div>
                </div>
                <div>
                  ...
                  >
                    Weak edge threshold. Lower = detect faint edges (more
                    noise).
                  </div>
                </div>
                <div>
                  ...
                  >
                    Strong edge threshold. Lower = more edges, higher = only
                    strong edges.
                  </div>
                </div>
              </div>
            ...
            <div className={css({ mb: 3 })}>
              ...
              <div>
                ...
                >
                  Detects straight lines and infers corners from intersections.
                  Fallback when contours fail.
                </div>
              </div>
              {enableHoughLines && (
                <div
            ...
            >
              ...
              >
                <li>
                  Document not detected? Lower Canny thresholds or enable
                  Histogram Eq.
                </li>
                <li>
                  Too many false edges? Raise Canny thresholds or increase
                  Adaptive C.
                </li>
                <li>
                  Fingers blocking edges? Enable Hough Lines - it finds edges by
                  detecting straight lines and computing their intersections.
                </li>
              </ul>
            </div>
          </div>
          ...
          {debugMode &&
            isCameraActive &&
            cameraDebugInfo.debugPolygons.length > 0 && (
              <div
                className={css({
                  mt: 3,
                  p: 2,
              ...
              >
                ...
                {cameraDebugInfo.debugPolygons.slice(0, 10).map((poly, i) => (
                  ...
                  >
                    ...
                    vertices: {poly.vertexCount}, area:{" "}
                    {(poly.areaRatio * 100).toFixed(1)}%
                    {poly.hullVertices &&
                      ` → hull: ${poly.hullVertices.length}`}
                    {poly.aspectRatio &&
                      ` ratio: ${poly.aspectRatio.toFixed(2)}`}
                  </div>
                ))}
          ...
          {staticDetectionResult && (
            ...
            >
              <div>
                ...
                <span
                  className={css({
                    color: staticDetectionResult.detected
                      ? "green.400"
                      : "yellow.400",
                  })}
                >
              ...
              <div
                className={css({ fontFamily: "mono", fontSize: "xs", mt: 1 })}
              >
                Corners:{" "}
                {JSON.stringify(
                  staticDetectionResult.corners.map((c) => [
                    Math.round(c.x),
                    Math.round(c.y),
                  ]),
                )}
      ...
      >
        ...
        <ul className={css({ listStyleType: "disc", pl: 4, lineHeight: 1.6 })}>
          <li>
            <strong>OpenCvProvider</strong> wraps the page and loads OpenCV via
            opencv-react
          </li>
          <li>
            <strong>useQuadDetection()</strong> combines OpenCV loading,
          ...
          <li>
            Status: orange=tracking, yellow=stable (3+ frames), green=locked (5+
            frames, high stability)
          </li>
...
function drawDebugOverlay(
  ...
  for (const poly of debugPolygons) {
    ...
    if (poly.vertices.length > 0) {
      ...
      const cx =
        poly.vertices.reduce((s, v) => s + v.x, 0) / poly.vertices.length;
      const cy =
        poly.vertices.reduce((s, v) => s + v.y, 0) / poly.vertices.length;
...
function ToggleSwitch({
  ...
  return (
    ...
    >
      ...
      <span className={css({ color: checked ? "gray.200" : "gray.400" })}>
        {label}
      </span>
    </label>
  );
  ...
  return (
    <div className={css({ fontSize: "xs" })}>
      ...
      >
        ...
        <span className={css({ color: "gray.200", fontFamily: "mono" })}>
          {value}
        </span>
      </div>
      <input
