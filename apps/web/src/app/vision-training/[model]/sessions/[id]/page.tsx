...
function TrainingGraph({ epochHistory, modelType }: TrainingGraphProps) {
  ...
  const maxLoss = Math.max(
    ...epochHistory.map((e) => Math.max(e.loss, e.val_loss)),
  );
  const minLoss = Math.min(
    ...epochHistory.map((e) => Math.min(e.loss, e.val_loss)),
  );
  ...
  const hasPixelError =
    ...
    epochHistory.some((e) => e.val_pixel_error);
  ...
  const scaleX = (epoch: number) =>
    padding.left + (epoch / maxEpochs) * graphWidth;
  const scaleY = (loss: number) =>
    padding.top + (1 - (loss - minLoss) / lossRange) * graphHeight;
  const scaleYPixel = (px: number) =>
    padding.top + (1 - px / (maxPixelError || 1)) * graphHeight;

  const trainLossPath = epochHistory
    .map(
      (e, i) => `${i === 0 ? "M" : "L"} ${scaleX(e.epoch)} ${scaleY(e.loss)}`,
    )
    .join(" ");
  ...
  const valLossPath = epochHistory
    .map(
      (e, i) =>
        `${i === 0 ? "M" : "L"} ${scaleX(e.epoch)} ${scaleY(e.val_loss)}`,
    )
    .join(" ");
  ...
  return (
    <div>
      <svg
        ...
        className={css({ width: "100%", maxWidth: "500px" })}
      >
        {/* Grid lines */}
...
function InfoRow({ label, value, highlight }: InfoRowProps) {
  ...
  return (
    ...
    >
      <span className={css({ color: "gray.400", fontSize: "sm" })}>
        {label}
      </span>
      <span
        className={css({
...
export default function SessionDetailPage({
    ...
    return (
      ...
      >
        <div className={css({ color: "red.400" })}>
          Error:{" "}
          {error ||
            (!session ? "Session not found" : "Session has no result data")}
        </div>
        <Link
  ...
  const hardware = (
    result as TrainingResult & { hardware?: TrainingHardwareInfo }
  ).hardware;
  const environment = (
    result as TrainingResult & { environment?: TrainingEnvironmentInfo }
  ).environment;
  const startedAt = (result as TrainingResult & { started_at?: string })
    .started_at;
  const completedAt = (result as TrainingResult & { completed_at?: string })
    .completed_at;
  const durationSeconds = (
    result as TrainingResult & { training_duration_seconds?: number }
  ).training_duration_seconds;
  const epochHistoryFromResult = (
    result as TrainingResult & { epoch_history?: EpochData[] }
  ).epoch_history;
    ...
    >
          ...
          <div className={css({ display: "flex", gap: 2, mt: 2 })}>
            <span
              className={css({
                ...
                py: 0.5,
                bg:
                  session.modelType === "column-classifier"
                    ? "purple.600/30"
                    : "orange.600/30",
                color:
                  session.modelType === "column-classifier"
      ...
      <main className={css({ p: 4, maxWidth: "1200px", mx: "auto" })}>
        ...
        >
          ...
          >
            <h2
              className={css({ fontSize: "lg", fontWeight: "semibold", mb: 4 })}
            >
              Training Progress
            </h2>
            <TrainingGraph
          ...
          <div className={css({ p: 4, bg: "gray.800", borderRadius: "lg" })}>
            ...
            <InfoRow
              label="Final Loss"
              value={(result.final_loss ?? 0).toFixed(4)}
            />
            {session.modelType === "boundary-detector" &&
              "final_pixel_error" in result && (
                <InfoRow
                  label="Pixel Error"
            ...
            <InfoRow
              label="TF.js Exported"
              value={result.tfjs_exported ? "Yes" : "No"}
            />
          </div>
          ...
          <div className={css({ p: 4, bg: "gray.800", borderRadius: "lg" })}>
            ...
            {session.config.heatmap_size && (
              <InfoRow
                label="Heatmap Size"
                value={session.config.heatmap_size}
              />
            )}
          ...
          {hardware && (
            <div className={css({ p: 4, bg: "gray.800", borderRadius: "lg" })}>
              ...
              <InfoRow
                label="Device Type"
                value={hardware.deviceType?.toUpperCase()}
              />
              {hardware.chipType && (
                <InfoRow label="Chip" value={hardware.chipType} />
              )}
              {hardware.systemMemory && (
                <InfoRow label="Memory" value={hardware.systemMemory} />
              )}
              {hardware.tensorflowVersion && (
                <InfoRow
                  label="TensorFlow"
                  value={hardware.tensorflowVersion}
                />
              )}
              {hardware.platform && (
                <InfoRow label="Platform" value={hardware.platform} />
              )}
            </div>
          )}
          ...
          <div className={css({ p: 4, bg: "gray.800", borderRadius: "lg" })}>
            ...
            {"total_frames" in session.datasetInfo && (
              <InfoRow
                label="Total Frames"
                value={session.datasetInfo.total_frames}
              />
            )}
            {"total_images" in session.datasetInfo && (
              <InfoRow
                label="Total Images"
                value={session.datasetInfo.total_images}
              />
            )}
            {"device_count" in session.datasetInfo && (
              <InfoRow
                label="Devices"
                value={session.datasetInfo.device_count}
              />
            )}
            {"color_augmentation_enabled" in session.datasetInfo && (
              <InfoRow
                label="Color Augmentation"
                value={
                  session.datasetInfo.color_augmentation_enabled ? "Yes" : "No"
                }
              />
            )}
          </div>
