      ...
      >
        ...
        {modelType === "column-classifier" && (
          <>
            ...
            >
              <div
                className={css({ flex: 1, height: "1px", bg: "gray.700" })}
              />
            ...
            >
              ...
              >
                ...
                <li>
                  Or point your camera at the abacus with ArUco markers visible
                </li>
              ...
              <p className={css({ mt: 3, fontSize: "xs", color: "gray.500" })}>
                Model location:{" "}
                <code>/public/models/abacus-column-classifier/</code>
              </p>
            </div>
          </>
        )}
        ...
        {modelType === "boundary-detector" && (
          <>
            ...
            >
              ...
              <p className={css({ mt: 3, fontSize: "xs", color: "gray.500" })}>
                Model location:{" "}
                <code>/public/models/abacus-boundary-detector/</code>
              </p>
            </div>
          </>
        )}
...
function ImageUploadTester({ preloadImageUrl }: ImageUploadTesterProps) {
  ...
  useEffect(() => {
    ...
    const loadPreloadedImage = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to load preloaded image",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const pathParts =
        file.webkitRelativePath?.split("/") || file.name.split("/");
  ...
  const isCorrect =
    result && expectedDigit !== null ? result.digit === expectedDigit : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className={css({ fontSize: "xs", color: "gray.500", ml: "auto" })}
        >
      ...
      >
        ...
        >
          {uploadedImage ? (
            ...
            <div
              className={css({ textAlign: "center", color: "gray.500", p: 2 })}
            >
              <div className={css({ fontSize: "2xl", mb: 1 })}>+</div>
              <div className={css({ fontSize: "xs" })}>
                Click to upload PNG from training data
              </div>
            </div>
          )}
        </div>
        ...
        <div className={css({ flex: 1 })}>
          ...
          {result && (
            ...
            >
              ...
              {/* Formula reminder */}
              <div
                className={css({ mt: 2, fontSize: "xs", color: "gray.500" })}
              >
                digit = heaven x 5 + earth = {result.heaven} x 5 +{" "}
                {result.earth} = {result.digit}
              </div>
            </div>
          )}
      ...
      >
        ...
        >
          {uploadedImage ? (
            ...
            <div
              className={css({ textAlign: "center", color: "gray.500", p: 4 })}
            >
              <div className={css({ fontSize: "3xl", mb: 2 })}>+</div>
              <div className={css({ fontSize: "sm" })}>
                Click to upload a frame image
              </div>
              <div
                className={css({ fontSize: "xs", mt: 2, color: "gray.600" })}
              >
        ...
        {result && (
          ...
          >
            ...
            <div className={css({ fontSize: "sm" })}>
              ...
              >
                <div
                  className={css({ bg: "gray.700", p: 2, borderRadius: "md" })}
                >
                  <div className={css({ color: "gray.500", fontSize: "xs" })}>
                    Top Left
                  </div>
                  <div
                    className={css({ fontFamily: "mono", color: "gray.200" })}
                  >
                    ({result.corners.topLeft.x.toFixed(3)},{" "}
                    {result.corners.topLeft.y.toFixed(3)})
                  </div>
                </div>
                <div
                  className={css({ bg: "gray.700", p: 2, borderRadius: "md" })}
                >
                  ...
                  <div
                    className={css({ fontFamily: "mono", color: "gray.200" })}
                  >
                    ({result.corners.topRight.x.toFixed(3)},{" "}
                    {result.corners.topRight.y.toFixed(3)})
                  </div>
                </div>
                <div
                  className={css({ bg: "gray.700", p: 2, borderRadius: "md" })}
...
function BoundaryCameraTester() {
    ...
    const loop = async () => {
      ...
      if (now - lastInferenceTimeRef.current > 100) {
        ...
        if (videoRef.current && videoRef.current.readyState >= 2) {
          const detectionResult = await detector.detectFromVideo(
            videoRef.current,
          );
  ...
  useEffect(() => {
    ...
    ctx.beginPath();
    ctx.moveTo(
      corners.topLeft.x * canvas.width,
      corners.topLeft.y * canvas.height,
    );
    ctx.lineTo(
      corners.topRight.x * canvas.width,
      corners.topRight.y * canvas.height,
    );
    ctx.lineTo(
      corners.bottomRight.x * canvas.width,
      corners.bottomRight.y * canvas.height,
    );
    ctx.lineTo(
      corners.bottomLeft.x * canvas.width,
      corners.bottomLeft.y * canvas.height,
    );
    ctx.closePath();
      ...
      {isRunning && result && (
        ...
        >
          ...
          <div className={css({ fontSize: "xs" })}>
            ...
            >
              <span>
                TL: ({result.corners.topLeft.x.toFixed(2)},{" "}
                {result.corners.topLeft.y.toFixed(2)})
              </span>
              <span>
                TR: ({result.corners.topRight.x.toFixed(2)},{" "}
                {result.corners.topRight.y.toFixed(2)})
              </span>
              <span>
