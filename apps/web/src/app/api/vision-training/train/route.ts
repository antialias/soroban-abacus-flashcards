 ...
 */
async function saveTrainingSession(sessionData: TrainingSessionData): Promise<void> {
  if (!sessionData.completeData || !sessionData.datasetInfo) {
    console.error('[Training] Cannot save session - missing complete data or dataset info')
    return
  }
...
export async function POST(request: Request): Promise<Response> {
  ...
  activeStopFilePath = path.join(cwd, 'data', 'vision-training', `.stop-${Date.now()}`)
          ...
          try {
            ...
            if (activeSessionData) {
              if (eventType === 'training_started') {
                ...
                activeSessionData.trainingDurationSeconds = event.training_duration_seconds || null
...
export async function PUT(): Promise<Response> {
  ...
  return new Response(
    JSON.stringify({
      message: 'Early stop requested - model will be saved at end of current epoch',
    }),
    {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
