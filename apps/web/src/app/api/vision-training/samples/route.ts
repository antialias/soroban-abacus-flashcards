...
const COLUMN_CLASSIFIER_DIR = path.join(
  process.cwd(),
...
type SamplesResponse =
  | ColumnClassifierSamplesResponse
  | BoundaryDetectorSamplesResponse;
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    return Response.json(
      { error: "Failed to read training samples" },
      { status: 500 },
    );
  }
}
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (files.length > 0 && samplePaths.length < 5) {
        ...
        samplePaths.push(
          `/api/vision-training/boundary-images/${entry.name}/${sampleFile}`,
        );
      }
