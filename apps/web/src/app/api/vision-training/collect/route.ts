...
export async function POST(
  request: NextRequest,
): Promise<NextResponse<TrainingDataResponse>> {
  try {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      if (!column.imageData) {
        console.warn(
          `[vision-training] Column ${i} missing imageData, skipping`,
        );
        continue;
      }
      ...
      try {
        ...
        console.error(
          `[vision-training] Failed to write column ${i}:`,
          writeError,
        );
      }
    }
