...
const MODEL_PATHS: Record<ModelType, { remote: string; local: string }> = {
  "column-classifier": {
    remote:
      "/volume1/homes/antialias/projects/abaci.one/data/vision-training/collected/",
    local: path.join(process.cwd(), "data/vision-training/collected/"),
  },
  "boundary-detector": {
    remote:
      "/volume1/homes/antialias/projects/abaci.one/data/vision-training/boundary-frames/",
    ...
    async start(controller) {
      ...
      try {
        ...
        try {
          ...
          console.error(
            "[vision-training/sync] Failed to record sync history:",
            dbError,
          );
        }
  ...
  try {
    ...
    return Response.json({
      ...
      remote: {
        ...
        totalFrames:
          modelType === "boundary-detector" ? remoteCount : undefined,
      },
      local: localStats,
