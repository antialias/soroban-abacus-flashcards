  ...
  try {
    ...
    const pendingClassrooms =
      uniquePendingClassroomIds.length > 0
        ...
        : [];
    const pendingClassroomMap = new Map(
      pendingClassrooms.map((c) => [c.id, c]),
    );
    ...
    const enrolledClassrooms: EnrolledClassroomInfo[] =
      enrolledClassroomsList.map((classroom) => {
    ...
    const pendingEnrollments: PendingEnrollmentInfo[] = pendingRequests.map(
      (request) => {
        ...
        return {
          ...
          teacherName: teacher?.name ?? "Unknown Teacher",
          pendingApproval:
            request.teacherApproval === null ? "teacher" : "parent",
    ...
    const isPresent =
      currentPresence && viewerClassroom?.id === currentPresence.classroomId;
