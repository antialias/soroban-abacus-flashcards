...
const CUSTOM_CROPS_PATH = join(
  process.cwd(),
  "src/arcade-games/know-your-world/customCrops.ts",
);
...
function parseCropsFile(): Record<string, Record<string, string>> {
  ...
  const startMatch = currentContent.match(
...
function writeCropsFile(crops: Record<string, Record<string, string>>): void {
  ...
  if (!startMatch || startMatch.index === undefined) {
    console.error(
      "[DevCropTool] Could not find customCrops declaration for writing",
    );
    return;
  }
  ...
  // Replace the object
  const newContent =
    currentContent.slice(0, objStart) +
    formattedCrops +
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(
      `[DevCropTool] Saved crop for ${mapId}/${continentId}: ${viewBox}`,
    );
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const crops = parseCropsFile();
    console.log(
      "[DevCropTool] Parsed crops before delete:",
