...
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    ...
    if (!attachment.rawParsingResult) {
      return NextResponse.json(
        { error: "Attachment has not been parsed" },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Failed to get review progress" },
      { status: 500 },
    );
  }
}
...
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to initialize review progress" },
      { status: 500 },
    );
  }
}
    ...
    if (body.problemUpdate) {
      ...
      if (oldStatus !== reviewStatus) {
        ...
        if (oldStatus === "pending" || oldStatus === "flagged") {
          ...
          reviewProgress = {
            ...reviewProgress,
            manuallyReviewedCount: Math.max(
              0,
              reviewProgress.manuallyReviewedCount - 1,
            ),
          };
