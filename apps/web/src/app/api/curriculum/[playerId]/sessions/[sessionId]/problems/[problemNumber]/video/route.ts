  ...
  try {
    ...
    if (video.status === "no_video") {
      return NextResponse.json(
        {
          error:
            "No video was recorded for this problem (camera may have been off)",
        },
        { status: 404 },
      );
    }
        ...
        try {
          ...
          if (frameFiles.length > 0) {
            ...
            if (ffmpegAvailable) {
              ...
              console.error(
                `[problem-video] Frames exist but ffmpeg not available: ${framesDir}`,
              );
              return NextResponse.json(
          ...
          console.error(
            `[problem-video] Error checking frames directory: ${err}`,
          );
        }
      }
