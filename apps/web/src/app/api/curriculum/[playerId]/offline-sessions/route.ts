...
import {
  getPracticeType,
  isValidPracticeTypeId,
} from "@/constants/practiceTypes";
import { db } from "@/db";
 ...
 */
function buildOfflineSessionParts(
  practiceTypes: SessionPartType[],
): SessionPart[] {
  return [
    {
      partNumber: 1,
  ...
  try {
    ...
    if (practiceTypesJson) {
      try {
        ...
        return NextResponse.json(
          { error: "Invalid practiceTypes format" },
          { status: 400 },
        );
      }
    }
    ...
    if (practiceTypes.length === 0) {
      return NextResponse.json(
        { error: "At least one practice type is required" },
        { status: 400 },
      );
    }
