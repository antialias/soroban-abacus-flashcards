  ...
  try {
    ...
    if (format === "monthly") {
      ...
      const yearSvg = renderToStaticMarkup(
        generateAbacusElement(year, yearColumns),
      );
      ...
      const dayOfWeek = new Date(year, month - 1, 1).toLocaleDateString(
        "en-US",
        {
          weekday: "long",
        },
      ...
      const yearSvgContent = yearSvg
        .replace(/<svg[^>]*>/, "")
        .replace(/<\/svg>$/, "");
      const daySvgContent = daySvg
        .replace(/<svg[^>]*>/, "")
        .replace(/<\/svg>$/, "");
