  ...
  try {
    ...
    if (numbers.length === 0) {
      return NextResponse.json(
        { error: "No valid numbers in range" },
        { status: 400 },
      );
    }
    ...
    const svgConfig = {
      ...
      hideInactiveBeads,
      showEmptyColumns,
      columns: (columns === "auto" ? "auto" : Number(columns)) as
        | number
        | "auto",
      scaleFactor,
      coloredNumerals,
    ...
    const availableWidth =
      pageWidth - 2 * marginInches - gutterInches * (cols - 1);
    const availableHeight =
      pageHeight - 2 * marginInches - gutterInches * (rows - 1);
