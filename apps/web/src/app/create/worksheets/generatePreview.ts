...
export async function generateWorksheetPreview(
  ...
  console.log(
    `[PREVIEW] Starting generation: ${totalProblems} problems, pages ${config.pages}`,
  );
  ...
  try {
    ...
    if (spaceValidation.warnings.length > 0) {
      console.log(
        "[PREVIEW] Problem space warnings:",
        spaceValidation.warnings,
      );
    }
    ...
    if (mode === "mastery" && operator === "mixed") {
      ...
      if (!addSkillId || !subSkillId) {
        return {
          success: false,
          error:
            "Mixed mastery mode requires both addition and subtraction skill IDs",
    ...
    const previewShareUrl = validatedConfig.includeQRCode
      ...
      : undefined;
    console.log(
      `[PREVIEW] Step 3: Generating Typst source for ${validatedConfig.pages} pages...`,
    );
    const startTypst = Date.now();
    const typstSources = await generateTypstSource(
      validatedConfig,
      problems,
      previewShareUrl,
    );
    ...
    console.log(
      `[PREVIEW] Step 3: ✓ Generated ${totalPages} Typst sources in ${typstTime}ms`,
    );
    ...
    for (let i = start; i <= end; i++) {
      ...
      try {
        ...
        console.log(
          `[PREVIEW] Step 4.${i + 1}: ✓ Page ${i} compiled in ${pageTime}ms`,
        );
    ...
    return {
      ...
      endPage: end,
      warnings:
        spaceValidation.warnings.length > 0
          ? spaceValidation.warnings
          : undefined,
    };
  } catch (error) {
