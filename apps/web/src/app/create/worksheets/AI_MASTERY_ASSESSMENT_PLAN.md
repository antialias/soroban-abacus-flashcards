...
`;
  ...
  if (!validStep) {
    console.warn(
      `AI suggested invalid step: ${result.suggestedStepId}, defaulting to first step`,
    );
...
async function updateMasteryProfile(
  ...
  if (existing) {
    ...
    const isMastered =
      newAccuracy >= suggestedStep.masteryThreshold &&
...
export function UploadWorksheetModal({ isOpen, onClose }: Props) {
  ...
  return (
    <Dialog open={isOpen} onClose={onClose}>
      ...
      <FileDropzone accept="image/*" onFileSelected={setFile} />
...
export default function AttemptResultsPage({ params }) {
  ...
  const { data: results } = useSuspenseQuery({
    ...
    queryFn: () =>
      fetch(`/api/worksheets/attempts/${attemptId}`).then((r) => r.json()),
  });
...
import {
  ...
  parseWorksheetProblems,
} from "@/lib/ocr/googleVision";
