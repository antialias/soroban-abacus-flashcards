...
function mergeDisplayRulesWithAuto(
  ...
  for (const key of Object.keys(skillRules) as Array<keyof DisplayRules>) {
    ...
    // Otherwise, use user's explicit value (manual override)
    result[key] =
      userValue === "auto" || userValue === undefined
        ? skillRules[key]
        : userValue;
  }
...
export function validateWorksheetConfig(
  ...
  if (total < 1 || total > WORKSHEET_LIMITS.MAX_TOTAL_PROBLEMS) {
    errors.push(
      `Total problems must be between 1 and ${WORKSHEET_LIMITS.MAX_TOTAL_PROBLEMS}`,
    );
  }
  ...
  const pAnyStart =
    formState.pAnyStart !== undefined && formState.pAnyStart !== null
      ? formState.pAnyStart
      : 0.75;
  const pAllStart =
    formState.pAllStart !== undefined && formState.pAllStart !== null
      ? formState.pAllStart
      : 0.25;
  ...
  if (
    fontSize < WORKSHEET_LIMITS.FONT_SIZE.MIN ||
    fontSize > WORKSHEET_LIMITS.FONT_SIZE.MAX
  ) {
    errors.push(
      ...
      if (addSkillId && subSkillId) {
        ...
        if (
          ...
          const userAdditionRules: Partial<DisplayRules> =
            (formState as any).additionDisplayRules ||
            formState.displayRules ||
            {};
          const userSubtractionRules: Partial<DisplayRules> =
            (formState as any).subtractionDisplayRules ||
            formState.displayRules ||
            {};

          console.log(
            '[MIXED MODE SCAFFOLDING] User rules (may contain "auto"):',
            {
          ...
          console.log(
            '[MIXED MODE SCAFFOLDING] Final config (after resolving "auto"):',
            {
          ...
          console.log(
            "[MIXED MODE SCAFFOLDING] Missing recommendedScaffolding",
            {
