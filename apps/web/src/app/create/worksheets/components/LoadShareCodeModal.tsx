 ...
 */
export function LoadShareCodeModal({
  isOpen,
  onClose,
  isDark,
}: LoadShareCodeModalProps) {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!isValidShareId(shareCode)) {
        setError(
          "Invalid share code. Share codes are 7 characters (letters and numbers).",
        );
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            setError(
              "Share code not found. Please check the code and try again.",
            );
          } else {
        ...
        setError(
          "Failed to load shared worksheet. Please check your connection.",
        );
      } finally {
        setIsLoading(false);
      }
        ...
        <form onSubmit={handleSubmit}>
          <div className={stack({ gap: "4" })}>
            ...
            <div>
              ...
              >
                Enter the 7-character share code printed under the QR code on a
                worksheet to load it.
              </p>
            </div>
