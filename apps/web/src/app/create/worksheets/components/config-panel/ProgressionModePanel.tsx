...
export function ProgressionModePanel({
  ...
  const currentStep =
    getStepById(currentStepId, SINGLE_CARRY_PATH) ?? SINGLE_CARRY_PATH[0];
  ...
  const [masteryStates, setMasteryStates] = useState<Map<string, MasteryState>>(
    new Map(),
  );
    ...
    async function loadMasteryStates() {
      try {
        ...
        const response = await fetch(
          "/api/worksheets/mastery?operator=addition",
        );
        if (!response.ok) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className={css({ fontSize: "0.75rem" })}>
          {showAdvanced ? "▲" : "▼"}
        </span>
      </button>
      ...
      {showAdvanced && (
        ...
        >
            ...
            >
              ...
              >
                <input
                  ...
                  name="scaffolding"
                  checked={
                    formState.displayRules?.tenFrames === "whenRegrouping"
                  }
          ...
          >
            ...
            >
              ⚠ Manual changes will move you to the nearest step on the
              progression path
            </p>
          </div>
        </div>
