...
export function MasteryModePanel({
  ...
  const [masteryStates, setMasteryStates] = useState<Map<SkillId, boolean>>(
    new Map(),
  );
  ...
  const getOverrideNotice = () => {
    if (!currentSkill?.recommendedScaffolding || !formState.displayRules)
      return null;
  ...
  useEffect(() => {
    async function loadMasteryStates() {
      try {
        ...
        const operatorsToLoad = isMixedMode
        ...
        for (const op of operatorsToLoad) {
          const response = await fetch(
            `/api/worksheets/mastery?operator=${op}`,
          );
          if (!response.ok) {
  ...
  useEffect(() => {
    ...
    isMixedMode,
    currentAdditionSkill?.id,
    currentSubtractionSkill?.id,
    currentSkill?.id,
  ]);
  ...
  const handlePreviousSkill = () => {
    if (!currentSkill) return;
    const currentIndex = availableSkills.findIndex(
  ...
  const handleSaveCustomization = async (config: {
    ...
    try {
      const response = await fetch(
        `/api/worksheets/skills/${currentSkill.id}/customize`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  if (isMixedMode) {
    ...
    const additionMastered =
      masteryStates.get(currentAdditionSkill.id) ?? false;
    const subtractionMastered =
      masteryStates.get(currentSubtractionSkill.id) ?? false;
    ...
    return (
      ...
      >
        ...
        <div className={css({ marginBottom: "1rem" })}>
          ...
          >
            Practicing operator recognition with problems from both current
            addition and subtraction skill levels
          </p>
        </div>
            ...
            >
              <button
                ...
                onClick={() => {
                  ...
                  if (currentIndex > 0) {
                    onChange({
                      currentAdditionSkillId:
                        additionSkills[currentIndex - 1].id,
                    } as Partial<WorksheetFormState>);
                  }
                }}
                disabled={
                  additionSkills.findIndex(
                    (s) => s.id === currentAdditionSkill.id,
                  ) === 0
                }
                className={css({
            ...
            >
              <button
                ...
                onClick={() => {
                  ...
                  if (currentIndex > 0) {
                    onChange({
                      currentSubtractionSkillId:
                        subtractionSkills[currentIndex - 1].id,
                    } as Partial<WorksheetFormState>);
                  }
                }}
                disabled={
                  subtractionSkills.findIndex(
                    (s) => s.id === currentSubtractionSkill.id,
                  ) === 0
                }
                className={css({
  ...
  const masteredCount = availableSkills.filter(
    ...
    <div data-component="mastery-mode-panel">
      ...
      <CustomizeMixModal
        ...
        currentSelectedReviewSkills={
          formState.selectedReviewSkills as SkillId[] | undefined
        }
