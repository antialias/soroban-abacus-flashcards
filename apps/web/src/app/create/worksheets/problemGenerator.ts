...
export function generateProblems(
  ...
  console.log(
    `[ADD GEN] Estimated unique problem space: ${estimatedSpace} (requesting ${total})`,
  );
  ...
  if (estimatedSpace < THRESHOLD) {
    ...
    const allProblems = generateAllAdditionProblems(
      minDigits,
      maxDigits,
      pAnyStart,
      pAllStart,
    ...
    if (interpolate) {
      ...
      for (let i = 0; i < total; i++) {
        ...
        const windowSize = Math.max(
          3,
          Math.floor(sortedByDifficulty.length * 0.2),
        );
...
export function generateSubtractionProblems(
  ...
  console.log(
    `[SUB GEN] Estimated unique problem space: ${estimatedSpace} (requesting ${count})`,
  );
  ...
  if (estimatedSpace < THRESHOLD && !interpolate) {
    console.log(
      `[SUB GEN] Using generate-all + shuffle (space < ${THRESHOLD})`,
    );
    const allProblems = generateAllSubtractionProblems(
      minDigits,
      maxDigits,
      pAnyBorrow,
      pAllBorrow,
    ...
    if (count > shuffled.length) {
      ...
      while (result.length < count) {
        result.push(
          ...shuffled.slice(
            0,
            Math.min(shuffled.length, count - result.length),
          ),
        );
      }
      const elapsed = Date.now() - startTime;
  ...
  for (let i = 0; i < count; i++) {
    ...
    while (tries++ < 100 && !ok) {
      ...
      if (picked === "both") {
        ...
        [minuend, subtrahend] = generateOnesOnlyBorrow(
          rand,
          minDigits,
          maxDigits,
        );
...
export function generateMasteryMixedProblems(
  ...
  console.log(
    `[MASTERY MIXED] Generating ${count} mixed problems (50/50 split)...`,
  );
  ...
  console.log(
    `[MASTERY MIXED] Step 1: Generating ${addCount} addition problems...`,
  );
  ...
  console.log(
    `[MASTERY MIXED] Step 2: Generating ${subCount} subtraction problems...`,
  );
  ...
  console.log(
    `[MASTERY MIXED] Step 3: ✓ Shuffled in ${Date.now() - shuffleStart}ms`,
  );
