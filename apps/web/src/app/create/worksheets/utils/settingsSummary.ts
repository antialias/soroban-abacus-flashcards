...
export function generateSettingsSummary(config: Partial<WorksheetFormState>): {
  ...
  if (config.operator) {
    ...
    const operatorName =
      config.operator.charAt(0).toUpperCase() + config.operator.slice(1);
    const digitRange = config.digitRange
      ...
      : "";
    lines.push(
      `${operatorIcon} ${operatorName}${digitRange ? ` • ${digitRange}` : ""}`,
    );
  ...
  if (config.mode) {
    const diffIcon =
      SETTING_ICONS.difficulty[
        config.mode as keyof typeof SETTING_ICONS.difficulty
      ];
    const modeName =
      config.mode === "custom"
        ? "Custom difficulty"
