...
export function useWorksheetState(
  ...
  const [formState, setFormState] = useState<WorksheetFormState>(() => {
    const initial = {
      ...
      displayRules:
        initialSettings.displayRules ?? defaultAdditionConfig.displayRules,
  ...
  const [debouncedFormState, setDebouncedFormState] =
    useState<WorksheetFormState>(() => {
      console.log(
        "[useWorksheetState] Initial debouncedFormState (same as formState)",
      );
      return formState;
    });
  ...
  useEffect(() => {
    console.log(
      "[useWorksheetState] debouncedFormState changed - preview will re-fetch:",
      {
  ...
  useEffect(() => {
    ...
    console.log(
      "[useWorksheetState Debounce] Current formState seed:",
      formState.seed,
    );
    console.log(
    ...
    if (formState === prevFormStateRef.current) {
      console.log(
        "[useWorksheetState Debounce] Skipping - formState reference unchanged",
      );
      return;
    }
    ...
    console.log(
      "[useWorksheetState Debounce] Setting timer to update debouncedFormState in 500ms",
    );
    const timer = setTimeout(() => {
      console.log(
        "[useWorksheetState Debounce] Timer fired - updating debouncedFormState",
      );
