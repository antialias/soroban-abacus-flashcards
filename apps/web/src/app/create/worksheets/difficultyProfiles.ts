 ...
 */
export function findRegroupingIndex(
  pAnyStart: number,
  pAllStart: number,
): number {
  let bestIndex = 0;
...
function describeScaffoldingChange(
  ...
  const additionOnlyScaffolds: Array<keyof DisplayRules> = [
    "carryBoxes",
    "tenFrames",
  ];
  const subtractionOnlyScaffolds: Array<keyof DisplayRules> = [
    "borrowNotation",
    "borrowingHints",
  ];
 ...
 */
function clampScaffoldingToValidRange(
  regroupingIdx: number,
  scaffoldingIdx: number,
): number {
  const range = getValidScaffoldingRange(regroupingIdx);
...
export function findNearestValidState(
  ...
  const clampedRegrouping = Math.max(
    0,
    Math.min(REGROUPING_PROGRESSION.length - 1, regroupingIdx),
  );
  const clampedScaffolding = clampScaffoldingToValidRange(
    clampedRegrouping,
    scaffoldingIdx,
  );
 ...
 */
export function calculateRegroupingIntensity(
  pAnyStart: number,
  pAllStart: number,
): number {
...
export function calculateScaffoldingLevel(
  ...
  const includeTenFrames =
    regroupingIntensity === undefined || regroupingIntensity >= 4;
...
export function calculateOverallDifficulty(
  ...
  const distFromEnd = Math.sqrt(
    (regrouping - endPoint.regrouping) ** 2 +
      (scaffolding - endPoint.scaffolding) ** 2,
  );
...
export function makeHarder(
  ...
  if (mode === "challenge") {
    ...
    const nearestPreset = findNearestPreset(
      currentRegrouping,
      currentScaffolding,
      "harder",
    );
    ...
    if (!moved) {
      ...
      if (newRegroupingIdx < REGROUPING_PROGRESSION.length - 1) {
        newRegroupingIdx++;
        newScaffoldingIdx = clampScaffoldingToValidRange(
          newRegroupingIdx,
          newScaffoldingIdx,
        );
        moved = true;
