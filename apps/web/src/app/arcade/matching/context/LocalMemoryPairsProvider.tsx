...
function localMemoryPairsReducer(
  ...
  switch (action.type) {
    case "START_GAME":
      return {
        ...
        matchedPairs: 0,
        moves: 0,
        scores: action.activePlayers.reduce(
...
export function LocalMemoryPairsProvider({
  children,
}: {
  children: ReactNode;
}) {
  ...
  const startGame = useCallback(() => {
    if (activePlayers.length === 0) {
      console.error(
        "[LocalMemoryPairs] Cannot start game without active players",
      );
      return;
    }
  ...
  const resetGame = useCallback(() => {
    if (activePlayers.length === 0) {
      console.error(
        "[LocalMemoryPairs] Cannot reset game without active players",
      );
      return;
    }
  ...
  const resumeGame = useCallback(() => {
    if (!canResumeGame) {
      console.warn(
        "[LocalMemoryPairs] Cannot resume - no paused game or config changed",
      );
      return;
    }
  ...
  const contextValue: MemoryPairsContextValue = {
    ...
    dispatch: () => {
      ...
      console.warn(
        "dispatch() is not available in local mode, use action creators instead",
      );
    },
    isGameActive,
    canFlipCard,
