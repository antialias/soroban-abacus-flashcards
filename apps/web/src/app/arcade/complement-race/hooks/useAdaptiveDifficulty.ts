...
export function useAdaptiveDifficulty() {
  ...
  const trackPerformance = (isCorrect: boolean, responseTime: number) => {
    ...
    const pairData: PairPerformance =
      state.difficultyTracker.pairPerformance.get(pairKey) || {
        attempts: 0,
        correct: 0,
        avgTime: 0,
        difficulty: 1,
    ...
    const newTracker = {
      ...
      consecutiveCorrect: isCorrect
        ? state.difficultyTracker.consecutiveCorrect + 1
        : 0,
      consecutiveIncorrect: !isCorrect
        ? state.difficultyTracker.consecutiveIncorrect + 1
        : 0,
    };
    ...
    if (newTracker.consecutiveCorrect >= 3) {
      ...
      newTracker.currentTimeLimit = Math.max(
        1000,
        newTracker.currentTimeLimit -
          newTracker.currentTimeLimit * newTracker.adaptationRate,
      );
      ...
      newTracker.currentTimeLimit = Math.min(
        5000,
        newTracker.currentTimeLimit +
          newTracker.baseTimeLimit * newTracker.adaptationRate,
      );
    }
    ...
    if (
      ...
      Array.from(newTracker.pairPerformance.values()).some(
        (data) => data.attempts >= 3,
      )
    ) {
  ...
  const calculateRecentSuccessRate = (): number => {
    ...
    const recentCorrect = Math.max(
      ...
      state.correctAnswers -
        Math.max(0, state.totalQuestions - recentQuestions),
    );
  ...
  const calculateAverageResponseTime = (): number => {
    const recentPairs = Array.from(
      state.difficultyTracker.pairPerformance.values(),
    )
  ...
  const adaptAISpeeds = (tracker: typeof state.difficultyTracker) => {
    ...
    if (state.totalQuestions % 5 === 0) {
      console.log("🤖 AI Speed Adaptation:", {
        ...
        swiftAISpeed: updatedRacers[0]
          ? Math.round(updatedRacers[0].speed * 1000) / 1000
          : 0,
        mathBotSpeed: updatedRacers[1]
          ? Math.round(updatedRacers[1].speed * 1000) / 1000
          : 0,
      });
    }
  };
    ...
    if (pairData && pairData.attempts >= 3) {
      ...
      if (accuracy < 0.6) {
        ...
        return {
          message:
            strugglingMessages[
              Math.floor(Math.random() * strugglingMessages.length)
            ],
          type: "struggling",
        };
      }
      ...
      if (accuracy > 0.85 && avgTime < 2000) {
        ...
        return {
          message:
            masteredMessages[
              Math.floor(Math.random() * masteredMessages.length)
            ],
          type: "mastered",
        };
      }
