  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (trainPosition < 1 && state.passengers.length > 0) {
        ...
        if (lastLoggedRoute !== state.currentRoute) {
          ...
          state.passengers.forEach((p) => {
            const origin = state.stations.find(
              (s) => s.id === p.originStationId,
            );
      ...
      currentBoardedPassengers.forEach((passenger) => {
        ...
        if (
          !passengersToDeliver.has(passenger.id) &&
          passenger.carIndex !== null
        ) {
      ...
      const isAtStation = state.stations.some(
        (s) => Math.abs(trainPosition - s.position) < 3,
      );
      if (
        isAtStation &&
        Math.floor(trainPosition) !== Math.floor(state.trainPosition)
      ) {
        const nearStation = state.stations.find(
          (s) => Math.abs(trainPosition - s.position) < 3,
        );
        console.log(
        ...
        for (let i = 0; i < maxCars; i++) {
          ...
          if (occupant) {
            const dest = state.stations.find(
              (s) => s.id === occupant.destinationStationId,
            );
            console.log(
