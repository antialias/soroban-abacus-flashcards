...
describe("TrainTerrainBackground", () => {
  ...
  test("renders ground layer rects", () => {
    ...
    const groundRect = Array.from(rects).find(
      (rect) =>
        rect.getAttribute("fill") === "#8B7355" &&
        rect.getAttribute("width") === "900",
    );
  ...
  test("renders ballast path with correct attributes", () => {
    ...
    const ballastPath = Array.from(container.querySelectorAll("path")).find(
      (path) =>
        path.getAttribute("d") === "M 0 300 L 800 300" &&
        path.getAttribute("stroke") === "#8B7355",
    );
  ...
  test("renders right tunnel structure", () => {
    ...
    const rightTunnel = container.querySelector(
      '[data-element="right-tunnel"]',
    );
  ...
  test("renders mountains with gradient fills", () => {
    ...
    const gradientPaths = Array.from(container.querySelectorAll("path")).filter(
      (path) => path.getAttribute("fill")?.includes("url(#mountainGradient"),
    );
  ...
  test("handles empty groundTextureCircles array", () => {
    ...
    expect(
      container.querySelector('[data-element="left-tunnel"]'),
    ).toBeTruthy();
  });
