 ...
 */
function calculateCarOpacity(
  ghostCarPosition: number,
  localCarPositions: number[],
): number {
  // Find minimum distance to any local car
  const minDistance = Math.min(
    ...localCarPositions.map((localPos) =>
      Math.abs(ghostCarPosition - localPos),
    ),
  );
...
export function GhostTrain({
  ...
  const locomotiveTarget = useMemo<CarTransform | null>(() => {
    ...
    const rotation =
      (Math.atan2(tangentPoint.y - point.y, tangentPoint.x - point.x) * 180) /
      Math.PI;
