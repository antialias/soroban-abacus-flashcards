...
export function PracticeClient({
  ...
  const getRetryContext = useCallback(
    (partIndex: number, slotIndex: number) => {
      ...
      const isManualRedo =
        redoState?.isActive && redoState?.originalSlotIndex === slotIndex;
      ...
      const { attemptCount } = getSlotRetryStatus(
        currentPlan,
        partIndex,
        slotIndex,
      );
  ...
  const handlePartTransitionComplete = useCallback(() => {
    ...
    if (pendingGameBreak) {
      ...
      console.log(
        "[PracticeClient] No pending game break, continuing to practice",
      );
    }
  ...
  useEffect(() => {
    ...
    setVisionFrameCallback((frame) => {
      ...
      if (!hasStartedRecordingRef.current && !isRecording) {
        console.log(
          "[PracticeClient] First vision frame received, starting recording",
        );
