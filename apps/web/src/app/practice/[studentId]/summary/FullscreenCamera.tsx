...
export function FullscreenCamera({
  ...
  const [presetMode, setPresetMode] = useState<
    "auto" | "normal" | "lowLight" | "bright"
  >("auto");
  const [detectedPreset, setDetectedPreset] = useState<
    "normal" | "lowLight" | "bright"
  >("normal");
  ...
  const drawerSwipeRef = useRef<{ startX: number; startY: number } | null>(
    null,
  );
            ...
            <div className={css({ position: "relative" })}>
              ...
              >
                ...
                <span className={css({ fontSize: "xs" })}>
                  {presetMode === "auto"
                    ? "Auto"
                    : LIGHTING_PRESETS[presetMode].label}
                </span>
              </button>
              ...
              {presetPopoverOpen && (
                <>
                  ...
                  >
                    ...
                    {/* Preset options */}
                    {(
                      Object.keys(LIGHTING_PRESETS) as Array<
                        keyof typeof LIGHTING_PRESETS
                      >
                    ).map((key) => (
                      <button
                        type="button"
