    ...
    case "progression": {
      return {
        ...
        context: {
          ...
          detail: sessionMode.tutorialRequired
            ? "Tutorial available"
            : "Ready to practice",
        },
        nextSkill: {
...
function processSkills(
  ...
  return skills.map((skill) => {
    ...
    if (
      bkt?.masteryClassification === undefined ||
      bkt?.masteryClassification === null
    ) {
      if (attempts === 0) {
      ...
      >
        ...
        {skill.stalenessWarning && onMarkCurrent && (
          ...
          >
            ...
            >
              ...
              <div>
                ...
                {skill.lastPracticedAt &&
                  skill.daysSinceLastPractice !== null && (
                    <div
                      className={css({
                        fontSize: "0.75rem",
                        color: isDark ? "orange.400" : "orange.600",
            ...
            >
              If the student has practiced this skill offline, mark it as
              current to reset the staleness timer.
            </p>
          </div>
        )}
              ...
              <div>
                ...
                >
                  {skill.insufficientDataReason.type === "never-practiced" && (
                    <>
                      This skill hasn&apos;t been practiced yet. We can&apos;t
                      assess mastery without practice data.
                    </>
                  )}
                  {skill.insufficientDataReason.type === "too-few-attempts" && (
                    <>
                      Only {skill.insufficientDataReason.attempts} attempts
                      recorded. Reliable assessment requires approximately{" "}
                      {skill.insufficientDataReason.needed}+ attempts.
                    </>
                  )}
                  {skill.insufficientDataReason.type === "low-confidence" && (
                    <>
                      Results have been mixed ({skill.correct}/{skill.attempts}{" "}
                      correct). We can&apos;t confidently classify this skill
                      yet.
                    </>
                  )}
                </div>
              ...
              <div>
                ...
                >
                  ...
                  {skill.insufficientDataReason.type === "too-few-attempts" && (
                    <>
                      Practice{" "}
                      {skill.insufficientDataReason.needed -
                        skill.insufficientDataReason.attempts}{" "}
                      more problems to get a reliable assessment.
                    </>
                  )}
                  {skill.insufficientDataReason.type === "low-confidence" && (
                    <>
                      Keep practicing — consistent results will help us assess
                      your mastery.
                    </>
                  )}
                </div>
            ...
            {!Number.isFinite(skill.pKnown) ? (
              ...
              <>
                ...
                {skill.uncertaintyRange &&
                  Number.isFinite(skill.uncertaintyRange.low) && (
                    <div
                      className={css({
                        fontSize: "0.75rem",
                        color: isDark ? "gray.500" : "gray.500",
...
function SkillsTab({
  ...
  const [activeFilters, setActiveFilters] = useState<Set<SkillClassification>>(
    new Set(),
  );
  ...
  const needsAssessment = useMemo(
    () =>
      practicingSkills.filter(
        (s) =>
          s.bktClassification === null && s.insufficientDataReason !== null,
      ),
    [practicingSkills],
  );
  ...
  const skillsNeedingAttention = useMemo(() => {
    ...
    const skillMap = new Map<
      string,
      { skill: ProcessedSkill; isWeak: boolean; isStale: boolean }
    >();
  ...
  const shownElsewhereByCategory = useMemo(() => {
    const byCategory = new Map<
      string,
      { attention: number; assessment: number }
    >();
  ...
  const skillsByCategory = useMemo(() => {
    const remainingSkills = practicingSkills.filter(
      (s) => !shownElsewhereIds.has(s.skillId),
    );
  ...
  const currentDistribution = useMemo<SkillDistribution>(() => {
    ...
    for (const skill of practicingSkills) {
      const classification = getSkillClassification(
        skill.bktClassification,
        skill.stalenessWarning,
      );
      dist[classification]++;
  ...
  const allSkillIds = useMemo(
    () => practicingSkills.map((s) => s.skillId),
    [practicingSkills],
  );
    ...
    <div data-tab-content="skills">
      ...
      {filteredSkillsNeedingAttention.length > 0 && (
        ...
        >
          ...
          >
            {filteredSkillsNeedingAttention.map(
              ({ skill, isWeak, isStale }) => {
      ...
      {filteredSkillsByCategory.length > 0 && (
        <div
          data-section="on-track"
          className={css({ marginTop: { base: "1rem", sm: "1.5rem" } })}
        >
          <h2
            className={css({
          ...
          {filteredSkillsByCategory.map(({ category, skills }) => {
            ...
            const hasElsewhere =
              elsewhere &&
              (elsewhere.attention > 0 || elsewhere.assessment > 0);
            return (
              <div
                key={category.id}
...
export function DashboardClient({
  ...
  const skillHealth = useMemo(() => {
    ...
    const practicingSkills = processedSkillsForHealth.filter(
      (s) => s.isPracticing,
    );
          ...
          {isObserving && activeSession && (
            <SessionObserverModal
              ...
              session={{
                ...
                totalProblems:
                  activeSession.parts?.reduce(
                    (sum, p) => sum + p.slots.length,
                    0,
                  ) ?? 0,
                completedProblems:
                  activeSession.results?.filter((r) => r.isCorrect !== null)
                    .length ?? 0,
              }}
              student={{
                name: player.name,
