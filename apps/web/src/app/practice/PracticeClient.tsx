...
const AddStudentModal = dynamic(
  () => import("./AddStudentModal").then((m) => m.AddStudentModal),
  {
    ssr: false,
  },
);
...
export function PracticeClient({
  ...
  const archivedCount = useMemo(
    () => players.filter((p) => p.isArchived).length,
    [players],
  );
  ...
  const [observingStudent, setObservingStudent] =
    useState<UnifiedStudent | null>(null);
        ...
        categories: bucket.categories
          .map((category) => ({
            ...
            students: category.students.filter(
              (s) => !attentionStudentIds.has(s.id),
            ),
          }))
  ...
  const promptEligibleIds = useMemo(() => {
    ...
    return new Set(
      Array.from(selectedIds).filter((id) => {
        ...
        return (
          student.relationship.isEnrolled && !student.relationship.isPresent
        );
      }),
    );
  ...
  const handleBulkPromptToEnter = useCallback(async () => {
    ...
    try {
      const result = await bulkEntryPrompt.mutateAsync(
        Array.from(promptEligibleIds),
      );
  ...
  const handleObserveSession = useCallback(
    (sessionId: string) => {
      ...
      const student = unifiedStudents.find(
        (s) => s.activity?.sessionId === sessionId,
      );
      if (student) {
  ...
  const hiddenArchivedCount = useMemo(() => {
    ...
    return filterStudents(players, searchQuery, skillFilters, true).filter(
      (p) => p.isArchived,
    ).length;
      ...
      >
        ...
        <StudentFilterBar
          ...
          onAddStudentToClassroom={
            isTeacher ? handleAddStudentToClassroom : undefined
          }
        ...
        >
          ...
          {isTeacher && classroomId && (
            <TeacherEnrollmentSection classroomId={classroomId} />
          )}
      ...
      {observingStudent && observingStudent.activity?.sessionId && (
        <SessionObserverModal
          ...
          session={{
            ...
            completedProblems:
              observingStudent.activity.sessionProgress?.current ?? 0,
            totalProblems:
              observingStudent.activity.sessionProgress?.total ?? 0,
      ...
      return (
        ...
        >
          ...
          <div>
            ...
            >
              Enrolled students can enter via their practice page, or you can
              prompt them to join.
            </p>
          </div>
