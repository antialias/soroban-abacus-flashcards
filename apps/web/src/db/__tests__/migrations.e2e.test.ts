...
describe("Migrations E2E", () => {
  ...
  it("cascades deletes from users to players", () => {
    ...
    sqlite
      .prepare(
        "INSERT INTO users (id, guest_id, created_at) VALUES ('u1', 'g1', 0)",
      )
      .run();
    ...
    const playerBefore = sqlite
      .prepare("SELECT * FROM players WHERE id = 'p1'")
      .get();
  ...
  it("cascades deletes from users to user_stats", () => {
    ...
    // Verify stats exist
    const statsBefore = sqlite
      .prepare("SELECT * FROM user_stats WHERE user_id = 'u1'")
      .get();
