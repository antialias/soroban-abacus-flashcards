...
export const visionTrainingSessions = sqliteTable(
  "vision_training_sessions",
  {
    ...
    datasetInfo: text("dataset_info", { mode: "json" })
      .notNull()
      .$type<DatasetInfo>(),
    ...
    epochHistory: text("epoch_history", { mode: "json" })
      .notNull()
      .$type<EpochData[]>(),
  ...
  (table) => ({
    ...
    modelTypeIdx: index("vision_training_sessions_model_type_idx").on(
      table.modelType,
    ),

    /** Index for finding active models */
    isActiveIdx: index("vision_training_sessions_is_active_idx").on(
      table.isActive,
    ),

    /** Index for sorting by creation date */
    createdAtIdx: index("vision_training_sessions_created_at_idx").on(
      table.createdAt,
    ),
...
export type NewVisionTrainingSession =
  typeof visionTrainingSessions.$inferInsert;
...
export function toVisionSessionSummary(
  ...
  return {
    ...
    finalPixelError:
      result.type === "boundary-detector"
        ? result.final_pixel_error
        : undefined,
  };
}
