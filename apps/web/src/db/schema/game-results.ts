...
export const gameResults = sqliteTable(
  ...
  (table) => ({
    ...
    playerGameIdx: index("game_results_player_game_idx").on(
      table.playerId,
      table.gameName,
    ),
    /** Index for game-wide leaderboards */
    gameScoreIdx: index("game_results_game_score_idx").on(
      table.gameName,
    ...
    playerTimeIdx: index("game_results_player_time_idx").on(
      table.playerId,
      table.playedAt,
    ),
  }),
