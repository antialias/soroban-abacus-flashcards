...
export function generateCalendarComposite(
  options: CalendarCompositeOptions,
): string {
  ...
  const MAX_SCALE_Y =
    (DAY_CELL_HEIGHT - CELL_PADDING * 2) / ABACUS_NATURAL_HEIGHT;
  ...
  ${(() => {
    ...
    const yearAbacusContent = yearAbacusSVG
      .replace(/<svg[^>]*>/, "")
      .replace(/<\/svg>$/, "");
  ...
  ${calendarCells
    .map((day, index) => {
      ...
      const croppedWidth = widthMatch
        ? parseFloat(widthMatch[1])
        : ABACUS_NATURAL_WIDTH;
      const croppedHeight = heightMatch
        ? parseFloat(heightMatch[1])
        : ABACUS_NATURAL_HEIGHT;
