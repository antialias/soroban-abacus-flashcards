...
export function skillConfigurationToSkillSets(config: SkillConfiguration): {
  ...
  Object.entries(config.fiveComplements).forEach(([skill, mode]) => {
    if (mode === "allowed" || mode === "target") {
      required.fiveComplements[skill as keyof typeof required.fiveComplements] =
        true;
    }
    if (mode === "target") {
      if (!target.fiveComplements) target.fiveComplements = {} as any;
      target.fiveComplements![skill as keyof typeof required.fiveComplements] =
        true;
    }
    if (mode === "forbidden") {
      if (!forbidden.fiveComplements) forbidden.fiveComplements = {} as any;
      forbidden.fiveComplements![
        skill as keyof typeof required.fiveComplements
      ] = true;
    }
  });
  ...
  Object.entries(config.tenComplements).forEach(([skill, mode]) => {
    if (mode === "allowed" || mode === "target") {
      required.tenComplements[skill as keyof typeof required.tenComplements] =
        true;
    }
    if (mode === "target") {
      if (!target.tenComplements) target.tenComplements = {} as any;
      target.tenComplements![skill as keyof typeof required.tenComplements] =
        true;
    }
    if (mode === "forbidden") {
      if (!forbidden.tenComplements) forbidden.tenComplements = {} as any;
      forbidden.tenComplements![skill as keyof typeof required.tenComplements] =
        true;
    }
  });
