...
function generateStepExplanation(
  ...
  for (const skill of skills) {
    ...
    if (skill === "basic.directAddition") {
      ...
      if (match) {
        explanations.push(
          `ten complement: +${match[1]} = +10-${match[2]} (carry)`,
        );
      }
    }
    // Subtraction skills
      ...
      if (match) {
        explanations.push(
          `ten complement: -${match[1]} = +${match[2]}-10 (borrow)`,
        );
      }
    }
  }
  ...
  const resultOnes = isSubtraction
    ? (before - term) % 10
    : (before + term) % 10;
 ...
 */
export function analyzeStepSkills(
  currentValue: number,
  term: number,
  _newValue: number,
): string[] {
  ...
  try {
    const sequence = generateUnifiedInstructionSequence(
      currentValue,
      targetValue,
    );
...
function isSkillEnabled(
  skillPath: string,
  skillSet: SkillSet | Partial<SkillSet>,
): boolean {
  ...
  if (category === "basic" && skillSet.basic) {
    ...
    return (
      skillSet.fiveComplements[
        skill as keyof typeof skillSet.fiveComplements
      ] || false
    );
  } else if (category === "tenComplements" && skillSet.tenComplements) {
    return (
      skillSet.tenComplements[skill as keyof typeof skillSet.tenComplements] ||
      false
    );
  } else if (category === "fiveComplementsSub" && skillSet.fiveComplementsSub) {
    return (
      skillSet.fiveComplementsSub[
        skill as keyof typeof skillSet.fiveComplementsSub
      ] || false
    );
  } else if (category === "tenComplementsSub" && skillSet.tenComplementsSub) {
    return (
      skillSet.tenComplementsSub[
        skill as keyof typeof skillSet.tenComplementsSub
      ] || false
    );
...
export function generateSingleProblemWithDiagnostics(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const termCount =
      Math.floor(Math.random() * (maxTerms - minTerms + 1)) + minTerms;
    ...
    if (problemSkills.some((skill) => skill.startsWith("tenComplements"))) {
      ...
      problemSkills.some((skill) => skill.startsWith("fiveComplements"))
    ) {
      difficulty = "medium";
    }
...
function generateSequence(
  ...
  return {
    ...
    trace: {
      ...
      totalComplexityCost:
        totalComplexityCost > 0 ? totalComplexityCost : undefined,
      skillMasteryContext,
    },
  };
}
...
function findStateDependentTargetSkills(
  ...
  return STATE_DEPENDENT_SKILLS.filter((depSkill) =>
    isSkillEnabled(depSkill.skillId, targetSkills),
  );
}
...
function collectValidTerms(
  ...
  const processTerm = (term: number, isSubtraction: boolean) => {
    ...
    const usesValidSkills = stepSkills.every((skillPath) => {
      ...
      if (forbiddenSkills && isSkillEnabled(skillPath, forbiddenSkills))
        return false;
      return true;
    });
    ...
    const termCost = costCalculator
      ? costCalculator.calculateTermCost(stepSkills)
      : undefined;
...
function findValidNextTermWithTrace(
  ...
  if (targetSkills && !isLastTerm) {
    const targetCandidates = candidates.filter((candidate) =>
      candidate.skillsUsed.some((skillPath) =>
        isSkillEnabled(skillPath, targetSkills),
      ),
    );
    ...
    if (stateDependentTargets.length > 0) {
      ...
      const setupCandidates = candidates.filter((candidate) =>
        stateDependentTargets.some(
          (depSkill) =>
            ...
            depSkill.wouldSetupState(
              currentValue,
              candidate.term,
              candidate.isSubtraction,
            ),
        ),
      );
      ...
      if (setupCandidates.length > 0) {
        return setupCandidates[
          Math.floor(Math.random() * setupCandidates.length)
        ];
      }
      ...
      if (progressCandidates.length > 0) {
        return progressCandidates[
          Math.floor(Math.random() * progressCandidates.length)
        ];
      }
    }
  }
 ...
 */
function generateSequentialExplanation(
  terms: number[],
  sum: number,
  skills: string[],
): string {
  ...
  if (skills.includes("basic.heavenBead")) {
    explanations.push(
      "Use the heaven bead when working with 5 or making totals involving 5.",
    );
  }
  ...
  if (skills.some((skill) => skill.startsWith("fiveComplements."))) {
    const complements = skills.filter((skill) =>
  ...
  if (skills.some((skill) => skill.startsWith("tenComplements."))) {
    const complements = skills.filter((skill) =>
      skill.startsWith("tenComplements."),
    );
    explanations.push(
  ...
  if (skills.some((skill) => skill.startsWith("fiveComplementsSub."))) {
    const complements = skills.filter((skill) =>
      skill.startsWith("fiveComplementsSub."),
    );
    explanations.push(
  ...
  if (skills.some((skill) => skill.startsWith("tenComplementsSub."))) {
    const complements = skills.filter((skill) =>
      skill.startsWith("tenComplementsSub."),
    );
    explanations.push(
...
function isDuplicateProblem(
  ...
  return existingProblems.some(
    (existing) => getProblemSignature(existing.terms) === signature,
  );
}
...
export function generateProblems(
  ...
  while (
    ...
    if (problem) {
      ...
      if (
        !problemSignatures.has(signature) &&
        !isDuplicateProblem(problem, problems)
      ) {
        ...
        if (consecutiveFailures > practiceStep.problemCount * 5) {
          console.warn(
            "Too many duplicate problems generated. Constraints may be too restrictive.",
          );
          break;
        }
      }
  ...
  while (problems.length < practiceStep.problemCount) {
    ...
    if (!isDuplicateProblem(fallbackProblem, problems)) {
      ...
      const modifiedProblem = createModifiedUniqueProblem(
        fallbackProblem,
        problems,
        constraints,
      );
...
function generateFallbackProblem(
  ...
  return {
    ...
    difficulty: "easy",
    explanation: generateSequentialExplanation(terms, sum, [
...
export function validatePracticeStepConfiguration(practiceStep: PracticeStep): {
  ...
  if (!hasAnyAllowedSkill) {
    ...
    suggestions.push(
      'Enable at least one skill in the "Allowed Skills" section.',
    );
  }
  ...
  if (
    practiceStep.sumConstraints?.maxSum &&
    practiceStep.sumConstraints.maxSum > maxPossibleSum
  ) {
    warnings.push(
      "Maximum sum constraint is higher than what the number range allows.",
    );
    suggestions.push(
      "Either increase the number range maximum or decrease the sum constraint.",
    );
  }
  ...
  if (
    practiceStep.sumConstraints?.maxSum &&
    practiceStep.sumConstraints.maxSum < 5
  ) {
    warnings.push("Very low sum constraint may limit problem variety.");
    suggestions.push(
      "Consider increasing the maximum sum to allow more diverse problems.",
    );
  }
  ...
  if (practiceStep.problemCount > 20) {
    warnings.push(
      "High problem count may take a long time to generate and complete.",
    );
    suggestions.push(
      "Consider reducing the problem count for better user experience.",
    );
  }
