...
export function analyzeRequiredSkills(
  ...
  for (const term of terms) {
    ...
    const requiredSkillsForStep = analyzeStepSkills(
      currentValue,
      term,
      newValue,
    );
...
function analyzeStepSkills(
  ...
  for (let column = 0; column < maxColumns; column++) {
    ...
    const columnSkills = analyzeColumnAddition(
      currentDigit,
      termDigit,
      newDigit,
      column,
...
export function problemMatchesSkills(
  ...
  const hasRequiredSkill = problem.requiredSkills.some((skillPath) => {
    ...
    if (category === "basic") {
      ...
      return requiredSkills.fiveComplements[
        skill as keyof typeof requiredSkills.fiveComplements
      ];
    } else if (category === "tenComplements") {
      return requiredSkills.tenComplements[
        skill as keyof typeof requiredSkills.tenComplements
      ];
    }
    return false;
  });
  ...
  if (forbiddenSkills) {
    const usesForbiddenSkill = problem.requiredSkills.some((skillPath) => {
      ...
      if (category === "basic" && forbiddenSkills.basic) {
        return forbiddenSkills.basic[
          skill as keyof typeof forbiddenSkills.basic
        ];
      } else if (
        ...
        forbiddenSkills.tenComplements
      ) {
        return forbiddenSkills.tenComplements[
          skill as keyof typeof forbiddenSkills.tenComplements
        ];
      }
      return false;
    });
  ...
  if (targetSkills) {
    const hasTargetSkill = problem.requiredSkills.some((skillPath) => {
      ...
      if (category === "basic" && targetSkills.basic) {
        ...
        return targetSkills.fiveComplements[
          skill as keyof typeof targetSkills.fiveComplements
        ];
      } else if (category === "tenComplements" && targetSkills.tenComplements) {
        return targetSkills.tenComplements[
          skill as keyof typeof targetSkills.tenComplements
        ];
      }
      return false;
    });
...
function findValidNextTerm(
  ...
  for (let term = min; term <= max; term++) {
    ...
    const usesValidSkills = stepSkills.every((skillPath) => {
      ...
      if (category === "basic") {
        hasSkill =
          requiredSkills.basic[skill as keyof typeof requiredSkills.basic];
      } else if (category === "fiveComplements") {
        hasSkill =
          requiredSkills.fiveComplements[
            skill as keyof typeof requiredSkills.fiveComplements
          ];
      } else if (category === "tenComplements") {
        hasSkill =
          requiredSkills.tenComplements[
            skill as keyof typeof requiredSkills.tenComplements
          ];
      }
...
function generateSequentialExplanation(
  ...
  explanations.push(
    `Calculate this problem by adding each number in sequence:\n${verticalDisplay}`,
  );
...
export function validatePracticeStepConfiguration(practiceStep: PracticeStep): {
  ...
  if (!hasAnyRequiredSkill) {
    warnings.push(
      "No required skills are enabled. Problems may be very basic.",
    );
    suggestions.push(
      'Enable at least one skill in the "Required Skills" section.',
    );
  }
