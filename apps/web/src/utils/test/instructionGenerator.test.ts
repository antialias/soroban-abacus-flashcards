...
describe("Automatic Abacus Instruction Generator", () => {
  ...
  describe("generateAbacusInstructions", () => {
    ...
    it("should generate correct instructions for five complement", () => {
      ...
      const heavenBead = instruction.highlightBeads.find(
        (b) => b.beadType === "heaven",
      );
      expect(heavenBead).toBeDefined();

      // Should highlight earth bead to remove (the last one in the sequence)
      const earthBead = instruction.highlightBeads.find(
        (b) => b.beadType === "earth",
      );
    ...
    it("should generate correct instructions for ten complement", () => {
      ...
      const onesBeads = instruction.highlightBeads.filter(
        (b) => b.placeValue === 0,
      );
    ...
    it("should generate correct instructions for multi-place operations", () => {
      ...
      const tensBeads = instruction.highlightBeads.filter(
        (b) => b.placeValue === 1,
      );

      expect(onesBeads.length + tensBeads.length).toBe(
        instruction.highlightBeads.length,
      );
      expect(instruction.expectedAction).toBe("multi-step");
    });
  });
  ...
  describe("validateInstruction", () => {
    ...
    it("should catch missing multi-step instructions", () => {
      ...
      expect(validation.issues).toContain(
        "Multi-step action without step instructions",
      );
    });
  });
  ...
  describe("Stress testing with random operations", () => {
    ...
    it("should handle 50 random subtraction operations", () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        const subtractionAmount =
          Math.floor(Math.random() * Math.min(start, 10)) + 1; // 1 to min(start, 10)
    ...
    it("should handle all systematic single-digit operations", () => {
      ...
      if (failedCases.length > 0) {
        console.error(
          "Failed systematic single-digit test cases:",
          failedCases,
        );
      }
  ...
  describe("Traditional abacus complement descriptions", () => {
    ...
    it("should handle large ten complement correctly", () => {
      ...
      expect(instruction.errorMessages.hint).toContain(
        "3 + 98 = 101, using if 98 = 100 - 2",
      );
    });
      ...
      const actualTenComplementCases = [
        {
          ...
          target: 11,
          description:
            "7 + 4 where 4 requires five complement which triggers ten complement",
        },
        {
          start: 6,
          target: 13,
      ...
      it("should consistently use compact math format for complements", () => {
        ...
        complementCases.forEach(({ start, target }) => {
          ...
          if (instruction.expectedAction === "multi-step") {
            ...
            expect(instruction.actionDescription).toMatch(
              /\d+ \+ \d+ = \d+ \+ \(\d+ - \d+\)/,
            );
            // Should NOT show the final arithmetic chain
            expect(instruction.actionDescription).not.toMatch(
              /= \d+ - \d+ = \d+$/,
            );
          }
        });
  ...
  describe("Progressive Step-Bead Mapping", () => {
    ...
    it("should generate correct pedagogical step ordering for 3 + 14 = 17 using generic algorithm", () => {
      ...
      expect(instruction.multiStepInstructions![0]).toContain(
        "earth bead 1 in the tens column",
      );
      expect(instruction.multiStepInstructions![1]).toContain(
        "heaven bead in the ones column",
      );
    ...
    describe("Pedagogical step ordering algorithm", () => {
      it("should use generic algorithm for 27 + 36 = 63 (multi-place with complement)", () => {
        ...
        const stepBeads = instruction.stepBeadHighlights!;
        const additionSteps = stepBeads.filter(
          (b) => b.direction === "activate",
        );
        const subtractionSteps = stepBeads.filter(
          (b) => b.direction === "deactivate",
        );
        ...
        if (additionSteps.length > 0 && subtractionSteps.length > 0) {
          const lastAdditionStep = Math.max(
            ...additionSteps.map((b) => b.stepIndex),
          );
          const firstSubtractionStep = Math.min(
            ...subtractionSteps.map((b) => b.stepIndex),
          );
      ...
      it("should use generic algorithm for 87 + 26 = 113 (crossing hundreds)", () => {
        ...
        if (
          hundredsBeads.length > 0 &&
          (tensBeads.length > 0 || onesBeads.length > 0)
        ) {
          const maxHundredsStep = Math.max(
            ...hundredsBeads.map((b) => b.stepIndex),
          );
        ...
        for (let start = 0; start <= 9; start++) {
          ...
          try {
            ...
            if (instruction.totalSteps && instruction.totalSteps > 1) {
              const stepBeads = instruction.stepBeadHighlights!;
              const placeValues = [
                ...new Set(stepBeads.map((b) => b.placeValue)),
              ].sort((a, b) => b - a);

              // For each place value, additions should come before subtractions
              placeValues.forEach((place) => {
                const placeBeads = stepBeads.filter(
                  (b) => b.placeValue === place,
                );
                const additions = placeBeads.filter(
                  (b) => b.direction === "activate",
                );
                const subtractions = placeBeads.filter(
                  (b) => b.direction === "deactivate",
                );

                if (additions.length > 0 && subtractions.length > 0) {
                  const lastAddition = Math.max(
                    ...additions.map((b) => b.stepIndex),
                  );
                  const firstSubtraction = Math.min(
                    ...subtractions.map((b) => b.stepIndex),
                  );
    ...
    describe("Anti-cheat verification", () => {
      it("should not contain hard-coded special cases for specific problems", () => {
        ...
        testCases.forEach(({ start, target }) => {
          ...
          expect(instruction1.stepBeadHighlights).toEqual(
            instruction2.stepBeadHighlights,
          );
          expect(instruction1.totalSteps).toBe(instruction2.totalSteps);
          expect(instruction1.multiStepInstructions).toEqual(
            instruction2.multiStepInstructions,
          );
        });
      });
      ...
      it("should generate identical results for equivalent problems", () => {
        ...
        equivalentPairs.forEach(([case1, case2]) => {
          const instruction1 = generateAbacusInstructions(
            case1.start,
            case1.target,
          );
          const instruction2 = generateAbacusInstructions(
            case2.start,
            case2.target,
          );
  ...
  describe("Expected States Calculation", () => {
    it("should correctly calculate expected states for each multi-step instruction", () => {
      ...
      if (instruction.stepBeadHighlights && instruction.multiStepInstructions) {
        const stepIndices = [
          ...new Set(
            instruction.stepBeadHighlights.map((bead) => bead.stepIndex),
          ),
        ].sort();
        let currentValue = 3;
      ...
      expect(expectedStates.length).toBe(
        instruction.multiStepInstructions?.length || 0,
      );
      ...
      if (instruction.stepBeadHighlights && instruction.multiStepInstructions) {
        ...
        stepIndices.forEach((stepIndex, _i) => {
          ...
          stepBeads.forEach((bead) => {
            ...
            const value =
              bead.beadType === "heaven" ? 5 * multiplier : multiplier;
