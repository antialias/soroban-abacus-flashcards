  ...
  if (hasPracticingSkills && daysSinceLastPractice > 14) {
    ...
    return {
      ...
      message:
        weeks >= 2
          ? `${weeks} weeks absent`
          : `${daysSinceLastPractice} days absent`,
    };
  }
 ...
 */
export function computeSkillCategory(
  practicingSkills: string[],
): SkillCategoryKey | null {
 ...
 */
export function getGroupCategoryName(
  category: SkillCategoryKey | null,
): string {
  if (category === null) {
    return "Not Started";
...
export function groupStudents(
  ...
  for (const bucket of bucketOrder) {
    ...
    const categoryOrder: (SkillCategoryKey | null)[] = [
      ...CATEGORY_PRIORITY,
      null,
    ];
