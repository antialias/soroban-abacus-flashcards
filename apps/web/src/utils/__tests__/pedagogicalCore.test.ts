...
function createStableSnapshot(
  result: ReturnType<typeof generateUnifiedInstructionSequence>,
) {
  return {
...
function assertArithmeticInvariant(
  seq: ReturnType<typeof generateUnifiedInstructionSequence>,
) {
  let currentValue = seq.startValue;
  seq.steps.forEach((step, i) => {
    const term = step.mathematicalTerm;
    const delta = term.startsWith("-")
      ? -parseInt(term.slice(1), 10)
      : parseInt(term, 10);
    currentValue += delta;
    expect(
      currentValue,
...
function assertComplementShape(
  seq: ReturnType<typeof generateUnifiedInstructionSequence>,
) {
  ...
  complementMatches.forEach((match) => {
    ...
    expect(
      num,
      `Complement positive part "${positive}" should be power of 10`,
    ).toSatisfy((n: number) => {
...
describe("Pedagogical Algorithm - Core Validation", () => {
  ...
  describe("Micro-Invariant Tests (Critical Behaviors)", () => {
    ...
    it("validates arithmetic step-by-step consistency", () => {
      ...
      result.steps.forEach((step, i) => {
        ...
        expect(
          runningValue,
          `Step ${i + 1}: After applying "${term}" to running total`,
        ).toBe(step.expectedValue);
      });
    ...
    it("validates complement shape contracts", () => {
      ...
      complementCases.forEach(([start, target]) => {
        ...
        complementMatches.forEach((match) => {
          ...
          const isValidStart =
            num === 5 || (num >= 10 && /^10+$/.test(num.toString()));
          expect(
            isValidStart,
          ...
          expect(
            match,
            "Complement should contain only subtraction after first term",
          ).toMatch(/\(\d+ - [\d\s-]+\)/);
        });
      });
    });
    ...
    stressCases.forEach(([start, target]) => {
      it(`validates invariants for ${start} → ${target}`, () => {
        ...
        result.steps.forEach((step) => {
          expect(
            step.isValid,
            `Step with term "${step.mathematicalTerm}" should be valid`,
          ).toBe(true);
        });
      });
    });
  ...
  describe("Pedagogical Segments - Basic Validation", () => {
    ...
    segmentTestCases.forEach(([start, target]) => {
      it(`generates valid segments for ${start} → ${target}`, () => {
        ...
        result.segments.forEach((segment, i) => {
          ...
          expect(
            segment.expression,
            `Segment ${i} should have expression`,
          ).toBeDefined();
          expect(
            segment.startValue,
            `Segment ${i} should have startValue`,
          ).toBeTypeOf("number");
          expect(
            segment.endValue,
            `Segment ${i} should have endValue`,
          ).toBeTypeOf("number");
          expect(
            segment.stepIndices,
            `Segment ${i} should have stepIndices`,
          ).toBeDefined();
        ...
        expect(
          allStepIndices.length,
          "Segments should cover all steps",
        ).toEqual(result.steps.length);
      });
    });
    ...
    it("validates segment term ranges map to decomposition string", () => {
      ...
      result.segments.forEach((segment, i) => {
        ...
        expect(
          termRange.endIndex,
          `Segment ${i} should have valid end index`,
        ).toBeGreaterThan(termRange.startIndex);
        expect(
          termRange.endIndex,
        ...
        const substring = result.fullDecomposition.slice(
          termRange.startIndex,
          termRange.endIndex,
        );
        expect(
          substring.length,
          `Segment ${i} should map to non-empty substring`,
        ).toBeGreaterThan(0);
      });
    });
  });
  ...
  describe("Pedagogical Segments - Production Pedagogy & Range Tests", () => {
    it("five-complement at ones (3 + 2 = 5)", () => {
      ...
      const txt = seq.fullDecomposition.slice(
    ...
    it("invariant: all steps with segmentId are included in their segments", () => {
      ...
      seq.steps.forEach((step, i) => {
        if (step.segmentId) {
          ...
          expect(
            segment,
            `Step ${i} references unknown segment ${step.segmentId}`,
          ).toBeDefined();
          expect(
    ...
    it("invariant: segment ranges are non-empty and well-formed", () => {
      ...
      seq.segments.forEach((seg) => {
        expect(
          seg.stepIndices.length,
          `Segment ${seg.id} should have steps`,
        ).toBeGreaterThan(0);
        expect(
          seg.termRange.endIndex,
