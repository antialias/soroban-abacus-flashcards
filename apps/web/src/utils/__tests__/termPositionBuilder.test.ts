...
describe("buildFullDecompositionWithPositions", () => {
  it("should correctly group cascading complement terms and map positions", () => {
    const seq = buildFullDecompositionWithPositions(3456, 3500, [
      "40",
      "100",
      "-90",
      "-6",
    ...
    expect(seq.fullDecomposition).toBe(
      "3456 + 44 = 3456 + 40 + (100 - 90 - 6) = 3500",
    );
    ...
    expect(seq.fullDecomposition.slice(p40.startIndex, p40.endIndex)).toBe(
      "40",
    );
    expect(seq.fullDecomposition.slice(p100.startIndex, p100.endIndex)).toBe(
      "100",
    );
    expect(seq.fullDecomposition.slice(p90.startIndex, p90.endIndex)).toBe(
      "90",
    ); // Maps to number only, not "-90"
  ...
  it("should handle ten-complement grouping", () => {
    ...
    expect(seq.fullDecomposition.slice(p10.startIndex, p10.endIndex)).toBe(
      "10",
    );
    expect(seq.fullDecomposition.slice(p2.startIndex, p2.endIndex)).toBe("2");
  });
  ...
  it("should handle mixed segments (single terms + complements)", () => {
    ...
    expect(seq.fullDecomposition.slice(p20.startIndex, p20.endIndex)).toBe(
      "20",
    );
    expect(seq.fullDecomposition.slice(p5.startIndex, p5.endIndex)).toBe("5");
  ...
  it("should handle complex cascading with multiple negative terms", () => {
    ...
    expect(seq.fullDecomposition).toBe(
      "9999 + 8 = 9999 + (10000 - 1000 - 100 - 10 - 1) = 10007",
    );

    const [p10000, p1000, p100, p10, p1] = seq.termPositions;
    expect(
      seq.fullDecomposition.slice(p10000.startIndex, p10000.endIndex),
    ).toBe("10000");
    expect(seq.fullDecomposition.slice(p1000.startIndex, p1000.endIndex)).toBe(
      "1000",
    );
    expect(seq.fullDecomposition.slice(p100.startIndex, p100.endIndex)).toBe(
      "100",
