...
export const guidedAdditionSteps: ExistingTutorialStep[] = [
  ...
  // Phase 4: More complex combinations
  {
    ...
    tooltip: {
      ...
      explanation:
        "When you have space in the earth section, just add directly",
    },
  },
  {
    id: "complex-2",
    ...
    problem: "7 + 4",
    description:
      "Add 4 to 7, but no room for 4 earth beads. Use complement again",
    startValue: 7,
    targetValue: 11,
    highlightBeads: [
    ...
    expectedAction: "multi-step",
    actionDescription:
      "Add tens heaven, then clear ones place: remove heaven + 2 earth",
    multiStepInstructions: [
...
export function convertGuidedAdditionTutorial(
  ...
  const convertedSteps = guidedAdditionSteps.map((step) => {
    ...
    const generatedInstruction = generateAbacusInstructions(
      step.startValue,
      step.targetValue,
    );
    ...
    return {
      ...
      tooltip: {
        ...
        explanation:
          stepTranslations.tooltip?.explanation || step.tooltip.explanation,
      },
      multiStepInstructions:
  ...
  try {
    ...
    tutorial.steps.forEach((step, index) => {
      ...
      if (
        ...
        errors.push(
          `Step ${index + 1}: Multi-step action missing instructions`,
        );
      }
    });
  } catch (error) {
...
export function getTutorialForEditor(
  tutorialMessages: Record<string, any> = {},
): Tutorial {
