...
function analyzeOperation(
  ...
  console.log(`\n=== ${operation} ===`);
  console.log(
    `Start: ${startValue} -> Target: ${targetValue} (difference: ${difference})`,
  );
  console.log(
    `Start state: heaven=${startState.heavenActive}, earth=${startState.earthActive}`,
  );
  console.log(
    `Target state: heaven=${targetState.heavenActive}, earth=${targetState.earthActive}`,
  );
...
export function auditTutorialSteps(): AuditIssue[] {
  ...
  guidedAdditionSteps.forEach((step, index) => {
    ...
    if (
      step.startValue + (step.targetValue - step.startValue) !==
      step.targetValue
    ) {
      issues.push({
        stepId: step.id,
    ...
    const _analysis = analyzeOperation(
      step.startValue,
      step.targetValue,
      step.problem,
    );
    ...
    switch (step.id) {
      ...
      case "heaven-intro": // 0 + 5
        if (
          !step.highlightBeads ||
          step.highlightBeads[0]?.beadType !== "heaven"
        ) {
          issues.push({
            stepId: step.id,
            stepTitle: step.title,
      ...
      case "heaven-plus-earth": // 5 + 1
        if (
          ...
          issues.push({
            ...
            severity: "major",
            description:
              "Should highlight first earth bead to add to existing heaven",
      ...
      case "complement-intro": // 3 + 4 = 7 (using 5 - 1)
        ...
        console.log(
          "Complement: 4 = 5 - 1, so add heaven (5) then remove 1 earth",
        );
        ...
        if (!step.highlightBeads || step.highlightBeads.length !== 2) {
          issues.push({
            ...
            severity: "major",
            description:
              "Should highlight heaven bead and first earth bead for 5-1 complement",
            currentState: `Highlights ${step.highlightBeads?.length || 0} beads`,
      ...
      case "complement-2": // 2 + 3 = 5 (using 5 - 2)
        ...
        console.log(
          "Complement: 3 = 5 - 2, so add heaven (5) then remove 2 earth",
        );
        ...
        if (!step.highlightBeads || step.highlightBeads.length !== 3) {
          issues.push({
            ...
            severity: "major",
            description:
              "Should highlight heaven bead and 2 earth beads for 5-2 complement",
            currentState: `Highlights ${step.highlightBeads?.length || 0} beads`,
      ...
      case "complex-2": {
        ...
        if (!step.highlightBeads || step.highlightBeads.length !== 4) {
          issues.push({
            ...
            severity: "major",
            description:
              "Should highlight tens heaven + all ones place beads to clear",
            currentState: `Highlights ${step.highlightBeads?.length || 0} beads`,
            expectedState:
              "Should highlight tens heaven + ones heaven + 2 ones earth",
          });
        }
        ...
        const onesEarthCount =
          step.highlightBeads?.filter(
            (h) => h.placeValue === 0 && h.beadType === "earth",
          ).length || 0;
      ...
      step.highlightBeads.forEach((bead) => {
        if (bead.placeValue !== 0 && bead.placeValue !== 1) {
          issues.push({
            ...
            expectedState:
              "Should use placeValue 0 (ones) or 1 (tens) for basic tutorial",
          });
        }
      });
    }
