...
export function useRoomData() {
  ...
  const [moderationEvent, setModerationEvent] =
    useState<ModerationEvent | null>(null);
  ...
  useEffect(() => {
    ...
    const handleRoomJoined = (data: {
      ...
      if (data.roomId === roomData?.id) {
        queryClient.setQueryData<RoomData | null>(
          roomKeys.current(),
          (prev) => {
            if (!prev) return null;
            return {
    ...
    const handleBannedFromRoom = (data: {
      roomId: string;
      bannedBy: string;
      reason: string;
    }) => {
 ...
 */
async function kickUserFromRoomApi(params: {
  roomId: string;
  userId: string;
}): Promise<void> {
