...
export function useUnifiedStudents(
  ...
  const isLoading =
    ...
    isLoadingChildren ||
    (isTeacher &&
      (isLoadingEnrolled || isLoadingPresence || isLoadingActiveSessions)) ||
...
> {
  ...
  > = {
    all: students.length,
    "needs-attention": students.filter(
      (s) => s.intervention != null && !s.isArchived,
    ).length,
  ...
  if (isTeacher) {
    ...
    counts["in-classroom"] = students.filter(
      (s) => s.relationship.isPresent,
    ).length;
