...
export function useBoundaryDetector(
  ...
  const [detectedCorners, setDetectedCorners] = useState<QuadCorners | null>(
    null,
  );
   ...
   */
  const getImageDataFromVideo = useCallback(
   ...
   */
  const getImageDataFromImage = useCallback(
  ...
  const updateStability = useCallback(
    (corners: QuadCorners | null, conf: number) => {
      ...
      if (
        ...
        cornersAreSimilar(corners, lastCornersRef.current)
      ) {
  ...
  const createCalibrationGrid = useCallback((): CalibrationGrid | null => {
    ...
    const left = Math.min(
      detectedCorners.topLeft.x,
      detectedCorners.bottomLeft.x,
    );
    const right = Math.max(
      detectedCorners.topRight.x,
      detectedCorners.bottomRight.x,
    );
    const top = Math.min(detectedCorners.topLeft.y, detectedCorners.topRight.y);
    const bottom = Math.max(
      detectedCorners.bottomLeft.y,
      detectedCorners.bottomRight.y,
    );
