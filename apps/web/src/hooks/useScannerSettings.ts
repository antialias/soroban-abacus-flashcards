...
export function useUpdateScannerSettings() {
  ...
  return useMutation({
    ...
    onMutate: async (updates) => {
      ...
      const previousSettings = queryClient.getQueryData<ScannerConfig>(
        scannerSettingsKeys.detail(),
      );

      // Optimistically update
      if (previousSettings) {
        const optimisticSettings = { ...previousSettings, ...updates };
        queryClient.setQueryData<ScannerConfig>(
          scannerSettingsKeys.detail(),
          optimisticSettings,
        );
      }
