...
export const recordingKeys = {
  ...
  player: (playerId: string) => [...recordingKeys.all, 'player', playerId] as const,
...
async function fetchSessionRecording(
  ...
  const response = await fetch(`/api/curriculum/${playerId}/sessions/${sessionId}/recording`)
...
async function fetchPlayerRecordings(
  ...
  const response = await fetch(`/api/curriculum/${playerId}/recordings?${params.toString()}`)
...
export function usePlayerRecordings(
  ...
  return useQuery({
    ...
    queryFn: () => fetchPlayerRecordings(playerId, options?.limit, options?.offset),
