...
export function useSessionObserver(
  ...
  const [dvrBufferInfo, setDvrBufferInfo] = useState<DvrBufferInfo | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (socket.connected) {
      console.log(
        "[SessionObserver] Socket already connected, triggering connect logic",
      );
      handleConnect();
    }
    ...
    socket.on(
      "vision-scrub-frame",
      (data: { imageData: string; timestamp: number }) => {
  ...
  const scrubTo = useCallback(
    (offsetMs: number) => {
      if (!socketRef.current || !isConnected || !sessionId) {
        console.warn(
          "[SessionObserver] Cannot scrub - not connected or no sessionId",
        );
        return;
      }
