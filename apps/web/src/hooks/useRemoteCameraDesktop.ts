...
export function useRemoteCameraDesktop(): UseRemoteCameraDesktopReturn {
  ...
  const [latestFrame, setLatestFrame] = useState<RemoteCameraFrame | null>(
    null,
  );
  ...
  const [phoneDetectedCorners, setPhoneDetectedCorners] =
  ...
  useEffect(() => {
    ...
    socketInstance.on("connect", () => {
      console.log(
        "[RemoteCameraDesktop] Socket connected! ID:",
        socketInstance.id,
      );
      setIsConnected(true);
      ...
      if (sessionId) {
        console.log(
          "[RemoteCameraDesktop] Re-subscribing to session after reconnect:",
          sessionId,
        );
  ...
  useEffect(() => {
    ...
    const handleConnected = ({
      ...
      console.log(
        "[RemoteCameraDesktop] Phone connected event:",
        phoneConnected,
      );
    ...
    const handleDisconnected = ({
      ...
      console.log(
        "[RemoteCameraDesktop] Phone disconnected event:",
        phoneConnected,
      );
      setIsPhoneConnected(phoneConnected);
    ...
    const handleError = ({ error: errorMsg }: { error: string }) => {
      ...
      if (errorMsg.includes("Invalid") || errorMsg.includes("expired")) {
        console.log(
          "[RemoteCameraDesktop] Session invalid, clearing persisted session",
        );
  ...
  const subscribe = useCallback(
    (sessionId: string) => {
      ...
      if (!socket || !isConnected) {
        console.log(
          "[RemoteCameraDesktop] Socket not connected yet, will subscribe on connect",
        );
        return;
      }
