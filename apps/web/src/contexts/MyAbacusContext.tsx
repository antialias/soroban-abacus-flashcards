...
export function MyAbacusProvider({ children }: { children: React.ReactNode }) {
  ...
  const [dockAnimationState, setDockAnimationState] =
    useState<DockAnimationState | null>(null);
  ...
  const isVisionSetupComplete =
    ...
    ((visionConfig.activeCameraSource === "local" &&
      visionConfig.cameraDeviceId !== null) ||
      (visionConfig.activeCameraSource === "phone" &&
  ...
  const setVisionCalibration = useCallback(
    (calibration: CalibrationGrid | null) => {
      setVisionConfig((prev) => {
  ...
  const setVisionCameraSource = useCallback(
  ...
  const setVisionFrameCallback = useCallback(
    (callback: VisionFrameCallback | null) => {
  ...
  const visionSourceRef: MutableRefObject<VisionSourceRef | null> =
    useRef<VisionSourceRef | null>(null);
  ...
  const captureTrainingColumns = useCallback(
    (columnCount: number): ColumnImageData[] | null => {
      ...
      try {
        ...
        console.error(
          "[MyAbacusContext] Failed to capture training columns:",
          error,
        );
        return null;
      }
