...
export function PageTransitionProvider({ children }: { children: ReactNode }) {
  ...
  const [phase, setPhase] = useState<
    "idle" | "expanding" | "navigating" | "revealing"
  >("idle");
    ...
    try {
      ...
      if (stored) {
        ...
        if (age < TRANSITION_MAX_AGE_MS) {
          ...
          console.log(
            "[PageTransitionContext] Found valid transition state, age:",
            age,
          );
  ...
  const signalReady = useCallback(() => {
    ...
    console.log(
      "[PageTransitionContext] signalReady called, setting phase to revealing",
    );
    setPhase("revealing");
    ...
    setTimeout(() => {
      console.log(
        "[PageTransitionContext] Cleanup: setting isTransitioning to false",
      );
