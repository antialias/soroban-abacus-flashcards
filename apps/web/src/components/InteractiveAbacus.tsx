...
export function InteractiveAbacus({
  ...
  const [_previousValue, setPreviousValue] = useState(initialValue);
  ...
  const _crossfadeSpring = useSpring({
  ...
  const handleBeadClick = useCallback(
    (event: Event) => {
      ...
      const beadColumn = parseInt(
        beadElement.getAttribute("data-bead-column") || "0",
        10,
      );
      ...
      if (beadType === "heaven") {
        ...
        const position = parseInt(beadPosition, 10); // 0-3 where 0 is top (closest to bar), 3 is bottom
        // Now using place-value based column numbering: 0=ones, 1=tens, 2=hundreds
        const placeValue = beadColumn;
        const columnPower = 10 ** placeValue;
        ...
        const columnContribution =
          (Math.floor(currentValue / columnPower) % 10) * columnPower;
        const newValue =
          currentValue - columnContribution + newDigit * columnPower;
    ...
    (event: React.KeyboardEvent) => {
      ...
      if (event.key >= "0" && event.key <= "9") {
        ...
        if (!isEditing) {
          ...
          const numValue = parseInt(newEditingValue, 10);
          const maxValue = 10 ** columns - 1;
        ...
        const liveValue = parseInt(newEditingValue, 10) || 0;
      ...
      >
                ...
                return (
                  ...
                  >
                    <div
                      ...
                      title={
                        showManualInput && colIndex === 0
                          ? "Click to edit, type numbers, Enter to confirm"
                          : undefined
                      }
                    >
                      <NumberFlow
        ...
        {showControls && (
          ...
          >
            ...
            >
              {(compact ? [1, 5, 10, 25] : [1, 5, 10, 25, 50, 99]).map(
                (preset) => (
                  <button
                    key={preset}
        ...
        {!compact && (
          ...
          >
            <strong>How to use:</strong> Click on the beads to activate or
            deactivate them! Heaven beads (top) are worth 5 each, earth beads
            (bottom) are worth 1 each.
            {showManualInput &&
              " You can also click the number to type directly."}
          </div>
        )}
      </div>
    </div>
