...
function useGridDimensions(gridConfig: any, totalCards: number) {
  ...
  useEffect(() => {
    const updateGrid = () => {
      ...
      const aspectRatio = window.innerWidth / window.innerHeight;
      setGridDimensions(
        calculateOptimalGrid(totalCards, aspectRatio, gridConfig),
      );
    };
...
export interface MemoryGridState<TCard = any> {
  ...
  playerMetadata?: Record<
    string,
    { emoji: string; name: string; color?: string; userId?: string }
  >;
...
>({
  ...
  const isMyTurn = useMemo(() => {
    ...
    const currentPlayerMetadata =
      state.playerMetadata?.[state.currentPlayer || ""];
    ...
    >
      ...
      >
        {state.gameCards.map((card) => {
          const isFlipped =
            state.flippedCards.some((c) => c.id === card.id) || card.matched;
          const isMatched = card.matched;
          const shouldShake =
            state.showMismatchFeedback &&
            state.flippedCards.some((c) => c.id === card.id);
          ...
          return (
            ...
            >
              {renderCard({
                ...
                isMatched,
                onClick: () =>
                  isValidForSelection ? handleCardClick(card.id) : undefined,
        ...
        Object.entries(state.playerHovers)
          ...
          .map(([playerId, cardId]) => {
            ...
            const cardElement = cardId
              ? (cardRefs.current.get(cardId) ?? null)
              : null;
            // Check if it's this player's turn
            const isPlayersTurn = state.currentPlayer === playerId;
            // Check if the card being hovered is flipped
            const hoveredCard = cardId
              ? state.gameCards.find((c) => c.id === cardId)
              : null;
            const isCardFlipped = hoveredCard
              ? state.flippedCards.some((c) => c.id === hoveredCard.id) ||
                hoveredCard.matched
              : false;
...
if (
  ...
  !document.getElementById("memory-grid-animations")
) {
