...
vi.mock("../AbacusDisplayDropdown", () => ({
  AbacusDisplayDropdown: () => (
    <div data-testid="abacus-dropdown">Dropdown</div>
  ),
}));
...
describe("AppNavBar Suspense Fix", () => {
  it("renders nav slot content with Suspense boundary (FIXED)", async () => {
    ...
    const LazyMatchingNav = React.lazy(() =>
      Promise.resolve({ default: MatchingNavContent }),
    );
  ...
  it("demonstrates the original issue was fixed", async () => {
    ...
    const LazyMemoryQuizNav = React.lazy(() =>
      Promise.resolve({ default: MemoryQuizContent }),
    );
