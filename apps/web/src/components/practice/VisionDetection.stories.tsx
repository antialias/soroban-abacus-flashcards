...
function VisionProgressDemo() {
  ...
  const [detectedIndex, setDetectedIndex] = useState<number | undefined>(
    undefined,
  );
      ...
      >
        {stages.map((stage, i) => (
          ...
          >
            ...
            {i < stages.length - 1 && (
              <div className={css({ fontSize: "1.5rem", color: "gray.400" })}>
                →
              </div>
            )}
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div className={css({ fontSize: "0.75rem", color: "gray.400" })}>
          DETECTED
        </div>
        <div
          className={css({
