...
import { GameBreakResultsScreen, RESULTS_COUNTDOWN_MS } from '../GameBreakResultsScreen'
...
describe('GameBreakResultsScreen', () => {
  ...
  const createMockResults = (overrides: Partial<GameResultsReport> = {}): GameResultsReport => ({
    gameName: 'matching',
  ...
  describe('result themes', () => {
    ...
    it('displays subheadline when provided', () => {
      ...
      expect(screen.getByText('You matched all pairs without any mistakes!')).toBeInTheDocument()
    })
  })
    ...
    it('displays stat icons', () => {
      render(
        <GameBreakResultsScreen
          isVisible={true}
          results={createMockResults({
            customStats: [{ label: 'Pairs', value: 8, icon: '🎯', highlight: true }],
          })}
          student={mockStudent}
          onComplete={mockOnComplete}
        />
  ...
  describe('data attributes', () => {
    it('includes data-component on root element', () => {
      ...
      const component = document.querySelector('[data-component="game-break-results-screen"]')
    ...
    it('includes data-action on continue button', () => {
      ...
      const button = document.querySelector('[data-action="continue-to-practice"]')
