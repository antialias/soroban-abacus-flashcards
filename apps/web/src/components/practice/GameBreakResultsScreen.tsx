  ...
  switch (theme) {
    ...
    default:
      return {
        ...
        border: isDark ? 'rgba(156, 163, 175, 0.3)' : 'rgba(156, 163, 175, 0.4)',
...
export function GameBreakResultsScreen({
  ...
  const highlightedStats = results.customStats?.filter((s) => s.highlight) ?? []
  ...
  return (
    ...
    >
      ...
      >
        ...
        {regularStats.length > 0 && (
          ...
          >
            {regularStats.map((stat, idx) => (
              ...
              >
                {stat.icon && (
                  <span className={css({ fontSize: '0.875rem', opacity: 0.8 })}>{stat.icon}</span>
                )}
                <span
                  className={css({
                    color: isDark ? 'gray.400' : 'gray.500',
        ...
        {/* Completion info */}
        {results.itemsCompleted !== undefined && results.itemsTotal !== undefined && (
          <div
