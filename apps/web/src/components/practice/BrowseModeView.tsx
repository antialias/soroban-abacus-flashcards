 ...
 */
export function buildLinearProblemList(
  parts: SessionPart[],
): LinearProblemItem[] {
...
function getResultForProblem(
  ...
  return results.find(
    (r) => r.partNumber === partNumber && r.slotIndex === slotIndex,
  );
}
...
export function BrowseModeView({
  ...
  const linearProblems = useMemo(
    () => buildLinearProblemList(plan.parts),
    [plan.parts],
  );
    ...
    >
      ...
      >
        <button
          type="button"
          data-action={
            isPracticing ? "close-practice" : "practice-this-problem"
          }
