...
export function PracticeHelpOverlay({
  ...
  const [beadHighlights, setBeadHighlights] = useState<
    StepBeadHighlight[] | undefined
  >();
  ...
  const isMeaningfulDecomposition =
    sequence?.isMeaningfulDecomposition ?? false;
  ...
  const renderHighlightedDecomposition = useCallback(() => {
    if (!sequence?.fullDecomposition || sequence.steps.length === 0)
      return null;
    ...
    if (currentStep.termPosition) {
      ...
      const highlighted = sequence.fullDecomposition.substring(
        startIndex,
        endIndex,
      );
    ...
    if (numberMatch) {
      ...
      if (numberIndex !== -1) {
        return {
          ...
          after: sequence.fullDecomposition.substring(
            numberIndex + number.length,
          ),
        };
      }
  ...
  useEffect(() => {
    ...
    return () => {
      if (beadTooltipTimerRef.current)
        clearTimeout(beadTooltipTimerRef.current);
  ...
  const handleBeadHighlightsChange = useCallback(
    (highlights: StepBeadHighlight[] | undefined) => {
