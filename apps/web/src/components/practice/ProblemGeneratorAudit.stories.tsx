...
function ProblemGeneratorAuditUI() {
  ...
  const generateProblem = useCallback(() => {
    ...
    const actualSkillsFromAnalyzer = analyzeRequiredSkills(
      result.terms,
      result.answer,
    );
  ...
  const formatStepByStepTrace = useCallback(
    (trace: GenerationTrace): string => {
            ...
            {debugTraces.map((trace, index) => (
              <div
                ...
                className={css({
                  ...
                  borderRadius: "8px",
                  border: "1px solid",
                  borderColor: trace.analysis.skillsMatch
                    ? "gray.200"
                    : "red.300",
                  overflow: "hidden",
                })}
              >
                ...
                >
                  <span
                    className={css({ fontSize: "0.75rem", color: "gray.500" })}
                  >
                    #{debugTraces.length - index} •{" "}
                  ...
                  >
                    ...
                    >
                      {trace.output.terms.map((term, i) => {
                        ...
                        const step = trace.stepByStepTrace.steps.find(
                          (s) => s.stepNumber === i + 1,
                        );
                        return (
                          <div
                    ...
                    <div className={css({ flex: 1 })}>
                      ...
                      <div className={css({ mb: "0.75rem" })}>
                        ...
                        >
                          {trace.analysis.actualSkillsFromAnalyzer.map(
                            (skill) => (
                              <span
                                key={skill}
                                className={css({
...
${JSON.stringify(
  Object.fromEntries(
    Object.entries(trace.input.skillSetSnapshot).map(([cat, skills]) => [
      cat,
      Object.fromEntries(
        Object.entries(skills as Record<string, boolean>).filter(
          ([, enabled]) => enabled,
        ),
      ),
    ]),
