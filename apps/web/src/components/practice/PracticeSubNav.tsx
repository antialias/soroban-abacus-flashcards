...
export function PracticeSubNav({
  ...
  const { actions, handlers, modals, classrooms } = useStudentActions(
    studentActionData,
    {
      onObserveSession,
    },
  ...
  useEffect(() => {
    ...
    sessionHud?.timing?.startTime,
    sessionHud?.timing?.accumulatedPauseMs,
    sessionHud?.isPaused,
  ]);
  ...
  const timingStats = sessionHud?.timing
    ? (() => {
        ...
        const threshold = hasEnoughData
          ? Math.max(
              30_000,
              Math.min(stats.mean + 2 * stats.stdDev, 5 * 60 * 1000),
            )
          : 60_000;
        return {
          ...stats,
  ...
  const gameBreakInfo = useMemo(() => {
    ...
    const gameBreakSettings = sessionHud.plan
      .gameBreakSettings as GameBreakSettings | null;
    const enabled = gameBreakSettings?.enabled ?? false;
    if (!enabled) return null;

    // Check if we're on the last part (no game break after last part)
    const isLastPart =
      sessionHud.currentPartIndex >= sessionHud.parts.length - 1;
                    ...
                    <RelationshipSummary
                      ...
                      otherStakeholders={
                        hasOtherStakeholders
                          ? {
                              parents:
                                stakeholders?.parents.filter((p) => !p.isMe)
                                  .length ?? 0,
                              teachers:
                ...
                {(classrooms.enrolled.length > 0 || classrooms.current) && (
                  <>
                    <DropdownMenu.Separator
                      className={separatorStyles(isDark)}
                    />
                    ...
                    {classrooms.current && (
                      ...
                      >
                        ...
                        />
                        <span>
                          In {classrooms.current.classroom.name} — Leave
                        </span>
                      </DropdownMenu.Item>
                    )}
      ...
      {sessionHud && !isOnGameBreak && (
        ...
        >
          ...
          <button
            ...
            aria-label={
              sessionHud.isBrowseMode ? "Exit browse mode" : "Enter browse mode"
            }
          >
            <span>🔍</span>
            ...
            <DropdownMenu.Portal>
              ...
              >
                ...
                >
                  ...
                  <span>
                    {sessionHud.isEndingSession ? "Ending..." : "End Session"}
                  </span>
                </DropdownMenu.Item>
          ...
          {timingStats && (
            <SessionMoodIndicator
              ...
              problemsRemaining={
                sessionHud.totalProblems - sessionHud.completedProblems
              }
