    ...
    if (classification === "strong") {
      ...
      if (!lastPracticed) {
        ...
        const daysSinceLastPractice =
          (referenceDate.getTime() - lastPracticed.getTime()) /
          (1000 * 60 * 60 * 24);
...
function computeSessionSnapshots(
  ...
  const recentSessions = sessionLimit
    ? sortedSessions.slice(-sessionLimit)
    : sortedSessions;
  ...
  const snapshots = recentSessions.map((session) => {
    ...
    const problemsAtPoint = problemHistory.filter(
      (p) => new Date(p.timestamp) <= sessionDate,
    );
...
function analyzeSessionTiming(
  ...
  for (let i = 1; i < snapshots.length; i++) {
    const gap =
      (snapshots[i].date.getTime() - snapshots[i - 1].date.getTime()) /
      (1000 * 60 * 60 * 24);
    gaps.push(gap);
  }
  ...
  if (gaps.length >= 3) {
    const variance =
      gaps.reduce((acc, g) => acc + (g - averageGapDays) ** 2, 0) / gaps.length;
...
function getMotivationalMessage(
  ...
  const achievedCounts = snapshots.map(
    (s) => s.distribution.strong + s.distribution.stale,
  );
...
export function SkillProgressChart({
  ...
  const presets = useMemo(
    () => calculateTimeWindowPresets(totalSessionCount),
    [totalSessionCount],
  );
  ...
  const windowContext: WindowContext = useMemo(
    () => ({
      ...
      isSubset:
        currentSessionLimit !== null && currentSessionLimit < totalSessionCount,
    }),
    ...
    const series = CLASSIFICATION_ORDER.map((classification) => {
      ...
      const seriesConfig: Record<string, unknown> = {
        ...
        data: snapshots.map((s) =>
          toPercent(s.distribution[classification], s.distribution.total),
        ),
      };
  ...
  const legendClassifications =
    activeFilters.size > 0
      ? CLASSIFICATION_ORDER.filter(
          (c) => currentDistribution[c] > 0 || activeFilters.has(c),
        )
            ...
            {presets.map((preset, index) => (
              <button
                ...
                className={css({
                  ...
                  borderRight:
                    index < presets.length - 1 ? "1px solid" : "none",
