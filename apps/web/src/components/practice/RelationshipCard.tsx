  ...
  if (isLoading) {
    return (
      ...
      >
        ...
        >
          <span
            className={css({ animation: "pulse 1.5s ease-in-out infinite" })}
          >
            Loading...
          </span>
        </div>
            ...
            >
              {stakeholders.enrolledClassrooms.map((classroom) => (
                <ClassroomRow
                  ...
                  classroom={classroom}
                  isPresent={
                    stakeholders.currentPresence?.classroomId === classroom.id
                  }
                  isDark={isDark}
                  compact={compact}
                />
    ...
    switch (relationship.type) {
      ...
      case "observer":
        return {
          ...
          subtitle: presence
            ? `Present in ${presence.classroomName}`
            : "In your classroom",
          color: {
            light: {
              bg: "emerald.50",
...
function StakeholderSection({
  ...
  return (
    ...
    >
      ...
      >
        <span className={css({ fontSize: compact ? "0.875rem" : "1rem" })}>
          {icon}
        </span>
        <span
          className={css({
...
function PendingRow({ pending, isDark, compact }: PendingRowProps) {
  return (
    ...
    >
      ...
      >
        {pending.pendingApproval === "teacher"
          ? "Needs Teacher"
          : "Needs Parent"}
      </div>
    </div>
