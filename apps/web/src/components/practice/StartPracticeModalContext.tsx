...
const StartPracticeModalContext =
  createContext<StartPracticeModalContextValue | null>(null);

export function useStartPracticeModal() {
  const context = useContext(StartPracticeModalContext);
  if (!context) {
    throw new Error(
      "useStartPracticeModal must be used within StartPracticeModalProvider",
    );
  }
  return context;
}
...
export function StartPracticeModalProvider({
  ...
  const [durationMinutes, setDurationMinutes] = useState(
    existingPlan?.targetDurationMinutes ?? 10,
  );
  ...
  const [gameBreakEnabled, setGameBreakEnabled] = useState(
    DEFAULT_GAME_BREAK_SETTINGS.enabled,
  );
  const [gameBreakMinutes, setGameBreakMinutes] = useState(
    DEFAULT_GAME_BREAK_SETTINGS.maxDurationMinutes,
  );
  const [gameBreakSelectionMode, setGameBreakSelectionMode] =
    useState<GameBreakSelectionMode>(DEFAULT_GAME_BREAK_SETTINGS.selectionMode);
  const [gameBreakSelectedGame, setGameBreakSelectedGameRaw] = useState<
    string | "random" | null
  ...
  const [gameBreakCustomConfig, setGameBreakCustomConfig] = useState<
  ...
  const setGameBreakSelectedGame = useCallback(
    (game: string | "random" | null) => {
  ...
  const estimatedProblems = useMemo(() => {
    return (
      problemsPerType.abacus +
      problemsPerType.visualization +
      problemsPerType.linear
    );
  }, [problemsPerType]);
  ...
  const tutorialConfig = useMemo(() => {
    if (sessionMode.type !== "progression" || !sessionMode.tutorialRequired)
      return null;
    return getSkillTutorialConfig(sessionMode.nextSkill.skillId);
  }, [sessionMode]);

  const nextSkill =
    sessionMode.type === "progression" ? sessionMode.nextSkill : null;
  ...
  const isStarting =
    generatePlan.isPending || approvePlan.isPending || startPlan.isPending;

  const displayError = useMemo(() => {
    if (
      generatePlan.error &&
      !(generatePlan.error instanceof ActiveSessionExistsClientError)
    ) {
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      ...
      if (
        existingPlan &&
        existingPlan.targetDurationMinutes === durationMinutes
      ) {
        plan = existingPlan;
      } else {
        try {
