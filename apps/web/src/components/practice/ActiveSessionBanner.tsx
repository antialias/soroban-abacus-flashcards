...
function DashboardBanner({
  ...
  const progressPercent = Math.round((session.completedCount / session.totalCount) * 100)
  const formattedChanges = session.skillChanges ? formatSkillChanges(session.skillChanges) : []
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          background: isLoading ? '#9ca3af' : 'linear-gradient(135deg, #3b82f6 0%, #7c3aed 100%)',
        }}
      >
        {isLoading ? '...' : 'Resume →'}
      </button>
