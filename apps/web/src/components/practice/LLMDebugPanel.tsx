...
export interface LLMDebugPanelProps {
  ...
  onViewContent: (title: string, content: string, contentType: 'text' | 'json') => void
}

export function LLMDebugPanel({ llm, onViewContent }: LLMDebugPanelProps): ReactNode {
  return (
    <div
    ...
    >
      ...
      >
        ...
        <span className={css({ color: 'gray.300' })}>{llm.imageSource ?? 'cropped'} ✓</span>
      ...
      >
        ...
        {llm.rawResponse && (
          <button
            type="button"
            onClick={() => onViewContent('Raw LLM Response', llm.rawResponse!, 'json')}
            className={css({
              fontSize: 'xs',
              color: 'gray.400',
              backgroundColor: 'gray.800',
        ...
        {llm.jsonSchema && (
          <button
            type="button"
            onClick={() =>
              onViewContent('JSON Schema (with field descriptions)', llm.jsonSchema!, 'json')
            }
            className={css({
              fontSize: 'xs',
              color: 'gray.400',
