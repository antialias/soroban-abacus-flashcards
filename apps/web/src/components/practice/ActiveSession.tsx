...
function ComplexitySection({
  ...
  const hasCost =
    ...
    !Number.isNaN(trace?.totalComplexityCost);
    ...
    case "challenge":
      return (
        <div className={tooltipStyles.container}>
          ...
          <p className={tooltipStyles.description}>
            Harder problems that require complement techniques for every term.
            These push your skills and build deeper fluency.
          </p>
          <div className={tooltipStyles.detail}>
...
export function ActiveSession({
  ...
  const collectVisionTrainingData = useCallback(
    async (correctAnswer: number, slotIndex: number) => {
      ...
      try {
        ...
        if (!response.ok) {
          console.warn(
            "[VisionTraining] Failed to save training data:",
            response.status,
          );
        }
      } catch (error) {
  ...
  const activeProblem = useMemo(() => {
    ...
    if (redoState?.isActive) {
      ...
      const redoProblem = (
        redoPart?.slots[redoState.originalSlotIndex] as ProblemSlot
      )?.problem;
      if (redoProblem) {
        return {
          problem: redoProblem,
  ...
  useEffect(() => {
    ...
    const extractComplexity = (
      slot: ProblemSlot | undefined,
    ): BroadcastComplexity | undefined => {
    ...
    if (phase.phase === "transitioning" && outgoingAttempt) {
      ...
      const prevSlotIndex =
        plan.currentSlotIndex > 0 ? plan.currentSlotIndex - 1 : 0;
      ...
      onBroadcastStateChange({
        ...
        currentPartIndex: redoState?.isActive
          ? redoState.originalPartIndex
          : plan.currentPartIndex,
        currentSlotIndex: redoState?.isActive
  ...
  useEffect(() => {
    ...
    switch (teacherControl.type) {
      case "set-value":
        ...
        console.log(
          "[ActiveSession] Teacher set abacus value to:",
          teacherControl.value,
        );
        break;
      case "show-abacus":
  ...
  useEffect(() => {
    ...
    console.log(
      "[ActiveSession] Teacher paused session:",
      teacherPauseRequest.message,
    );
  ...
  const browseIndex = isControlledBrowseIndex
    ? browseIndexProp
    : internalBrowseIndex;
  ...
  const showOnScreenKeypad =
    hasPhysicalKeyboard === false || keypadWasShownRef.current;
  ...
  const redoSlot = useMemo(() => {
    ...
    return redoPart.slots[redoState.originalSlotIndex] as
      | ProblemSlot
      | undefined;
  }, [redoState, parts]);
  ...
  useEffect(() => {
    ...
    console.log("[ActiveSession] Part transition effect:", {
      ...
      willTriggerTransition:
        currentPartIndex !== prevIndex && currentPartIndex < parts.length,
    });
  ...
  const handleTransitionComplete = useCallback(() => {
    console.log(
      "[ActiveSession] Part transition complete, calling onPartTransitionComplete",
    );
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (isDockedByUser && phase.phase !== "helpMode") {
      const prefixMatch = findMatchedPrefixIndex(
    ...
    const responseTimeMs =
      Date.now() - attemptData.startTime - attemptData.accumulatedPauseMs;
    ...
    if (isCorrect) {
      collectVisionTrainingData(
        attemptData.problem.answer,
        attemptData.slotIndex,
      );
    }
    ...
    setTimeout(
      () => {
        ...
        if (nextSlot && currentPart && isCorrect) {
          ...
          console.log(
            "[ActiveSession] Calling clearToLoading (end of part or incorrect)",
          );
          clearToLoading();
        }
      },
              ...
              >
                {currentPart.format === "vertical" ? (
                  <VerticalProblem
                    ...
                    complexityBudget={
                      currentSlot?.constraints?.maxComplexityBudgetPerTerm
                    }
                  />
                ) : (
                  <LinearProblem
            ...
            >
              <animated.button
                ...
                className={css({
                  ...
                  border: "none",
                  cursor:
                    !canSubmit || !showSubmitButton ? "not-allowed" : "pointer",
