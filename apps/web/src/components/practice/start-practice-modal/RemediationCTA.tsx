...
export function RemediationCTA() {
  ...
  const { sessionMode, showRemediationCta, showTutorialGate, isStarting, handleStart } =
    useStartPracticeModal()

  // Only show for remediation mode and when tutorial gate is not active
  if (!showRemediationCta || showTutorialGate || sessionMode.type !== 'remediation') {
    return null
  }
  ...
  return (
    ...
    >
        ...
        <div className={css({ flex: 1 })}>
          ...
          >
            {weakSkills.slice(0, 4).map((skill) => (
              ...
              >
                {skill.displayName}{' '}
                <span style={{ opacity: 0.7 }}>({Math.round(skill.pKnown * 100)}%)</span>
              </span>
            ))}
            {weakSkills.length > 4 && (
              <span
      ...
      <button
        ...
        style={{
          background: isStarting ? '#9ca3af' : 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)',
