...
export function OfflineWorkSection({
  ...
  useEffect(() => {
    ...
    if (parsedAttachments.length > 0) {
      const isCurrentValid = parsedAttachments.some(
        (a) => a.id === expandedResultId,
      );
      if (!isCurrentValid) {
    ...
    >
      ...
      >
        ...
        {attachments.map((att, index) => {
          ...
          const isStreaming =
            parsingContext.state.activeAttachmentId === att.id &&
            streamingState !== null;
        ...
        >
          ...
          {parsedAttachments.map((att) => {
            ...
            const worksheetIndex =
              parsedAttachments.findIndex((a) => a.id === att.id) + 1;
