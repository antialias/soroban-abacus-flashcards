...
export function PracticeTimingDisplay({
  ...
  const partTypeStats = (["abacus", "visualization", "linear"] as const).map(
    (type) => {
  ...
  const threshold = hasEnoughData
    ? Math.max(
        30_000,
        Math.min(overallStats.mean + 2 * overallStats.stdDev, 5 * 60 * 1000),
      )
      ...
      >
        ...
        >
          <span className={css({ fontSize: "1rem" })}>
            {getPartTypeEmoji(currentPartType)}
          </span>
          <span
            className={css({
              fontSize: "0.75rem",
