...
export function JoinRoomModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (room.accessMode === "retired") {
        setError(
          "This room has been retired and is no longer accepting members",
        );
        setIsLoading(false);
        return;
      }
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      const res = await fetch(
        `/api/arcade/rooms/${roomInfo.id}/join-requests`,
        {
          method: "POST",
  ...
  useEffect(() => {
    ...
    console.log(
      "[JoinRoomModal] Setting up approval listener for room:",
      roomInfo.id,
    );
    ...
    const setupSocket = async () => {
      try {
        ...
        socket.on(
          ...
          async (data: { roomId: string; requestId: string }) => {
          ...
          <div>
            {approvalRequested ? (
              ...
              <>
                ...
                >
                  You'll be able to join once the host approves your request.
