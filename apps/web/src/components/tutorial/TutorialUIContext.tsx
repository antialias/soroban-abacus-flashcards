...
export function TutorialUIProvider({
  ...
  const [activeSegment, setActiveSegment] = useState<PedagogicalSegment | null>(
    initialSegment,
  );
    ...
    () => ({
      ...
      requestFocus: (who: HintFocus) => {
        ...
        if (process.env.NODE_ENV !== "production") {
          console.debug(
            `[tutorial-ui] focus denied: ${who}, owned by ${hintFocus}`,
          );
        }
        return false;
      },
...
export function useTutorialUI(): TutorialUIState {
  ...
  if (!ctx) {
    throw new Error(
      "useTutorialUI must be used within <TutorialUIProvider> (tutorial routes)",
    );
  }
  return ctx;
}
