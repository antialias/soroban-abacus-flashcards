...
function findTopmostBeadWithArrows(
  ...
  if (beadsWithArrows.length === 0) {
    console.warn(
      "No beads with arrows found in step highlights:",
      stepBeadHighlights,
    );
    return null;
  }
...
function TutorialPlayerContent({
  ...
  const {
    ...
    try {
      ...
      const positions = unifiedSequence.steps
        .map((step) => step.termPosition)
        .filter(Boolean);
  ...
  const getCurrentStepBeads = useCallback(() => {
    ...
    try {
      ...
      const stepBeadHighlights: StepBeadHighlight[] = beadDiff.changes.map(
        (change, _index) => ({
    ...
    (beadInfo: any) => {
      ...
      if (
        ...
        const isCorrectBead = currentStep.highlightBeads.some((highlight) => {
          ...
          const highlightPlaceValue =
            highlight.placeValue ?? 4 - highlight.columnIndex;
  ...
  const dynamicColumnHighlights = useMemo(() => {
    ...
    activeTermIndices.forEach((termIndex) => {
      ...
      console.log(
        `  - Group term ${termIndex} maps to column ${columnIndex} (using rhsPlace)`,
      );
      if (columnIndex !== null) {
  ...
  const customStyles = useMemo(() => {
    ...
    if (
      ...
      currentStep.highlightBeads.forEach((highlight) => {
        ...
        const columnIndex =
          highlight.placeValue !== undefined
            ? 4 - highlight.placeValue
    ...
    Object.keys(dynamicColumnHighlights).forEach((columnIndexStr) => {
      ...
      Object.assign(
        mergedHighlights[columnIndex],
        dynamicColumnHighlights[columnIndex],
      );
    });

    return Object.keys(mergedHighlights).length > 0
      ? { columns: mergedHighlights }
      : undefined;
  ...
  return (
    ...
    >
      ...
      >
                    ...
                    <>
                      ...
                      <button
                        ...
                        className={css({
                          ...
                          border: "1px solid",
                          borderColor:
                            currentMultiStep === 0 ? "gray.200" : "orange.300",
                          borderRadius: "md",
                          bg: currentMultiStep === 0 ? "gray.100" : "white",
                          color:
                            currentMultiStep === 0 ? "gray.400" : "orange.700",
                          cursor:
                            currentMultiStep === 0 ? "not-allowed" : "pointer",
                          _hover:
        ...
        {/* Progress bar */}
        <div
          className={css({ mt: 2, bg: "gray.200", borderRadius: "full", h: 2 })}
        >
          <div
            className={css({
              bg: "blue.500",
          ...
          <div className={css({ flex: 1, p: 6 })}>
            <div className={vstack({ gap: 6, alignItems: "center" })}>
              ...
              <div className={css({ textAlign: "center", maxW: "600px" })}>
                ...
                <p
                  className={css({ fontSize: "lg", color: "gray.700", mb: 4 })}
                >
