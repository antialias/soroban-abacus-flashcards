  ...
  const getEnhancedTooltipContent = () => {
    ...
    if (rule === "Direct") {
      ...
      const enhancedChips = [
        ...
        ...(() => {
          const rodChip = readable?.chips.find((c) =>
            /^(this )?rod shows$/i.test(c.label),
          );
          ...
          <div className="reason-tooltip__content">
            <div className="reason-tooltip__header">
              ...
              <div className="reason-tooltip__title">
                ...
                <p
                  ...
                  className="reason-tooltip__description"
                >
            ...
            {((enhancedContent?.chips || readable?.chips)?.length ||
              ...
              <div className="reason-tooltip__details">
                ...
                {showDetails && (
                  <div
                    id={`${tooltipId}-details`}
                    className="reason-tooltip__details-content"
                  >
                    {/* Context chips */}
                    {(enhancedContent?.chips || readable?.chips)?.length ? (
                    ...
                    {segment?.plan?.some((p) => p.rule === "Cascade") &&
                      readable?.carryPath && (
                        ...
                        {showMath && (
                          <div className="reason-tooltip__math-content">
                            {readable.showMath.lines.map((line, index) => (
                              <p
                                key={index}
                                className="reason-tooltip__math-line"
                              >
                                {line}
                              </p>
                            ))}
                    ...
                    {readable && readable.stepsFriendly.length > 1 && (
                      <div className="reason-tooltip__steps">
                        ...
                        {showBeadDetails && (
                          <ol
                            id={`${tooltipId}-steps`}
                            className="reason-tooltip__step-list"
                          >
                            {readable.stepsFriendly.map(
                              (stepInstruction, idx) => (
            ...
            {process.env.NODE_ENV !== "production" &&
              ...
              !segment.readable.validation.ok && (
                <div className="reason-tooltip__dev-warn">
                  ⚠ Summary/guard mismatch:{" "}
                  {segment.readable.validation.issues.join("; ")}
                </div>
              )}
            ...
            {originalValue && segment?.expression && (
              <div className="reason-tooltip__formula">
                <div className="reason-tooltip__expansion">
                  <span className="reason-tooltip__original">
                    {originalValue}
                  </span>
                  <span className="reason-tooltip__arrow">→</span>
                  <code className="reason-tooltip__expanded">
                    {segment.expression}
                  </code>
                </div>
