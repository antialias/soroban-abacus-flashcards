    ...
    <div className={vstack({ gap: 6, p: 6, maxW: "800px", mx: "auto" })}>
      ...
      >
        <p className={css({ fontSize: "sm", color: "blue.800" })}>
          Enter any start and target values, and the system will automatically
          generate correct abacus instructions, including complement operations,
          multi-step procedures, and proper bead highlighting.
        </p>
      </div>
      ...
      <div className={hstack({ gap: 4, justifyContent: "center" })}>
        ...
        <div className={css({ alignSelf: "end", fontSize: "2xl", pb: 2 })}>
          →
        </div>
        ...
        >
          ...
          >
            ...
            <div
              className={css({
                ...
                py: 1,
                fontSize: "xs",
                bg: generatedInstruction.validation.isValid
                  ? "green.100"
                  : "red.100",
                color: generatedInstruction.validation.isValid
                  ? "green.800"
                  : "red.800",
                borderRadius: "md",
              })}
          ...
          {!generatedInstruction.validation.isValid && (
            ...
            >
              ...
              {generatedInstruction.validation.issues.join(", ")}
            </div>
          )}
          ...
          <div className={vstack({ gap: 3, alignItems: "start" })}>
            <div>
              ...
              {generatedInstruction.expectedAction}
            </div>
            ...
            <div>
              <strong>Highlighted Beads:</strong>{" "}
              {generatedInstruction.highlightBeads.length}
              <ul className={css({ ml: 4, mt: 1 })}>
                {generatedInstruction.highlightBeads.map(
                  (bead: any, index: number) => (
            ...
            <div className={vstack({ gap: 1, alignItems: "start" })}>
              ...
              <div className={css({ fontSize: "sm" })}>
                <strong>Wrong Bead:</strong>{" "}
                {generatedInstruction.errorMessages.wrongBead}
              </div>
              <div className={css({ fontSize: "sm" })}>
                <strong>Wrong Action:</strong>{" "}
                {generatedInstruction.errorMessages.wrongAction}
              </div>
              <div className={css({ fontSize: "sm" })}>
