...
function TermSpan({
  ...
  const { activeSegmentId, addActiveTerm, removeActiveTerm } =
    useContext(DecompositionContext);
...
export function DecompositionWithReasons({
  ...
  const _segmentRanges = useMemo(() => {
    ...
    return segments
      .map((seg) => ({
        ...
        startIndex: Math.min(
          ...seg.termIndices.map(
            (i) => termPositions[i]?.startIndex ?? Infinity,
          ),
        ),
        endIndex: Math.max(
          ...seg.termIndices.map((i) => termPositions[i]?.endIndex ?? 0),
        ),
      }))
  ...
  const renderElements = () => {
    ...
    for (let termIndex = 0; termIndex < termPositions.length; termIndex++) {
      ...
      if (segment && segment.termIndices[0] === termIndex) {
        ...
        for (const segTermIndex of segment.termIndices) {
          ...
          const segText = fullDecomposition.slice(
            segPos.startIndex,
            segPos.endIndex,
          );
          const segReason = termReasons?.find(
            (r) => r.termIndex === segTermIndex,
          );
    ...
    if (cursor < fullDecomposition.length) {
      elements.push(
        <span key="trailing">{fullDecomposition.slice(cursor)}</span>,
      );
    }
