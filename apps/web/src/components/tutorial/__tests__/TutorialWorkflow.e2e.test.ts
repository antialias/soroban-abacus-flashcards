...
const getAbacusValue = async (page: Page): Promise<number> => {
  ...
  const valueElement = await page
    .locator('[data-testid="current-value"]')
    .first();
...
test.describe("Tutorial Workflow E2E", () => {
  ...
  test("should initialize tutorial with correct start value", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator('[data-testid="step-description"]'),
    ).toBeVisible();
  });
  ...
  test("should navigate between tutorial steps and reset abacus value", async ({
    page,
  }) => {
  ...
  test("should show success feedback when target value is reached", async ({
    page,
  }) => {
    // This test assumes we can determine the target value from the UI
    const targetValueText = await page
      .locator('[data-testid="target-value"]')
      .textContent();
  ...
  test("should handle multi-step tutorial progression", async ({ page }) => {
    ...
    const multiStepIndicator = page.locator(
    ...
    if (await multiStepIndicator.isVisible()) {
      ...
      await expect(
        page.locator('[data-testid="multi-step-instruction"]'),
      ).toBeVisible();
  ...
  test("should automatically advance multi-steps when target is reached", async ({
    page,
  }) => {
  ...
  test("should show tooltip guidance when available", async ({ page }) => {
    ...
    if (await tooltip.isVisible()) {
      ...
      const abacusBox = await page
        .locator('[data-testid="abacus-container"]')
        .boundingBox();
  ...
  test("should work on different screen sizes", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid^="bead-place-"]').first(),
    ).toBeVisible();
