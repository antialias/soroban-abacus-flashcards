...
const getSeg = (seq: ReturnType<typeof generateUnifiedInstructionSequence>, place: number): PedagogicalSegment =>
  seq.segments.find(s => s.place === place)!
...
describe('semantic summaries', () => {
  ...
  it('shows provenance information when available', () => {
    ...
    if (firstStep?.provenance) {
      const provenanceChip = tensSegment.readable.chips.find(chip =>
        chip.label === 'From addend'
      )
  ...
  it('guard validation catches mismatches', () => {
    ...
    if (seg.plan[0].rule === 'Direct') {
      const hasDirectGuard = seg.plan.flatMap(p => p.conditions)
