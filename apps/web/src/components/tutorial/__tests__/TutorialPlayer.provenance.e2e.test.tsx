...
describe("TutorialPlayer Provenance E2E Test", () => {
  ...
  it('should show provenance information in tooltip for the "20" term', async () => {
    ...
    await waitFor(
      () => {
        ...
        const provenanceTitle = screen.getByText(
          "Add the tens digit — 2 tens (20)",
        );
    ...
    await waitFor(() => {
      const provenanceExplanation = screen.getByText(
        /We're adding the tens digit of 25 → 2 tens/,
      );
  ...
  it("should provide data for addend digit highlighting", async () => {
    ...
    const fullEquation = screen.getByText(
      /3475 \+ 25 = 3475 \+ 20 \+ \(100 - 90 - 5\) = 3500/,
    );
