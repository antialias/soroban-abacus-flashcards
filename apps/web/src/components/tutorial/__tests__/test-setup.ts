...
expect.extend({
  toHaveCorrectStartValue(received, expected) {
    ...
    if (pass) {
      return {
        message: () =>
          `Expected abacus not to have start value ${expected}, but it did`,
        pass: true,
      };
    } else {
      return {
        message: () =>
          `Expected abacus to have start value ${expected}, but got ${received}`,
        pass: false,
      };
    }
  },
  ...
  toAdvanceMultiStep(received) {
    ...
    if (pass) {
      return {
        message: () =>
          `Expected multi-step not to advance, but it advanced to step ${received}`,
        pass: true,
      };
    } else {
      return {
        message: () =>
          `Expected multi-step to advance, but it remained at step ${received}`,
        pass: false,
      };
    }
  },
...
vi.mock("../../../utils/unifiedStepGenerator", () => ({
  generateUnifiedInstructionSequence: vi.fn(
