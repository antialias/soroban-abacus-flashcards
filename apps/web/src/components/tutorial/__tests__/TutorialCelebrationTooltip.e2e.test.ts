...
test.describe("Tutorial Celebration Tooltip E2E", () => {
  ...
  test("celebration tooltip appears when reaching target value", async ({
    page,
  }) => {
  ...
  test("celebration tooltip disappears when moving away from target", async ({
    page,
  }) => {
    // Wait for tutorial to load
    await expect(page.locator("text=3 + 2")).toBeVisible({ timeout: 10000 });
    ...
    const heavenBead = page
      .locator('svg circle[data-bead-type="heaven"]')
      .first();
  ...
  test("celebration tooltip shows instruction mode when moved away", async ({
    page,
  }) => {
    // Wait for tutorial to load
    await expect(page.locator("text=3 + 2")).toBeVisible({ timeout: 10000 });
    ...
    const activeEarthBeads = page.locator(
      'svg circle[data-bead-type="earth"][data-active="true"]',
    );
  ...
  test("celebration tooltip positioned at correct bead", async ({ page }) => {
    ...
    const targetEarthBead = page
      .locator('svg circle[data-bead-type="earth"]')
      .first();
    ...
    if (await tooltip.isVisible()) {
      ...
      expect(tooltipBox?.x).toBeLessThan(
        (abacusBox?.x ?? 0) + (abacusBox?.width ?? 0) + 200,
      );
    }
  });
