...
const defaultPracticeStep: PracticeStep = {
  ...
  description:
    "Practice basic addition problems using direct addition and heaven bead",
  problemCount: 5,
  maxTerms: 3,
  allowedSkills: {
    ...createBasicSkillSet(),
...
function ProblemGeneratorDemo() {
  const [practiceStep, setPracticeStep] =
    useState<PracticeStep>(defaultPracticeStep);
  ...
  const handlePracticeComplete = (results: any) => {
    ...
    alert(
      `Practice completed! Score: ${results.correctAnswers}/${results.totalProblems}`,
    );
  };

  return (
    <div
      className={css({ h: "100vh", display: "flex", flexDirection: "column" })}
    >
      {/* Header */}
      <div
        className={css({
      ...
      <div className={css({ flex: 1, display: "flex" })}>
        {mode === "editor" ? (
          <div className={css({ w: "100%", display: "flex" })}>
            ...
            >
              <h3
                className={css({ fontSize: "lg", fontWeight: "bold", mb: 4 })}
              >
                ...
                {generatedProblems.map((problem, _index) => (
                  ...
                  >
                    ...
                    <div className={css({ fontSize: "sm", color: "gray.600" })}>
                      <div>
                        Sequential skills: {problem.skillsUsed.join(", ")}
                      </div>
