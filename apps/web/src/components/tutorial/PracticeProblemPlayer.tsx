...
export function PracticeProblemPlayer({
  ...
  const calculateExpectedValue = useCallback(
    (problem: GeneratedProblem, step: number): number => {
  ...
  useEffect(() => {
    if (currentProblem) {
      setExpectedValue(
        calculateExpectedValue(currentProblem, currentSequenceStep),
      );
    }
      ...
      >
        <div>
          ...
          <div className={css({ fontSize: "sm", color: "gray.600" })}>
            Creating {practiceStep.problemCount} problems based on your skill
            settings
          </div>
        </div>
      </div>
    ...
    return (
      ...
      >
        ...
        <p className={css({ color: "red.700", mb: 4 })}>
          Unable to generate problems with the current skill and constraint
          settings.
        </p>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Progress bar */}
        <div
          className={css({ mt: 3, bg: "gray.200", borderRadius: "full", h: 2 })}
        >
          <div
            className={css({
              bg: "green.500",
      ...
      <div className={css({ flex: 1, p: 6 })}>
        <div className={vstack({ gap: 6, alignItems: "center" })}>
                ...
                {currentProblem.terms.map((term, index) => (
                  <div
                    ...
                    className={css({
                      py: 1,
                      color:
                        index <= currentSequenceStep ? "blue.800" : "gray.400",
                    })}
                  >
                    {term}
                  </div>
          ...
          {isCorrect === false && (
            ...
            >
              Not quite right. Current value: {userAnswer}. Target:{" "}
              {expectedValue}. Keep trying!
            </div>
          )}
          ...
          {showExplanation && (
            ...
            >
              ...
              <div className={css({ mt: 2, fontSize: "sm" })}>
                ...
                {currentProblem.requiredSkills.join(", ")}
              </div>
            </div>
          )}
