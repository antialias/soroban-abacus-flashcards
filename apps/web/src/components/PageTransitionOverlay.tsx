...
export function PageTransitionOverlay() {
  ...
  const [phase, setPhase] = useState<
    "idle" | "expanding" | "covering" | "revealing"
  >("idle");
  ...
  useEffect(() => {
    if (isRevealing && phase === "covering") {
      console.log(
        "[PageTransitionOverlay] Context signaled revealing, switching to reveal phase",
      );
      setPhase("revealing");
    }
  }, [isRevealing, phase]);
...
function OverlayAnimation({ transitionState, phase }: OverlayAnimationProps) {
  const { originBounds, studentEmoji, studentName, studentColor } =
    transitionState;
