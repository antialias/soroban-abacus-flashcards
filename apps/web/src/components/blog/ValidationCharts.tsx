...
export function ExampleTrajectoryChart() {
  ...
  const option = {
    ...
    tooltip: {
      trigger: "axis",
      formatter: (
        params: Array<{ seriesName: string; value: number; axisValue: number }>,
      ) => {
  ...
  return (
    <div
      data-component="example-trajectory-chart"
    ...
    >
      ...
      >
        Fast learner deficient in <code>fiveComplements.3=5-2</code>. Adaptive
        reaches 80% mastery by session 6; classic takes until session 9.
      </p>
...
export function ConvergenceSpeedChart() {
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (
        ...
        for (const p of params) {
          const value =
            p.value === null ? "Never (>12 sessions)" : `${p.value} sessions`;
  ...
  return (
    <div
      data-component="convergence-speed-chart"
      className={chartContainerStyles}
    >
      <h4
        className={css({
      ...
      >
        Adaptive mode consistently reaches mastery faster. "Never" indicates the
        mode did not reach 80% within 12 sessions.
      </p>
      <ReactECharts option={option} style={{ height: "350px" }} />
    </div>
  );
...
export function AutomaticityMultiplierChart() {
  ...
  return (
    <div
      data-component="automaticity-multiplier-chart"
      className={chartContainerStyles}
    >
      <h4
        className={css({
      ...
      >
        The squared mapping provides better differentiation at high mastery
        levels. A skill at 50% P(known) costs 3.3× more than a fully automated
        skill.
      </p>
...
export function ValidationResultsCharts() {
  ...
  return (
    <div
      data-component="validation-results-charts"
      className={css({ my: "2rem" })}
    >
      {/* Summary Cards */}
...
function getSkillTier(
  skillId: string,
): "basic" | "fiveComp" | "tenComp" | "cascading" {
...
function getLineWidth(
  tier: "basic" | "fiveComp" | "tenComp" | "cascading",
): number {
  switch (tier) {
    case "basic":
      return 1;
...
function interpolateColor(
  ...
  const r = Math.round(
    colors[lower].r + (colors[upper].r - colors[lower].r) * blend,
  );
  const g = Math.round(
    colors[lower].g + (colors[upper].g - colors[lower].g) * blend,
  );
  const b = Math.round(
    colors[lower].b + (colors[upper].b - colors[lower].b) * blend,
  );
...
function MultiSkillTrajectoryChart({ data }: { data: TrajectoryData | null }) {
  ...
  const adaptiveAvg = sessions.map((_, sessionIdx) => {
    if (sessionIdx === 0) {
      ...
      const sum = data.skills.reduce(
        (acc, skill) => acc + skill.adaptive.data[0] * 0.3,
        0,
      );
      return Math.round(sum / numSkills);
    }
    const sum = data.skills.reduce(
      (acc, skill) => acc + skill.adaptive.data[sessionIdx - 1],
      0,
    );
    return Math.round(sum / numSkills);
  });
  ...
  const classicAvg = sessions.map((_, sessionIdx) => {
    if (sessionIdx === 0) {
      // Session 0: use initial estimate (30% of first session value)
      const sum = data.skills.reduce(
        (acc, skill) => acc + skill.classic.data[0] * 0.3,
        0,
      );
      return Math.round(sum / numSkills);
    }
    const sum = data.skills.reduce(
      (acc, skill) => acc + skill.classic.data[sessionIdx - 1],
      0,
    );
    return Math.round(sum / numSkills);
  });
  ...
  for (const skill of sortedSkills) {
    ...
    const adaptiveWithZero = [
      skill.adaptive.data[0] * 0.3,
      ...skill.adaptive.data,
    ];
    const classicWithZero = [
      skill.classic.data[0] * 0.3,
      ...skill.classic.data,
    ];
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (
        ...
        const session = (params[0] as unknown as { axisValue: number })
          ?.axisValue;
        // Only show averages in tooltip (not ghost lines)
        const adaptiveParam = params.find(
          (p) => p.seriesName === "Adaptive (avg)",
        );
        const classicParam = params.find(
          (p) => p.seriesName === "Classic (avg)",
        );
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Bold lines show average across {numSkills} skills. Ghost lines show
        individual skills (greens = adaptive, grays = classic). Thicker lines =
        harder skills (ten-complements). Darker shades = later in pedagogical
        sequence.
      </p>
...
const CATEGORY_COLORS: Record<
  SkillCategory,
  { adaptive: string; classic: string }
> = {
...
function InteractiveTrajectoryChart({ data }: { data: TrajectoryData | null }) {
  ...
  const [showCategoryAverages, setShowCategoryAverages] = useState<
    Set<SkillCategory>
  >(new Set());
  ...
  const adaptiveData = [
    selectedSkill.adaptive.data[0] * 0.3,
    ...selectedSkill.adaptive.data,
  ];
  const classicData = [
    selectedSkill.classic.data[0] * 0.3,
    ...selectedSkill.classic.data,
  ];

  // Calculate category averages
  const categoryAverages: Record<
    SkillCategory,
    { adaptive: number[]; classic: number[] }
  > = {
  ...
  for (const cat of [
    ...
    for (let i = 0; i < sessions.length; i++) {
      if (i === 0) {
        ...
        const adaptiveSum = skills.reduce(
          (acc, s) => acc + s.adaptive.data[0] * 0.3,
          0,
        );
        const classicSum = skills.reduce(
          (acc, s) => acc + s.classic.data[0] * 0.3,
          0,
        );
        categoryAverages[cat].adaptive.push(
          Math.round(adaptiveSum / skills.length),
        );
        categoryAverages[cat].classic.push(
          Math.round(classicSum / skills.length),
        );
      } else {
        const adaptiveSum = skills.reduce(
          (acc, s) => acc + s.adaptive.data[i - 1],
          0,
        );
        const classicSum = skills.reduce(
          (acc, s) => acc + s.classic.data[i - 1],
          0,
        );
        categoryAverages[cat].adaptive.push(
          Math.round(adaptiveSum / skills.length),
  ...
  if (adaptiveTo80 !== null && classicTo80 !== null) {
    ...
    advantageText =
      diff > 0 ? `Adaptive ${diff} sessions faster` : "Same speed";
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      {availableCategories.length > 0 && (
        ...
        >
          <span className={css({ fontSize: "0.75rem", color: "text.muted" })}>
            Show averages:
          </span>
      ...
      >
        ...
        {advantageText && (
          <span className={css({ color: "green.400", ml: "0.5rem" })}>
            ({advantageText})
          </span>
        )}
      </p>
...
function ConvergenceChart({ data }: { data: TrajectoryData | null }) {
  ...
  const adaptiveTo80 = data?.skills.map((s) => s.adaptive.sessionsTo80) ?? [
    6, 6, 5, 10, 7,
  ];

  const classicTo80 = data?.skills.map((s) => s.classic.sessionsTo80) ?? [
    9,
    8,
    6,
    null,
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Adaptive mode consistently reaches mastery faster across all tested
        skills. "—" or "Never" indicates the mode did not reach 80% within 12
        sessions.
      </p>
      <ReactECharts option={option} style={{ height: "350px" }} />
    </div>
...
function ValidationDataTable({ data }: { data: TrajectoryData | null }) {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Sessions to reach 80% mastery for each skill, comparing adaptive vs
        classic modes.
      </p>
          ...
          <tbody>
            {comparisonData.map((row) => (
              <tr key={row.skill}>
                ...
                <td className={css({ color: "text.muted" })}>
                  {row.classicTo80 ?? "never"}
                </td>
                <td
                  className={css({
                    color: row.advantage?.includes("Adaptive")
                      ? "green.400"
                      : "text.muted",
                    fontWeight: 500,
                  })}
...
export function ThreeWayComparisonCharts() {
  return (
    <div
      data-component="three-way-comparison-charts"
      className={css({ my: "2rem" })}
    >
      {/* Summary insight */}
      <div className={summaryCardStyles}>
...
function ThreeWayComparisonChart() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Both adaptive modes perform identically—the benefit comes from BKT{" "}
        <em>targeting</em>, not from BKT-based cost calculation.
      </p>
      <ReactECharts option={option} style={{ height: "350px" }} />
    </div>
  );
...
function FatigueComparisonChart() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        All modes have similar cognitive load. Adaptive modes are slightly
        higher because they include more challenging (weak skill) problems.
      </p>
      <ReactECharts option={option} style={{ height: "350px" }} />
    </div>
  );
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.skill}>
                ...
                <td className={css({ color: "green.400" })}>
                  {row.fluencyTo50}
                </td>
                <td className={css({ color: "green.400" })}>
                  {row.fluencyTo80}
...
export function EvidenceQualityCharts() {
  return (
    <div
      data-component="evidence-quality-charts"
      className={css({ my: "2rem" })}
    >
...
function HelpLevelChart() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Using hints or scaffolding reduces evidence strength. A correct answer
        with full solution shown provides only 50% of the evidence weight.
      </p>
...
function ResponseTimeChart() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Fast correct answers suggest automaticity (1.2×). Slow incorrect answers
        suggest genuine confusion (1.2×). Very fast incorrect answers are likely
        careless slips (0.5×).
      </p>
      <ReactECharts option={option} style={{ height: "300px" }} />
    </div>
...
export function AutomaticityMultiplierCharts() {
  return (
    <div
      data-component="automaticity-multiplier-charts"
      className={css({ my: "2rem" })}
    >
      ...
      <table className={tableStyles}>
        ...
        <tbody>
          {data.map((row) => (
            <tr key={row.pKnown}>
              <td>{row.pKnown}</td>
              <td className={css({ color: "purple.400", fontWeight: 500 })}>
                {row.multiplier}
              </td>
              <td>{row.meaning}</td>
            </tr>
...
export function StalenessIndicatorsCharts() {
  return (
    <div
      data-component="staleness-indicators-charts"
      className={css({ my: "2rem" })}
    >
...
function StalenessVisual() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Skills are flagged based on days since last practice. Staleness is shown
        as a separate indicator, not by decaying P(known).
      </p>
      <ReactECharts option={option} style={{ height: "300px" }} />
    </div>
  );
    ...
    <div className={chartContainerStyles}>
      ...
      <table className={tableStyles}>
        ...
        <tbody>
          ...
          <tr>
            <td>7-14</td>
            <td className={css({ color: "yellow.400" })}>
          ...
          <tr>
            <td>&gt; 30</td>
            <td className={css({ color: "red.400" })}>
              "Very stale — may need review"
            </td>
          </tr>
        </tbody>
      </table>
...
function ClassificationVisual() {
    ...
    tooltip: {
      ...
      formatter: (p: { name: string; value: number; seriesName: string }) => {
        const descriptions: Record<string, string> = {
          Struggling:
            "P(known) < 50%: Student has not yet internalized this pattern",
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Skills are classified into zones based on their P(known) value when
        confidence meets threshold. Low-confidence estimates default to
        "Learning" regardless of P(known).
      </p>
      ...
      >
        Note: Classification requires confidence ≥ threshold (default 50%).
        Skills with insufficient data are always classified as "Learning" until
    ...
    <div className={chartContainerStyles}>
      ...
      <table className={tableStyles}>
        ...
        <tbody>
          ...
          <tr>
            <td className={css({ color: "red.400", fontWeight: 600 })}>
              Struggling
            </td>
            <td>P(known) &lt; 50% AND confidence ≥ threshold</td>
          </tr>
          <tr>
            <td className={css({ color: "yellow.400", fontWeight: 600 })}>
              Learning
            </td>
...
export function BlameAttributionCharts() {
  return (
    <div
      data-component="blame-attribution-charts"
      className={css({ my: "2rem" })}
    >
      {/* Summary insight */}
      <div className={summaryCardStyles}>
...
function BlameComparisonChart() {
  ...
  const option = {
    ...
    tooltip: {
      ...
      formatter: (
        ...
        const heur =
          params.find((p) => p.seriesName === "Heuristic")?.value ?? 0;
        const baye =
          params.find((p) => p.seriesName === "Bayesian")?.value ?? 0;
        const winner =
          heur > baye ? "Heuristic" : baye > heur ? "Bayesian" : "Tie";
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Fast learner profiles across 5 random seeds. Higher correlation = BKT
        estimates track true mastery more accurately.
      </p>
...
function BlameDataTable() {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        t = -0.41, p &gt; 0.05 (df=4). The difference is not statistically
        significant.
      </p>
    </div>
  );
