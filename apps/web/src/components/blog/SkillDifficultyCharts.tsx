...
export function SkillDifficultyCharts() {
  ...
  if (!data) {
    return (
      ...
      >
        Failed to load data. Run:{" "}
        <code>npx tsx scripts/generateSkillDifficultyData.ts</code>
      </div>
    );
  }
  ...
  return (
    <div
      data-component="skill-difficulty-charts"
...
function MasteryCurvesChart({ data }: { data: ReportData }) {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Harder skills (higher difficulty multiplier) require more exposures to
        reach the same mastery level.
      </p>
...
function ExposuresToMasteryChart({ data }: { data: ReportData }) {
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        Ten-complements require roughly 2x the practice of basic skills to reach
        the same mastery level.
      </p>
...
function ThresholdsChart({ data }: { data: ReportData }) {
  const skills = Object.entries(
    data.fiftyPercentThresholds.exposuresFor50Percent,
  );
  ...
  return (
    <div className={chartContainerStyles}>
      ...
      >
        The K value in the Hill function determines where P(correct) = 50%.
        Higher K = harder skill.
      </p>
      <ReactECharts option={option} style={{ height: "300px" }} />
    </div>
  );
