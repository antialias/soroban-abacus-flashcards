...
export function FamilyCodeDisplay({
  ...
  const fetchFamilyCode = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to fetch family code",
      );
    } finally {
      setIsLoading(false);
    }
...
function FamilyCodeContent({
  ...
  return (
    <div data-section="family-code-content">
      ...
      >
        Share this code or QR with another parent to give them equal access to{" "}
        {playerName}&apos;s practice data.
      </p>
