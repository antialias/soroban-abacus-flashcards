...
export function SessionObserverView({
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/curriculum/${session.playerId}/sessions/${session.sessionId}/videos`,
    )
  ...
  const problemAttempts = useMemo(
    () =>
      selectedProblemNumber
        ? getVideoAttemptsForProblem(allVideos, selectedProblemNumber)
        : [],
      ...
      const currentProblemLinearIndex =
        ...
        state?.sessionParts
          ? state.sessionParts
              ...
              .reduce((acc, part) => acc + part.slots.length, 0) +
