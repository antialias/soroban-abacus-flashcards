  ...
  useEffect(() => {
    if (showRectifiedView && !opencvReady) {
      loadOpenCV()
        ...
        .catch((err) =>
          console.error("[VisionCameraFeed] Failed to load OpenCV:", err),
        );
    }
    ...
    const drawGrid = () => {
      ...
      for (const divider of calibration.columnDividers) {
        ...
        const bottomX =
          ...
          divider *
            (displayCorners.bottomRight.x - displayCorners.bottomLeft.x);
        const bottomY =
          displayCorners.bottomLeft.y +
          divider *
            (displayCorners.bottomRight.y - displayCorners.bottomLeft.y);
