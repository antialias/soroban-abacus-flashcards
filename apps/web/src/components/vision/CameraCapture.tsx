...
export function CameraCapture({
  ...
  const [phoneCalibration, setPhoneCalibration] =
  ...
  const markersFound =
    cameraSource === "local" ? localMarkersFound : phoneMarkersFound;
  const calibration =
    cameraSource === "local" ? localCalibration : phoneCalibration;
  const isCalibrated =
    cameraSource === "local" ? localIsCalibrated : phoneCalibration !== null;
  ...
  const [rectifiedCanvasElement, setRectifiedCanvasElement] =
          ...
          if (frameCorners) {
            ...
            const grid: CalibrationGrid = {
              roi: {
                ...
                width:
                  Math.max(
                    frameCorners.topRight.x,
                    frameCorners.bottomRight.x,
                  ) -
                  Math.min(frameCorners.topLeft.x, frameCorners.bottomLeft.x),
                height:
                  Math.max(
                    frameCorners.bottomLeft.y,
                    frameCorners.bottomRight.y,
            ...
            const changed =
              prev === null ||
              JSON.stringify(prev.corners) !== JSON.stringify(grid.corners);

            if (changed) {
              console.log(
                "[CAMERA-CAPTURE] Calibration changed, calling onCalibrationChange",
              );
            ...
            console.log(
              "[CAMERA-CAPTURE] No frameCorners, setting phoneMarkersFound=0",
            );
                ...
                <div className={css({ display: "flex", gap: 1 })}>
                  ...
                  {camera.isTorchAvailable && (
                    <button
                      ...
                      title={
                        camera.isTorchOn ? "Turn off flash" : "Turn on flash"
                      }
                    >
                      {camera.isTorchOn ? "🔦" : "💡"}
                    </button>
