 ...
 */
export interface SingleThermometerProps<T extends string>
  extends ThermometerBaseProps {
  options: ThermometerOption<T>[];
  value: T;
  onChange: (value: T) => void;
  /** Render custom content over an option (e.g., auto-resolve indicator) */
  renderOverlay?: (
    option: ThermometerOption<T>,
    index: number,
    isSelected: boolean,
  ) => ReactNode;
}
 ...
 */
export interface RangeThermometerProps<T extends string>
  extends ThermometerBaseProps {
  options: ThermometerOption<T>[];
  minValue: T;
  maxValue: T;
