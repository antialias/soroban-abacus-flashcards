...
import type { ParsedProblem, WorksheetParsingResult, ReviewProgress } from '@/lib/worksheet-parsing'
...
export interface ProblemReviewFlowProps {
  ...
  onCorrectProblem: (problemIndex: number, correction: ProblemCorrection) => Promise<void>
...
export function ProblemReviewFlow({
  ...
  const problemsNeedingReview = useMemo(() => getProblemsNeedingReview(problems), [problems])
  ...
  }, [displayedIndex, selectedMapIndex, queuePosition, problemsNeedingReview.length, onFlagProblem])
  ...
  }, [selectedMapIndex, queuePosition, problemsNeedingReview.length, problems.length])
  ...
  const isReviewComplete = problemsNeedingReview.length === 0 || stats.pending === 0
        ...
        <div className={css({ display: 'flex', alignItems: 'center', gap: 3 })}>
          ...
          >
            {selectedMapIndex !== null ? (
              ...
              <span>
                Reviewing {queuePosition + 1} of {problemsNeedingReview.length} flagged
              </span>
            )}
          </div>
        </div>
