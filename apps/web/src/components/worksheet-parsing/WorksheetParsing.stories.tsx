...
import { OfflineWorkSection, type OfflineAttachment } from '../practice/OfflineWorkSection'
import { PhotoViewerEditor, type PhotoViewerEditorPhoto } from '../practice/PhotoViewerEditor'
 ...
 */
type RawProblem = Omit<ParsedProblem, 'notes' | 'excluded' | 'reviewStatus' | 'reviewedAt'> & {
  notes?: string | null
  excluded?: boolean
...
const WORKSHEET_20_PROBLEMS_URL = '/storybook-assets/worksheets/worksheet-20-problems.jpg'

/** 10-problem worksheet image (low confidence scan) */
const WORKSHEET_10_PROBLEMS_URL = '/storybook-assets/worksheets/worksheet-10-problems.jpg'
...
const summaryMeta: Meta<typeof WorksheetReviewSummary> = {
  ...
  decorators: [
    (Story) => (
      <div style={{ width: '360px', height: '600px', backgroundColor: '#1a1a2e' }}>
        <Story />
      </div>
    ),
  ],
...
function FullFlowDemo() {
  ...
  const [initializingReviewId, setInitializingReviewId] = useState<string | null>(null)
  ...
  return (
    ...
    >
      ...
      <div className={css({ display: 'flex', gap: 4 })}>
        ...
        >
          <h3 className={css({ fontSize: 'sm', fontWeight: 'bold', mb: 2 })}>Action Log</h3>
      ...
      >
        <h3 className={css({ fontSize: 'sm', fontWeight: 'bold', mb: 2 })}>Test Instructions</h3>
        <ul className={css({ fontSize: 'sm', listStyleType: 'disc', pl: 4 })}>
          <li>
            <strong>Unparsed photo (att-3):</strong> Click the scan icon to parse
          </li>
          <li>
            <strong>Parsed worksheets (att-1, att-2):</strong> Click &quot;Check N problems&quot; to
            open review mode
          </li>
          <li>
            <strong>In review mode:</strong> Use the <strong>List/Focus toggle</strong> to switch
            between split-view (List) and one-at-a-time review (Focus)
          </li>
          <li>
            <strong>Approved worksheet (att-4):</strong> Has &quot;✓ Done&quot; badge
          </li>
        </ul>
      </div>
