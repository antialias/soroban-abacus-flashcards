  ...
  static async processJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `OpenSCAD reported errors, but checking if output was created:`,
          execError,
        );
        ...
        try {
          ...
          console.log(
            `Output file created despite OpenSCAD warnings - proceeding`,
          );
        } catch (readError) {
          // File doesn't exist, this is a real failure
          console.error(
            `OpenSCAD execution failed and no output file created:`,
            execError,
          );
