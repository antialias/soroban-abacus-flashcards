...
export async function getPlayerAccess(
  ...
  const parentLink = await db.query.parentChild.findFirst({
    where: and(
      eq(parentChild.parentUserId, viewerId),
  ...
  console.log(
    `[PERF] getPlayerAccess: ${total.toFixed(1)}ms | ` +
      ...
      (timings.enrollmentCheck
        ? `, enrollment=${timings.enrollmentCheck.toFixed(1)}ms`
        : "") +
      (timings.presenceCheck
        ? `, presence=${timings.presenceCheck.toFixed(1)}ms`
        : ""),
  );
 ...
 */
export async function getAccessiblePlayers(
  viewerId: string,
  ...
  if (classroom) {
    ...
    presentStudents = [...ownChildren, ...enrolledStudents].filter((s) =>
      presentIds.has(s.id),
    );
  }
...
export function generateAuthorizationError(
  ...
  return {
    ...
    remediation: {
      type: "no-access",
      description:
        "Contact the student's parent or your administrator for access.",
      playerId: access.playerId,
    },
  };
}
