...
export function calculateSessionTrends(
  ...
  const accuracyDelta =
    previousAccuracy !== null ? currentAccuracy - previousAccuracy : null;

  // This week stats (last 7 days)
  const weekSessions = allSessions.filter((s) => isSessionWithinDays(s, 7));
  const weekProblems = weekSessions.reduce(
    (sum, s) => sum + getSessionProblemCount(s),
    0,
  );
  ...
  // All-time stats
  const totalProblems = allSessions.reduce(
    (sum, s) => sum + getSessionProblemCount(s),
    0,
  );
