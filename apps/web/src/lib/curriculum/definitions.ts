 ...
 */
const FIVE_COMPLEMENT_SUB: Record<
  number,
  keyof SkillSet["fiveComplementsSub"]
> = {
  4: "-4=-5+1",
  3: "-3=-5+2",
  2: "-2=-5+3",
...
function createLevel1AdditionPhases(): CurriculumPhase[] {
  ...
  for (let num = 1; num <= 9; num++) {
    ...
    phases.push({
      ...
      primarySkillId:
        num <= 4 ? "basic.directAddition" : "basic.simpleCombinations",
      order: order++,
    });
...
function createLevel1SubtractionPhases(): CurriculumPhase[] {
  ...
  for (let num = 9; num >= 1; num--) {
    ...
    phases.push({
      ...
      primarySkillId:
        num <= 4 ? "basic.directSubtraction" : "basic.simpleCombinationsSub",
      order: order++,
    });
...
export const CURRICULUM_LEVELS: CurriculumLevel[] = [
  {
    ...
    phases: [
      ...createLevel1AdditionPhases(),
      ...createLevel1SubtractionPhases(),
    ],
  },
  {
    id: 2,
...
export const ALL_PHASES: CurriculumPhase[] = CURRICULUM_LEVELS.flatMap(
  (level) => level.phases,
);
 ...
 */
export function getPhaseSkillConstraints(
  phaseId: string,
): PhaseSkillConstraints {
...
function subtractSkillSets(
  all: SkillSet,
  unlocked: Partial<SkillSet>,
): Partial<SkillSet> {
 ...
 */
export function getPhasesForLevel(
  levelId: CurriculumLevelId,
): CurriculumPhase[] {
...
export function parsePhaseId(phaseId: string): {
  ...
  const match = phaseId.match(
    /^L(\d)\.(add|sub)\.([-+]?\d+)\.(direct|five|ten|tenFive)$/,
  );
