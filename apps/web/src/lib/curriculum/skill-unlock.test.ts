...
describe("SKILL_TUTORIAL_CONFIGS", () => {
  ...
  it("all configs have valid example problems", () => {
    for (const [skillId, config] of Object.entries(SKILL_TUTORIAL_CONFIGS)) {
      ...
      for (const problem of config.exampleProblems) {
        expect(typeof problem.start, `${skillId} start should be number`).toBe(
          "number",
        );
        expect(
          typeof problem.target,
          `${skillId} target should be number`,
        ).toBe("number");
        expect(
          problem.start,
          `${skillId} start should be non-negative`,
        ).toBeGreaterThanOrEqual(0);
        expect(
          problem.target,
          `${skillId} target should be non-negative`,
        ).toBeGreaterThanOrEqual(0);
      }
    }
  });
  ...
  it("all configs have non-empty title and description", () => {
    for (const [skillId, config] of Object.entries(SKILL_TUTORIAL_CONFIGS)) {
      expect(
        config.title.length,
        `${skillId} should have title`,
      ).toBeGreaterThan(0);
      expect(
        config.description.length,
        `${skillId} should have description`,
      ).toBeGreaterThan(0);
    }
  });
