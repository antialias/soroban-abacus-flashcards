...
export function calculateCategoryMastery(
  ...
  for (const [category, skills] of Object.entries(categorySkills) as [
    ...
    categories[category] = {
      ...
      masteredCount: skills.filter((s) => s.pKnown >= BKT_THRESHOLDS.strong)
        .length,
 ...
 */
export function calculateNormalizedResponseTime(
  ...
  const sptValues = validResults.map(
    (r) => r.responseTimeMs / 1000 / r.problem.terms.length,
  );

  // Overall average
  const avgSecondsPerTerm =
    sptValues.reduce((sum, v) => sum + v, 0) / sptValues.length;
  ...
  const recentSptAvg =
    sptValues.slice(0, midpoint).reduce((sum, v) => sum + v, 0) / midpoint;
  const olderSptAvg =
    sptValues.slice(midpoint).reduce((sum, v) => sum + v, 0) /
    (sptValues.length - midpoint);
...
export function calculateAccuracy(results: ProblemResultWithContext[]): {
  ...
  const newerCorrect = recentResults
    .slice(0, midpoint)
    .filter((r) => r.isCorrect).length;
  const olderCorrect = recentResults
    .slice(midpoint)
    .filter((r) => r.isCorrect).length;
...
export function calculateImprovementRate(
  ...
  const olderResults = allResults.filter(
...
function getDaysDifference(day1: string, day2: string): number {
  ...
  return Math.abs(
    Math.round((date2.getTime() - date1.getTime()) / (24 * 60 * 60 * 1000)),
  );
}
 ...
 */
export function calculateWeeklyProblems(
  results: ProblemResultWithContext[],
): number {
  const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  return results.filter((r) => new Date(r.timestamp).getTime() > weekAgo)
    .length;
}
...
export function calculateCategorySpeed(
  ...
  const validResults = results.filter((r) => {
    if (r.hadHelp || r.responseTimeMs <= 0 || !r.problem?.terms?.length)
      return false;
...
export function computeClassroomLeaderboard(
  ...
  const byWeeklyProblems = createRanking(
    playersData,
    (p) => p.metrics.progress.weeklyProblems,
  );
  const byTotalProblems = createRanking(
    playersData,
    (p) => p.metrics.progress.totalProblems,
  );
  const byPracticeStreak = createRanking(
    playersData,
    (p) => p.metrics.progress.practiceStreak,
  );
