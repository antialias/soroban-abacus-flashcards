    ...
    try {
      ...
      try {
        ...
        if ((model as LayersModel).outputs.length !== 2) {
          console.error(
            "[ColumnClassifier] Model has wrong architecture, expected 2 outputs",
          );
          modelCheckFailed = true;
          return null;
        }
...
function extractOutputs(
  ...
  if (
    ...
    const namedOutputs = output as Record<
      string,
      import("@tensorflow/tfjs").Tensor
    >;
    return {
...
export async function classifyColumns(
  ...
  const preprocessed = await Promise.all(
    columnImages.map((img) => preprocessImage(img)),
  );
  ...
  for (let i = 0; i < columnImages.length; i++) {
    ...
    const earthProbs = Array.from(
      allEarthProbs.slice(earthStart, earthStart + NUM_EARTH_CLASSES),
    );
