...
const PREPROCESSING_PATH = '/models/abacus-boundary-detector/preprocessing.json'
...
async function loadPreprocessingConfig(): Promise<PreprocessingConfig | null> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('[BoundaryDetector] No preprocessing.json found, using defaults')
      return null
    }
    preprocessingConfig = await response.json()
    console.log('[BoundaryDetector] Loaded preprocessing config:', preprocessingConfig)
    return preprocessingConfig
  } catch {
    console.warn('[BoundaryDetector] Failed to load preprocessing.json, using defaults')
    return null
  }
}
...
async function applySobelEdges(
  ...
  const grayBatched = gray.expandDims(0).expandDims(-1) as import('@tensorflow/tfjs').Tensor4D
  ...
  const squeezed = normalized.squeeze([0, 3]) as import('@tensorflow/tfjs').Tensor2D
...
async function dsntDecode(
  ...
  const heatmapsFlat = heatmapsReshaped.reshape([batchSize * numKeypoints, height * width])
  ...
  const coordinates = tf.stack([xExpected, yExpected], 2) as import('@tensorflow/tfjs').Tensor3D
...
async function loadModel(): Promise<SupportedModel | null> {
  ...
  modelLoadPromise = (async () => {
    ...
    console.log('[BoundaryDetector] Model file exists, loading preprocessing config...')
...
export async function detectBoundary(
  ...
  const isHeatmapModel = modelType === 'heatmap_dsnt' || output.shape.length === 4
  ...
  if (isHeatmapModel) {
    ...
    // Diagnostic summary
    const avgCornerMax = cornerMaxes.reduce((a, b) => a + b, 0) / cornerMaxes.length
    if (avgCornerMax < 0.1) {
      console.warn(
    ...
    console.log('[BoundaryDetector] DSNT decoded coordinates:', Array.from(coordsData))
