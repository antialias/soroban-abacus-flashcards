...
export type PreprocessingStrategy = 'standard' | 'enhanced' | 'adaptive' | 'multi'
...
export function extractBestCorners(
  ...
  const cornerCandidates = vertexScores.filter((v) => v.angle < 180 - minCornerAngle)
 ...
 */
function lineIntersection(line1: LineSegment, line2: LineSegment): Point | null {
  const x1 = line1.x1,
    y1 = line1.y1,
    x2 = line1.x2,
    y2 = line1.y2
 ...
 */
function groupLinesByAngle(lines: LineSegment[], toleranceRadians: number): LineSegment[][] {
 ...
 */
function findDominantLines(lines: LineSegment[], toleranceRadians: number): LineSegment[] | null {
  // Group by angle
  ...
  let direction2 = groupsWithScore.find(
    (g) => Math.abs(Math.abs(g.avgAngle - direction1.avgAngle) - Math.PI / 2) < toleranceRadians * 2
  )
  ...
  const splitGroup = (group: LineSegment[]): [LineSegment, LineSegment] | null => {
    if (group.length < 2) {
    ...
    return [getBestLineFromGroup([sorted[0]]), getBestLineFromGroup([sorted[sorted.length - 1]])]
  }
...
function detectQuadFromHoughLines(
  ...
  try {
    ...
    const isRoughlyPerpendicular = (l1: LineSegment, l2: LineSegment): boolean => {
    ...
    if (!isDocumentAspectRatio(aspectRatio, cfg.expectedAspectRatios, cfg.aspectRatioTolerance)) {
      return null
    }
 ...
 */
function preprocessStandard(cv: CV, gray: CVMat, edges: CVMat, cfg: QuadDetectorConfig): void {
  const blurred = new cv.Mat()

  try {
    // Blur to reduce noise
    cv.GaussianBlur(gray, blurred, new cv.Size(cfg.blurSize, cfg.blurSize), 0, 0, cv.BORDER_DEFAULT)
 ...
 */
function preprocessEnhanced(cv: CV, gray: CVMat, edges: CVMat, cfg: QuadDetectorConfig): void {
  ...
  try {
    ...
    cv.Canny(blurred, edges2, cfg.cannyThresholds[0] / 2, cfg.cannyThresholds[1] / 2)
 ...
 */
function preprocessAdaptive(cv: CV, gray: CVMat, edges: CVMat, cfg: QuadDetectorConfig): void {
  const equalized = new cv.Mat()
  const blurred = new cv.Mat()
  ...
  try {
    ...
    if (cfg.enableAdaptiveThreshold) {
      ...
      cv.threshold(adaptiveEdges, adaptiveEdges, 127, 255, cv.THRESH_BINARY_INV)
    }
 ...
 */
function preprocessMulti(cv: CV, gray: CVMat, edges: CVMat, cfg: QuadDetectorConfig): void {
  const equalized = new cv.Mat()
  const blurred = new cv.Mat()
  ...
  try {
    ...
    cv.Canny(blurred, cannyEdges, cfg.cannyThresholds[0], cfg.cannyThresholds[1])

    // Strategy B: Low-threshold Canny (catches faint edges)
    cv.Canny(blurred, cannyLowEdges, cfg.cannyThresholds[0] / 3, cfg.cannyThresholds[1] / 3)
 ...
 */
function preprocessImage(cv: CV, gray: CVMat, edges: CVMat, cfg: QuadDetectorConfig): void {
  try {
  ...
  function detect(source: HTMLCanvasElement): DetectedQuad[] {
    ...
    try {
      ...
      for (let i = 0; i < contours.size(); i++) {
        ...
        if (approx.rows === 4) {
        ...
        } else if (approx.rows > 4 && approx.rows <= cfg.maxVerticesForCornerFit) {
          ...
          if (hullPoints.length >= 4) {
            ...
            const extracted = extractBestCorners(hullPoints, cfg.minCornerAngle)
            if (extracted) {
      ...
      if (quads.length === 0 && cfg.enableHoughLines && edges) {
        const houghQuad = detectQuadFromHoughLines(cv, edges, cfg, source.width, source.height)
        if (houghQuad) {
