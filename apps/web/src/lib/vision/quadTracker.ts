 ...
 */
function calculateCornerStability(history: Array<[Point, Point, Point, Point]>): number {
...
export function createQuadTracker(config: Partial<QuadTrackerConfig> = {}) {
  ...
  function update(
    ...
    const frameDiagonal = Math.sqrt(frameSize.width ** 2 + frameSize.height ** 2)
    ...
    for (const detected of detectedQuads) {
      ...
      for (const [id, tracked] of trackedQuads) {
        if (
          !seenIds.has(id) &&
          quadsMatch(detected.corners, tracked.corners, frameDiagonal, cfg.matchThreshold)
        ) {
