...
import { loadAruco, initArucoDetector, MARKER_IDS, type MarkerCorners } from './arucoDetection'
...
function getMarkerBounds(
  ...
  const xs = [marker.topLeft.x, marker.topRight.x, marker.bottomRight.x, marker.bottomLeft.x]
  const ys = [marker.topLeft.y, marker.topRight.y, marker.bottomRight.y, marker.bottomLeft.y]
...
function fillPolygonWithNoise(
  ...
  expandedPath.moveTo(cx + (marker.topLeft.x - cx) * scale, cy + (marker.topLeft.y - cy) * scale)
  expandedPath.lineTo(cx + (marker.topRight.x - cx) * scale, cy + (marker.topRight.y - cy) * scale)
  expandedPath.lineTo(
...
export async function maskMarkersInImage(
  ...
  // Detect markers
  const AR = (window as { AR?: { Detector: new (options?: object) => ArucoDetector } }).AR
  if (!AR) {
