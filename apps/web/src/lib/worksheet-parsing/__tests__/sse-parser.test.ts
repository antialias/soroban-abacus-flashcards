...
describe('parseSSEStream', () => {
  describe('connection events', () => {
    it("should call onStarted for 'started' event", async () => {
      ...
      const response = createSSEResponse([sseEvent('started', { responseId: 'resp-123' })])
  ...
  describe('reasoning events', () => {
    it('should call onReasoning with text and isDelta', async () => {
      ...
      expect(onReasoning).toHaveBeenCalledWith('Looking at the image...', true, undefined)
    })
  ...
  describe('output_delta events', () => {
    it('should call onOutputDelta with text', async () => {
      ...
      const response = createSSEResponse([sseEvent('output_delta', { text: '{"problems": [' })])
    ...
    it('should handle delta property as fallback', async () => {
      const onOutputDelta = vi.fn()
      const response = createSSEResponse([sseEvent('output_delta', { delta: '{"key": "value"}' })])
