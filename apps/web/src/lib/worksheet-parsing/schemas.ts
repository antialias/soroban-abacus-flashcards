...
export const BoundingBoxSchema = z
  ...
  .describe(
    "Rectangular region on the worksheet image, in normalized 0-1 coordinates",
  );
...
export const ParsedProblemSchema = z
  .object({
    ...
    problemNumber: z
      ...
      .min(1)
      .describe(
        "The problem number as printed on the worksheet (1, 2, 3, etc.)",
      ),
    row: z
      .number()
      .int()
      .min(1)
      .describe(
        "Which row of problems this belongs to (1 = top row, 2 = second row, etc.)",
      ),
    column: z
      .number()
      .int()
      .min(1)
      .describe(
        "Which column position in the row (1 = leftmost, counting right)",
      ),
    ...
    // Optional notes explaining low confidence or issues
    notes: z
      .string()
      .nullable()
      .describe(
    ...
    reviewedAt: z
      ...
      .default(null)
      .describe(
        "ISO timestamp when this problem was reviewed. Null if not yet reviewed",
      ),
  })
...
export type WorksheetParsingResult = z.infer<
  typeof WorksheetParsingResultSchema
>;
...
export const ProblemCorrectionSchema = z
  .object({
    ...
    correctedTerms: z
      ...
      .optional()
      .describe(
        "Corrected terms if the LLM got them wrong. Omit or null to keep original",
      ),
    correctedStudentAnswer: z
      ...
      .optional()
      .describe(
        "Corrected student answer. Null means empty/not answered. Omit to keep original",
      ),
    shouldExclude: z
      .boolean()
      .optional()
      .describe(
        "True to exclude this problem from the session (e.g., illegible)",
      ),
    shouldRestore: z
      .boolean()
      .optional()
      .describe("True to restore a previously excluded problem"),
    note: z
      .string()
      .nullable()
      .optional()
