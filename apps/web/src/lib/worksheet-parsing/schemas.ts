...
export const ProblemCorrectionSchema = z
  .object({
    ...
    correctedTerms: z
      ...
      .nullable()
      .optional()
      .describe('Corrected terms if the LLM got them wrong. Omit or null to keep original'),
    correctedStudentAnswer: z
      .number()
      .int()
      .nullable()
      .optional()
      .describe('Corrected student answer. Null means empty/not answered. Omit to keep original'),
    shouldExclude: z
      .boolean()
      .optional()
      .describe('True to exclude this problem from the session (e.g., illegible)'),
    shouldRestore: z.boolean().optional().describe('True to restore a previously excluded problem'),
    note: z.string().nullable().optional().describe('Optional note explaining the correction'),
  })
