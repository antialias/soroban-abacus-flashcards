...
describe("player-ownership utilities", () => {
  ...
  const mockRoomData: RoomData = {
    ...
    memberPlayers: {
      ...
      "user-b": [
        { id: "player-3", name: "Player 3", emoji: "🐭", color: "#0000ff" },
      ],
    },
  };
  ...
  describe("isPlayerOwnedByUser", () => {
    it("should return true when player belongs to user", () => {
      expect(isPlayerOwnedByUser("player-1", "user-a", mockOwnershipMap)).toBe(
        true,
      );
      expect(isPlayerOwnedByUser("player-2", "user-a", mockOwnershipMap)).toBe(
        true,
      );
    });

    it("should return false when player does not belong to user", () => {
      expect(isPlayerOwnedByUser("player-1", "user-b", mockOwnershipMap)).toBe(
        false,
      );
      expect(isPlayerOwnedByUser("player-3", "user-a", mockOwnershipMap)).toBe(
        false,
      );
    });
    ...
    it("should return false for non-existent player", () => {
      expect(
        isPlayerOwnedByUser("player-999", "user-a", mockOwnershipMap),
      ).toBe(false);
    });
  });
  ...
  describe("filterPlayersByOwner", () => {
    it("should return only players owned by specified user", () => {
      ...
      const userAPlayers = filterPlayersByOwner(
        playerIds,
        "user-a",
        mockOwnershipMap,
      );
      ...
      const userBPlayers = filterPlayersByOwner(
        playerIds,
        "user-b",
        mockOwnershipMap,
      );
      ...
      const userCPlayers = filterPlayersByOwner(
        playerIds,
        "user-c",
        mockOwnershipMap,
      );
    ...
    it("should return empty array when user owns no players", () => {
      ...
      const result = filterPlayersByOwner(
        playerIds,
        "user-nonexistent",
        mockOwnershipMap,
      );
