...
export async function listActiveRooms(
  ...
  whereConditions.push(
    or(
      eq(schema.arcadeRooms.accessMode, "open"),
      eq(schema.arcadeRooms.accessMode, "password"),
    ),
    or(
...
export async function cleanupExpiredRooms(): Promise<number> {
  ...
  const toDelete = expiredRooms.filter((room) => {
    const expiresAt = new Date(
      room.lastActivity.getTime() + room.ttlMinutes * 60 * 1000,
    );
  ...
  if (toDelete.length > 0) {
    ...
    await db
      ...
      .where(or(...ids.map((id) => eq(schema.arcadeRooms.id, id))));
