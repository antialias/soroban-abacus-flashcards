...
const ComplementRaceContext = createContext<ComplementRaceContextValue | null>(
  null,
);
...
export function useComplementRace() {
  ...
  if (!context) {
    throw new Error(
      "useComplementRace must be used within ComplementRaceProvider",
    );
  }
  return context;
}
  ...
  switch (typedMove.type) {
    case "CLAIM_PASSENGER": {
      ...
      const updatedPassengers = state.passengers.map((p) =>
        p.id === passengerId
          ? { ...p, claimedBy: typedMove.playerId, carIndex }
          : p,
      );
...
export function ComplementRaceProvider({ children }: { children: ReactNode }) {
  ...
  const initialState = useMemo((): ComplementRaceState => {
    const gameConfig = roomData?.gameConfig as
      ...
      | null
      | undefined;
    const savedConfig = gameConfig?.["complement-race"] as
      | Partial<ComplementRaceConfig>
      | undefined;
    ...
    const config: ComplementRaceConfig = {
      ...
      mode:
        (savedConfig?.mode as ComplementRaceConfig["mode"]) ||
        DEFAULT_COMPLEMENT_RACE_CONFIG.mode,
      complementDisplay:
      ...
      enableAI:
        savedConfig?.enableAI ?? DEFAULT_COMPLEMENT_RACE_CONFIG.enableAI,
      aiOpponentCount:
        savedConfig?.aiOpponentCount ??
        DEFAULT_COMPLEMENT_RACE_CONFIG.aiOpponentCount,
      maxPlayers:
        savedConfig?.maxPlayers ?? DEFAULT_COMPLEMENT_RACE_CONFIG.maxPlayers,
      routeDuration:
        savedConfig?.routeDuration ??
        DEFAULT_COMPLEMENT_RACE_CONFIG.routeDuration,
      enablePassengers:
        savedConfig?.enablePassengers ??
        DEFAULT_COMPLEMENT_RACE_CONFIG.enablePassengers,
      passengerCount:
        savedConfig?.passengerCount ??
        DEFAULT_COMPLEMENT_RACE_CONFIG.passengerCount,
      maxConcurrentPassengers:
        savedConfig?.maxConcurrentPassengers ??
        DEFAULT_COMPLEMENT_RACE_CONFIG.maxConcurrentPassengers,
      raceGoal:
        savedConfig?.raceGoal ?? DEFAULT_COMPLEMENT_RACE_CONFIG.raceGoal,
      winCondition:
        (savedConfig?.winCondition as ComplementRaceConfig["winCondition"]) ||
        DEFAULT_COMPLEMENT_RACE_CONFIG.winCondition,
      targetScore:
        savedConfig?.targetScore ?? DEFAULT_COMPLEMENT_RACE_CONFIG.targetScore,
      timeLimit:
        savedConfig?.timeLimit ?? DEFAULT_COMPLEMENT_RACE_CONFIG.timeLimit,
      routeCount:
        savedConfig?.routeCount ?? DEFAULT_COMPLEMENT_RACE_CONFIG.routeCount,
    };
  ...
  const compatibleState = useMemo((): CompatibleGameState => {
    ...
    if (
      multiplayerState.gamePhase === "setup" ||
    ...
    return {
      ...
      elapsedTime: multiplayerState.gameStartTime
        ? Date.now() - multiplayerState.gameStartTime
        : 0,
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (serverPosition !== undefined && serverPosition > 0) {
        console.log(
          `[POSITION_SYNC] Restoring position from server: ${serverPosition.toFixed(1)}%`,
        );
  ...
  useEffect(() => {
    if (compatibleState.isGameActive && multiplayerState.config.enableAI) {
      ...
      if (count > 0 && clientAIRacers.length === 0) {
        ...
        const personalities: Array<"competitive" | "analytical"> = [
          "competitive",
          "analytical",
        ];
        ...
        for (let i = 0; i < Math.min(count, aiNames.length); i++) {
          ...
          newAI.push({
            id: `ai-${i}`,
            name: aiNames[i],
            personality: personalities[i % personalities.length] as
              | "competitive"
              | "analytical",
            position: 0,
            speed: baseSpeed, // Balanced speed from original single-player version
            icon:
              personalities[i % personalities.length] === "competitive"
                ? "🏃‍♂️"
                : "🏃",
            lastComment: 0,
            commentCooldown: 0,
    ...
    const interval = setInterval(() => {
      ...
      const decayRate =
        MOMENTUM_DECAY_RATES[
          compatibleState.timeoutSetting as keyof typeof MOMENTUM_DECAY_RATES
  ...
  useEffect(() => {
    ...
    multiplayerState.currentRoute,
    compatibleState.style,
    multiplayerState.passengers.length,
  ]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const posDiff = Math.abs(
        currentPos - lastBroadcastLogRef.current.position,
      );
    ...
    return () => {
      console.log(
        `[POS_BROADCAST] Stopping interval (sent ${broadcastCountRef.current} updates)`,
      );
      ...
      switch (action.type) {
        ...
        case "SUBMIT_ANSWER":
          if (action.answer !== undefined) {
            const responseTime =
              Date.now() - (multiplayerState.questionStartTime || Date.now());
        ...
        case "CLAIM_PASSENGER":
          if (
            action.passengerId !== undefined &&
            action.carIndex !== undefined
          ) {
        ...
        default:
          console.warn(
            `[ComplementRaceProvider] Unknown action type: ${action.type}`,
          );
      }
    },
    [
