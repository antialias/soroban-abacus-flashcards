...
function findMiddlePiece(pieces: Piece[]): Piece | null {
  ...
  const positions = [
    parseSquare(p1.square),
    parseSquare(p2.square),
    parseSquare(p3.square),
  ];
 ...
 */
function isArithmeticProportion(
  a: number,
  m: number,
  b: number,
 ...
 */
function isGeometricProportion(
  a: number,
  m: number,
  b: number,
): HarmonyValidationResult {
...
export function validateHarmony(
  ...
  const getVal = (p: Piece) => {
    ...
    if (val === null) {
      throw new Error(
        `Piece ${p.id} has no effective value (Pyramid face not set?)`,
      );
    }
    return val;
  };
...
export function isHarmonyStillValid(
  ...
  const relevantPieces = harmony.pieceIds
    .map((id) => pieces[id])
