 ...
 */
export class RithmomachiaValidator
  implements GameValidator<RithmomachiaState, RithmomachiaMove>
{
  /**
  ...
  private validateCapture(
    ...
    target: Piece,
    capture: NonNullable<
      Extract<RithmomachiaMove, { type: "MOVE" }>["data"]["capture"]
    >,
  ...
  private applyMove(
    ...
    if (ambush) {
      const ambushValidation = this.validateAmbush(
        newState,
        piece.color,
        ambush,
      );
    ...
    // Compute new hash
    const newHash = computeZobristHash(
      newState.pieces,
    ...
    if (
      newState.pendingHarmony &&
      newState.pendingHarmony.by === newState.turn
    ) {
      ...
      if (config.allowAnySetOnRecheck) {
        ...
        if (hasAnyValidHarmony(newState.pieces, newState.pendingHarmony.by)) {
          ...
          moveRecord.resultAfter =
            newState.winner === "W" ? "WINS_W" : "WINS_B";
        } else {
          // Harmony broken
