...
export function BoardDisplay() {
  ...
  const [selectedRelation, setSelectedRelation] = useState<RelationKind | null>(
    null,
  );
  ...
  const { selectedSquare, handleSquareClick, clearSelection } =
    usePieceSelection(state.pieces, playerColor, isMyTurn, {
  ...
  const findAvailableRelations = (
    ...
    if (checkMultiple(moverValue, targetValue).valid)
      available.push("MULTIPLE");
    ...
    if (findValidHelpers(moverValue, targetValue, "SUM").length > 0)
      available.push("SUM");
    if (findValidHelpers(moverValue, targetValue, "DIFF").length > 0)
      available.push("DIFF");
    if (findValidHelpers(moverValue, targetValue, "PRODUCT").length > 0)
      available.push("PRODUCT");
    if (findValidHelpers(moverValue, targetValue, "RATIO").length > 0)
      available.push("RATIO");
  ...
  const handleCaptureWithRelation = (relation: RelationKind) => {
    ...
    if (needsHelper) {
      ...
      if (targetValue === undefined || targetValue === null) {
        console.log(
          "[handleCaptureWithRelation] Undefined/null target value, returning",
        );
        return;
      }
      ...
      if (
        ...
        console.log(
          "[handleCaptureWithRelation] Total unique helpers: " +
            validHelpers.length,
        );

        if (validHelpers.length === 0) {
          console.log(
            "[handleCaptureWithRelation] No valid helpers found for any pyramid face",
          );
          return;
        }

        console.log(
          "[handleCaptureWithRelation] Auto-selecting first helper:",
          validHelpers[0],
        );
        ...
        if (moverValue === undefined || moverValue === null) {
          console.log(
            "[handleCaptureWithRelation] Undefined/null mover value, returning",
          );
          return;
        }
      ...
      console.log(
        "[handleCaptureWithRelation] No helper needed, executing capture immediately",
      );
  ...
  const handleSvgClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const x =
      ((e.clientX - rect.left) / rect.width) * boardWidth -
      labelMargin -
      padding;
  ...
  const handleSvgMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    console.log(
      "[HOVER_ERROR] Mouse move - isMyTurn: " +
        isMyTurn +
        ", selectedSquare: " +
        selectedSquare,
    );
  ...
  const helpersWithPositions = (() => {
    ...
    if (
      ...
      for (const faceValue of moverPiece.pyramidFaces) {
        const helpers = findValidHelpers(
          faceValue,
          targetValue,
          selectedRelation,
        );
  ...
  const { availableRelations, pyramidFaceValues } = (() => {
    ...
    if (
      ...
      console.log(
        "[availableRelations] Checking pyramid with faces: " +
          moverPiece.pyramidFaces.join(", "),
      );
      ...
      console.log(
        "[availableRelations] All pyramid relations: " + result.join(", "),
      );
      console.log(
  ...
  const showHoverError = (() => {
    ...
    console.log(
      "[HOVER_ERROR] Found pieces - mover: " +
        !!moverPiece +
        ", target: " +
        !!targetPiece,
    );
    ...
    if (
      ...
      console.log(
        "[HOVER_ERROR] Checking pyramid with faces: " +
          moverPiece.pyramidFaces.join(", "),
      );
      ...
      for (const faceValue of moverPiece.pyramidFaces) {
        const relations = findAvailableRelations(faceValue, targetValue);
        console.log(
          "[HOVER_ERROR] Face " + faceValue + " relations: " + relations.length,
        );
        if (relations.length > 0) {
          console.log(
            "[HOVER_ERROR] Found valid relations with face " +
              faceValue +
              " - no error",
          );
      ...
      console.log(
        "[HOVER_ERROR] No pyramid face has valid relations - show error",
      );
    ...
    >
      ...
      >
        ...
        {Array.from({ length: cols }, (_, col) => {
          ...
          const x =
            labelMargin + padding + col * (cellSize + gap) + cellSize / 2;
        ...
        {activePieces.map((piece) => {
          ...
          const isBorrowedHelper = helpersWithPositions.some(
            (h) => h.piece.id === piece.id,
          );
          const isBorrowedMover =
            selectedHelper && selectedHelper.moverPiece.id === piece.id;
        ...
        {captureDialogOpen &&
          ...
          (() => {
            ...
            console.log(
              "[Render] helpersWithPositions.length:",
              helpersWithPositions.length,
            );
              ...
              <CaptureProvider value={captureContextValue}>
                ...
                {selectedRelation && selectedHelper && (
                  <NumberBondVisualization
                    ...
                    moverStartPos={(() => {
                      const moverFile =
                        selectedHelper.moverPiece.square.charCodeAt(0) - 65;
                      ...
                      return {
                        x:
                          padding + moverFile * (cellSize + gap) + cellSize / 2,
                    ...
                    helperStartPos={(() => {
                      ...
                      const angleStep =
                        helpersWithPositions.length > 1
                          ? 360 / helpersWithPositions.length
                          : 0;
        ...
        {showHoverError && hoverErrorPosition && (
          <g
            transform={`translate(${hoverErrorPosition.x}, ${hoverErrorPosition.y})`}
          >
            <foreignObject
              x={-cellSize * 1.8}
              y={-cellSize * 0.5}
            ...
            >
              ...
              >
                <span style={{ fontSize: `${cellSize * 0.2}px`, opacity: 0.7 }}>
                  ⚠
                </span>
