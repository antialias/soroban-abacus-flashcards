...
export function RithmomachiaGame() {
  ...
  const rosterWarning = useRosterWarning(
    state.gamePhase === "setup" ? "setup" : "playing",
  );
  ...
  const [dockPreviewSide, setDockPreviewSide] = useState<
    "left" | "right" | null
  >(null);
    ...
    if (typeof window !== "undefined") {
      ...
      if (savedDocked !== null) {
        ...
        if (isDocked && savedSide) {
          setGuideDockSide(
            savedSide === "left" || savedSide === "right" ? savedSide : "right",
          );
        }
        ...
        console.log(
          "[RithmomachiaGame] Guide opened in default docked right position",
        );
      }
    }
  };
