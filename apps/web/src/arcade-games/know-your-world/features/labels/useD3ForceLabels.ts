  ...
  useEffect(() => {
    ...
    const updateLabelPositions = () => {
      ...
      [...mapData.regions, ...excludedRegions].forEach((region) => {
        ...
        const centroidPixelX =
          (region.center[0] - viewBoxX) * scaleX + svgOffsetX;
        const centroidPixelY =
          (region.center[1] - viewBoxY) * scaleY + svgOffsetY;
        ...
        const regionPath = svgRef.current?.querySelector(
        ...
        if (shouldShowLabel && !isExcluded) {
          const players = regionsFound.includes(region.id)
            ? guessHistory
                .filter(
                  (guess) => guess.regionId === region.id && guess.correct,
                )
                .map((guess) => guess.playerId)
                .filter(
                  (playerId, index, self) => self.indexOf(playerId) === index,
                )
            : undefined;
      ...
      if (useObstacles) {
        mapData.regions.forEach((region) => {
          ...
          const pixelX =
            ghostRect.left - containerRect.left + ghostRect.width / 2;
          const pixelY =
            ghostRect.top - containerRect.top + ghostRect.height / 2;
