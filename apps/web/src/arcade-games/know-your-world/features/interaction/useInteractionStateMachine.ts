    ...
    case "TOUCH_MOVE":
      if (state.phase === "touched" || state.phase === "mapPanning") {
        ...
        return {
          ...
          touchedRegion:
            event.regionId !== undefined ? event.regionId : state.touchedRegion,
...
export function useInteractionStateMachine(
  ...
  const isPointerLocked =
    state.mode === "desktop" && state.phase === "pointerLocked";
  const isReleasingPointerLock =
    state.mode === "desktop" && state.phase === "releasing";
  ...
  const isMobilePanning =
    state.mode === "mobile" &&
    (state.phase === "mapPanning" || state.phase === "magnifierPanning");
  ...
  const isMagnifierDragging =
    state.mode === "mobile" && state.phase === "magnifierPanning";
  const isPinching =
    state.mode === "mobile" && state.phase === "magnifierPinching";
  /** True if magnifier was activated by map drag (shows Select button) */
  const magnifierTriggeredByDrag =
    state.mode === "mobile" && state.magnifierTriggeredByDrag;

  // Shared derived state
  const isDragging = isDesktopDragging || isMobilePanning;
  const cursorPosition =
    state.mode === "desktop" ? state.cursor : state.touchCenter;
  const hoveredRegionId =
    state.mode === "desktop" ? state.hoveredRegion : state.touchedRegion;
  ...
  const screenPixelRatio =
    state.mode === "desktop" ? state.precision.screenPixelRatio : 0;
