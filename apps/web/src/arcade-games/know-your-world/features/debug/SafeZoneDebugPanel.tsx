/**
 * SafeZoneDebugPanel Component
 *
 * Debug overlay showing safe zone margins, crop regions, and viewport calculations.
...
export interface SafeZoneDebugPanelProps {
  /** Whether debug is enabled */
  visible: boolean
  /** Whether fillContainer mode is active */
  fillContainer: boolean
  /** SVG dimensions */
  svgDimensions: { width: number; height: number }
  /** Current display viewBox string */
  displayViewBox: string
  /** Original viewBox string from map data */
  originalViewBox: string
  /** Custom crop viewBox string (if defined) */
  customCrop: string | null | undefined
}
...
/**
 * Renders debug overlays showing safe zones, crop regions, and viewport info.
 */
export const SafeZoneDebugPanel = memo(function SafeZoneDebugPanel({
  visible,
  fillContainer,
  svgDimensions,
  displayViewBox,
  originalViewBox,
  customCrop,
}: SafeZoneDebugPanelProps) {
  // Get safe zone margins from context
  const { safeZoneMargins } = useMapRendererContext()

  if (!visible || !fillContainer) return null
  ...
  const originalBounds = parseViewBox(originalViewBox)
  const cropRegion = customCrop ? parseViewBox(customCrop) : originalBounds
  const isCustomCrop = !!customCrop
