...
export function MagnifierPixelGrid({
  ...
  if (screenPixelRatio <= precisionModeThreshold) {
    ...
    gridOpacity =
      ...
      (precisionModeThreshold - fadeStartRatio);
  } else {
    // Fading out: 1 at 100%, 0 at 130%
    gridOpacity =
      (fadeEndRatio - screenPixelRatio) /
      (fadeEndRatio - precisionModeThreshold);
  }
  ...
  const firstVerticalLine =
    Math.floor((gridLeft - cursorSvgX) / gridSpacingSvgUnits) *
      gridSpacingSvgUnits +
    cursorSvgX;
  ...
  const firstHorizontalLine =
    Math.floor((gridTop - cursorSvgY) / gridSpacingSvgUnits) *
      gridSpacingSvgUnits +
    cursorSvgY;
