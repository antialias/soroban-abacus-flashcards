...
export interface UseMagnifierTouchHandlersOptions {
  ...
  largestPieceSizesRef: React.MutableRefObject<
    ...
    (e: React.TouchEvent<HTMLDivElement>) => {
      ...
      if (e.touches.length === 1) {
        ...
        if (
          ...
          const cursorSvgX =
            (cursorPositionRef.current.x - svgOffsetX) / viewport.scale +
            parsedViewBox.x;
          const cursorSvgY =
            (cursorPositionRef.current.y - svgOffsetY) / viewport.scale +
            parsedViewBox.y;
          ...
          const touchInMagnifierX =
            (touch.clientX - magnifierRect.left) / magnifierRect.width;
          const touchInMagnifierY =
            (touch.clientY - magnifierRect.top) / magnifierRect.height;

          // Convert to SVG coordinates within the magnified view
          const touchSvgX =
            cursorSvgX -
            magnifiedWidth / 2 +
            touchInMagnifierX * magnifiedWidth;
          const touchSvgY =
            cursorSvgY -
            magnifiedHeight / 2 +
            touchInMagnifierY * magnifiedHeight;
  ...
  const handleMagnifierTouchMove = useCallback(
    (e: React.TouchEvent<HTMLDivElement>) => {
      ...
      if (empiricalScale.isValid && anchorInitialScreen) {
        ...
        const actualMagnifierWidth = isMagnifierExpanded
          ? leftoverWidth
          : magnifierWidth;
        const actualMagnifierHeight = isMagnifierExpanded
          ? leftoverHeight
          : magnifierHeight;
      ...
      if (
        ...
        if (targetRegion && checkHotCold) {
          ...
          const svgOffsetXWithLetterbox =
            svgRect.left - containerRect.left + viewport.letterboxX;
          const svgOffsetYWithLetterbox =
            svgRect.top - containerRect.top + viewport.letterboxY;
          const targetPixelX =
            (targetRegion.center[0] - viewBoxX) * viewport.scale +
            svgOffsetXWithLetterbox;
          const targetPixelY =
            (targetRegion.center[1] - viewBoxY) * viewport.scale +
            svgOffsetYWithLetterbox;
          const cursorSvgX =
            (clampedX - svgOffsetXWithLetterbox) / viewport.scale + viewBoxX;
          const cursorSvgY =
            (clampedY - svgOffsetYWithLetterbox) / viewport.scale + viewBoxY;
      ...
      if (e.changedTouches.length === 1 && !didMove && tapPosition) {
        ...
        if (
          ...
          const tapSvgX =
            cursorSvgX -
            magnifiedWidth / 2 +
            (tapPosition.x / magnifierRect.width) * magnifiedWidth;
          const tapSvgY =
            cursorSvgY -
            magnifiedHeight / 2 +
          ...
          const tapContainerX =
            (tapSvgX - viewBoxX) * viewport.scale + svgOffsetX;
          const tapContainerY =
            (tapSvgY - viewBoxY) * viewport.scale + svgOffsetY;
