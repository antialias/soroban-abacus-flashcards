...
export function useMusicEngine(): MusicEngine {
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      console.log('[MusicEngine] repl methods:', repl ? Object.keys(repl) : 'null')
      ...
      const message = err instanceof Error ? err.message : 'Failed to initialize music engine'
  ...
  const play = useCallback(async () => {
    ...
    try {
      ...
      const patternWithVolume = wrapWithVolume(currentPatternRef.current, volumeRef.current)
      ...
      const message = err instanceof Error ? err.message : 'Failed to play pattern'
  ...
  const stop = useCallback(async () => {
    ...
    try {
      ...
      if (strudelReplRef.current) {
        console.log('[MusicEngine] repl ref exists, methods:', Object.keys(strudelReplRef.current))
  ...
  const setVolume = useCallback(
    async (volume: number) => {
      ...
      if (state.isPlaying && state.isInitialized && !state.isMuted) {
        try {
          const patternWithVolume = wrapWithVolume(currentPatternRef.current, clampedVolume)
          await evaluate(patternWithVolume)
        } catch (err) {
  ...
  const evaluatePattern = useCallback(
    async (pattern: string) => {
      console.log('[MusicEngine] evaluatePattern() called, isPlaying:', state.isPlaying)
      ...
      if (state.isPlaying && state.isInitialized && !state.isMuted) {
        ...
        console.log('[MusicEngine] evaluatePattern: not playing, skipped evaluate()')
      }
    },
    [state.isPlaying, state.isInitialized, state.isMuted, wrapWithVolume]
  )
  ...
  const evaluatePatternFullVolume = useCallback(
    async (pattern: string) => {
      console.log('[MusicEngine] evaluatePatternFullVolume() called, isPlaying:', state.isPlaying)
      ...
      if (state.isPlaying && state.isInitialized && !state.isMuted) {
        ...
        console.log('[MusicEngine] evaluatePatternFullVolume: not playing, skipped evaluate()')
      }
    },
  ...
  const playOneShot = useCallback(
    async (pattern: string) => {
      console.log('[MusicEngine] playOneShot() called, isInitialized:', state.isInitialized)
  ...
  const { isInitialized, isPlaying, isMuted, volume, error, currentPattern } = state
