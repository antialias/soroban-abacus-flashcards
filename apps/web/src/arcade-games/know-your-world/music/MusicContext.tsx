...
export function MusicProvider({
  ...
  const [currentTemperature, setCurrentTemperatureState] =
  ...
  const enableMusic = useCallback(async () => {
    ...
    console.log(
      "[MusicContext] Checking if should play, isPlaying:",
      engine.isPlaying,
    );
    if (!engine.isPlaying) {
  ...
  useEffect(() => {
    ...
    if (hint) {
      hintTimerRef.current = setTimeout(() => {
        ...
        if (
          currentRegionId === lastRegionIdRef.current &&
          engine.isPlaying &&
          !engine.isMuted
        ) {
          ...
          const combinedPattern = buildCombinedPattern(
            preset.pattern,
            hint.pattern,
          );
  ...
  const setTemperature = useCallback(
    (feedbackType: FeedbackType | null) => {
      ...
      const modifiedPattern = applyTemperatureToPattern(
  ...
  const playCelebration = useCallback(
    (type: CelebrationType) => {
      ...
      const musicIsPlaying =
        engine.isPlaying && !engine.isMuted && basePatternRef.current;
      ...
      if (musicIsPlaying) {
        ...
        console.log(
          "[MusicContext] playCelebration: layering with Strudel music",
        );
        ...
        console.log(
          "[MusicContext] playCelebration: using Web Audio (music off)",
        );
