 ...
 */
export function findOptimalZoom(
  context: AdaptiveZoomSearchContext,
  ...
  const cursorSvgX =
    (cursorX - (svgRect.left - containerRect.left)) * scaleX + viewBoxX;
  const cursorSvgY =
    (cursorY - (svgRect.top - containerRect.top)) * scaleY + viewBoxY;
  ...
  const regionsWithScores = detectedRegions.map((detectedRegion) => {
    const regionPath = svgElement.querySelector(
      `path[data-region-id="${detectedRegion.id}"]`,
    );
    if (!regionPath) {
  ...
  const sortedRegions = regionsWithScores.sort(
  ...
  for (
    let testZoom = maxZoom;
    testZoom >= calculatedMinZoom;
    testZoom *= zoomStep
  ) {
    ...
    for (const { region: detectedRegion, importance } of sortedRegions) {
      ...
      if (!inViewport) {
        ...
        const existingDecision = regionDecisions.find(
          (d) => d.regionId === detectedRegion.id,
        );
        if (!existingDecision) {
      ...
      if (widthFits || heightFits) {
        ...
        const acceptedBox = boundingBoxes.find(
          (bbox) => bbox.regionId === detectedRegion.id,
        );
        if (acceptedBox) {
