...
export const ASSISTANCE_LEVELS: AssistanceLevelConfig[] = [
  ...
  },
  {
    ...
    emoji: "🎓",
    description:
      "Maximum help - auto hints, hot/cold feedback, shows names on wrong clicks",
    hotColdEnabled: true,
    hintsMode: "onRequest",
    autoHintDefault: true,
    struggleHintEnabled: true,
 ...
 */
export const ALL_REGION_SIZES: RegionSize[] = [
  "huge",
  "large",
  "medium",
  "small",
...
export const DEFAULT_DIFFICULTY_CONFIG: MapDifficultyConfig = {
  levels: [
    ...
    },
    {
      ...
      detailedDescription:
        "Most countries (~163). Hot/cold feedback. Press H for hints anytime.",
    ...
    },
    {
      ...
      detailedDescription:
        "Nearly all countries (~223). No hot/cold. 3 hints per game.",
    ...
    },
    {
      ...
      detailedDescription:
        "All countries including tiny islands (256). No hints. Pure geography.",
 ...
 */
function shouldIncludeUSAState(
  stateId: string,
...
export const USA_DIFFICULTY_CONFIG: MapDifficultyConfig = {
  levels: [
    ...
    },
    {
      ...
      detailedDescription:
        "Most states (~35). Hot/cold feedback. Press H for hints anytime.",
      includeSizes: ["huge", "large", "medium"],
      hotColdEnabled: true,
      hintsMode: "onRequest",
      autoHintDefault: false,
    ...
    },
    {
      ...
      detailedDescription:
        "Nearly all states (~45). No hot/cold. 3 hints per game.",
      includeSizes: ["huge", "large", "medium", "small"],
      hotColdEnabled: false,
      hintsMode: "limited",
      hintLimit: 3,
    ...
    },
    {
      ...
      detailedDescription:
        "All 51 states/territories. No hints. Pure geography.",
      includeSizes: ["huge", "large", "medium", "small", "tiny"],
      hotColdEnabled: false,
      hintsMode: "none",
      autoHintDefault: false,
 ...
 */
export const ALL_POPULATION_LEVELS: PopulationLevel[] = [
  "huge",
  "large",
  "medium",
  "small",
 ...
 */
export function getRegionImportanceCategory(
  regionId: string,
 ...
 */
export function getRegionPopulationCategory(
  regionId: string,
...
export function filterRegionsByImportance(
  ...
  if (
    ...
    includeLevels.length === ALL_IMPORTANCE_LEVELS.length
  ) {
    return regions;
  }
  return regions.filter((r) =>
    shouldIncludeRegionByImportance(r.id, includeLevels),
  );
}
...
export function filterRegionsByPopulation(
  ...
  if (
    includeLevels.length === 0 ||
    includeLevels.length === ALL_POPULATION_LEVELS.length
  ) {
    return regions;
  }
  return regions.filter((r) =>
    shouldIncludeRegionByPopulation(r.id, includeLevels),
  );
}
 ...
 */
export function shouldIncludeRegion(
  regionId: string,
  includeSizes: RegionSize[],
): boolean {
 ...
 */
export function getLargestSubpathCentroid(
  pathString: string,
...
export function calculateSafeZoneViewBox(
  ...
  const viewBoxX_min =
    cropRegion.x + cropRegion.width - (viewportWidth - margins.right) / scale;
  const viewBoxY_max = cropRegion.y - margins.top / scale;
  const viewBoxY_min =
    cropRegion.y +
    cropRegion.height -
    (viewportHeight - margins.bottom) / scale;
...
function filterRegionsByDifficulty(
  ...
  if (level.excludeRegions && level.excludeRegions.length > 0) {
    const filtered = regions.filter(
      (r) => !level.excludeRegions!.includes(r.id),
    );
    console.log(
  ...
  const filtered = regionsWithAreas
    .slice(0, keepCount)
  ...
  if (excluded.length > 0) {
    console.log(
      ...
      excluded.map(
        (item) => `${item.region.name} (${item.area.toFixed(0)} units²)`,
      ),
    );
  }
...
export async function getFilteredMapData(
  ...
  if (!difficultyLevel) {
    ...
    if (!defaultLevel) {
      throw new Error(
        `Invalid difficulty config for map "${mapId}": no default level found`,
      );
    }
  }
