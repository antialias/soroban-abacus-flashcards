...
interface KnowYourWorldContextValue {
  ...
  setPuzzlePieceTarget: React.Dispatch<
    React.SetStateAction<PuzzlePieceTarget | null>
  >;
...
const KnowYourWorldContext = createContext<KnowYourWorldContextValue | null>(
  null,
);
...
export function useKnowYourWorld() {
  ...
  if (!context) {
    throw new Error(
      "useKnowYourWorld must be used within KnowYourWorldProvider",
    );
  }
  return context;
}

export function KnowYourWorldProvider({
  children,
}: {
  ...
  const [controlsState, setControlsState] =
    useState<ControlsState>(defaultControlsState);
  ...
  // Puzzle piece animation state (Learning mode only)
  const [puzzlePieceTarget, setPuzzlePieceTarget] =
    useState<PuzzlePieceTarget | null>(null);
  ...
  const initialState = useMemo(() => {
    ...
    const assistanceLevel: AssistanceLevel =
      typeof rawAssistance === "string" &&
      validAssistanceLevels.includes(rawAssistance)
    ...
    return {
      ...
      gameMode:
        (gameConfig?.gameMode as "cooperative" | "race" | "turn-based") ||
        "cooperative",
      includeSizes,
      assistanceLevel,
      selectedContinent,
  ...
  const setMap = useCallback(
    (selectedMap: "world" | "usa") => {
      ...
      if (roomData?.id) {
        ...
        const currentConfig =
          (currentGameConfig["know-your-world"] as Record<string, any>) || {};
