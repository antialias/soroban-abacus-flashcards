...
function calculateMovingAverageDirection(state: HotColdState): number {
  ...
  const normalizedDirection =
    weightedDeltaSum / weightSum / (avgDistance * 0.1);
 ...
 */
function updateZoneWithHysteresis(
  state: HotColdState,
  direction: number,
): Zone {
...
export function useHotColdFeedback({
  ...
  const [lastFeedbackType, setLastFeedbackType] = useState<FeedbackType | null>(
    null,
  );
  ...
  const speak = useCallback(
    (type: FeedbackType, regionForAccent: string | null) => {
      ...
      const regionLang = regionForAccent
        ? getLanguageForRegion(mapName, regionForAccent)
        : userLang;
  ...
  const checkPosition = useCallback(
    ({
      ...
      const regionForAccent =
        hoveredRegionId ??
        (cursorSvgPosition
          ? findClosestRegion(cursorSvgPosition, regions)
          : null);
      ...
      if (isOverTarget) {
        ...
        if (
          timeSinceLastFoundIt >= FOUND_IT_COOLDOWN &&
          !isSpeaking &&
          !externalSpeaking
        ) {
      ...
      if (state.lastDistanceToTarget !== null) {
        const distanceDelta = Math.abs(
          currentDistance - state.lastDistanceToTarget,
        );
      ...
      const visualFeedbackType = determineFeedbackType(
        state,
        zone,
        currentDistance,
        false,
      ...
      if (confidence < CONFIDENCE_THRESHOLD) {
        ...
        if (
          state.consecutiveNeutralSamples > 15 &&
          !isSpeaking &&
          !externalSpeaking
        ) {
      ...
      const audioFeedbackType = determineFeedbackType(
        state,
        zone,
        currentDistance,
        false,
  ...
  const getSearchMetrics = useCallback(
    (promptStartTime: number): SearchMetrics => {
      const state = stateRef.current;
      ...
      const searchEfficiency =
        totalCursorDistance > 0
          ? Math.min(1, straightLineDistance / totalCursorDistance)
          : 1;
