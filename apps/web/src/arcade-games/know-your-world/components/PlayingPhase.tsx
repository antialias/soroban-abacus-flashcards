...
export function PlayingPhase() {
  ...
  useEffect(() => {
    const handleCropUpdate = () => {
      console.log(
        "[PlayingPhase] Received crop update event, forcing re-render",
      );
    ...
    return () =>
      window.removeEventListener(CROP_UPDATE_EVENT, handleCropUpdate);
  }, []);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(CROP_MODE_EVENT, handleCropModeChange);
  }, []);
  ...
  if (state.currentPrompt && !currentRegion) {
    ...
    console.error(
      "[PlayingPhase] CRITICAL: Prompt not in filtered regions!",
      errorInfo,
    );
    throw new Error(
