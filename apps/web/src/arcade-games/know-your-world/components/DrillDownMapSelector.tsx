 ...
 */
const SIZE_OPTIONS: ThermometerOption<RegionSize>[] = ALL_REGION_SIZES.map(
  (size) => ({
    value: size,
 ...
 */
const IMPORTANCE_OPTIONS: ThermometerOption<ImportanceLevel>[] =
 ...
 */
const POPULATION_OPTIONS: ThermometerOption<PopulationLevel>[] =
  ALL_POPULATION_LEVELS.map((level) => ({
    value: level,
 ...
 */
const ALL_FILTER_CRITERIA: FilterCriteria[] = [
  "size",
  "importance",
  "population",
];
...
export function DrillDownMapSelector({
  ...
  const [sizeRangePreview, setSizeRangePreview] =
    useState<RangePreviewState<RegionSize> | null>(null);
  ...
  const [previewRegionName, setPreviewRegionName] = useState<string | null>(
    null,
  );

  // Filter criteria tab state
  const [activeFilterCriteria, setActiveFilterCriteria] =
    useState<FilterCriteria>("size");
  ...
  useEffect(() => {
    ...
    const pathMatches =
      ...
      path.every((p, i) => p === expectedPath[i]);
  ...
  const { mapData, viewBox, visibleRegions, highlightedRegions } =
    useMemo(() => {
      if (currentLevel === 2) {
        ...
        if (subMapData) {
          ...
          if (
            ...
            const cropRegion = customCrop
              ? parseViewBox(customCrop)
              : originalBounds;
      ...
      const continentId: ContinentId | "all" =
        currentLevel >= 1 && path[0] ? path[0] : "all";
      ...
      if (
        ...
        const customCrop =
          continentId !== "all" ? getCustomCrop("world", continentId) : null;
        const cropRegion = customCrop
          ? parseViewBox(customCrop)
          : originalBounds;
        ...
        const visible =
          continentId !== "all"
            ? filterRegionsByContinent(WORLD_MAP.regions, continentId).map(
                (r) => r.id,
              )
            : undefined;
        ...
        if (continentId !== "all") {
          ...
          highlighted = allContinentIds.flatMap((cId) =>
            getSubMapsForContinent(cId),
          );
        }
  ...
  const regionGroups = useMemo(() => {
    ...
    for (const [countryId, continentId] of Object.entries(
      COUNTRY_TO_CONTINENT,
    )) {
  ...
  const excludedRegions = useMemo(() => {
    ...
    switch (activeFilterCriteria) {
      case "size":
        filteredRegions = filterRegionsBySizes(
          baseRegions,
          includeSizes,
          "world",
        );
        break;
      case "importance":
        filteredRegions = filterRegionsByImportance(
          baseRegions,
          includeImportance,
        );
        break;
      case "population":
        filteredRegions = filterRegionsByPopulation(
          baseRegions,
          includePopulation,
        );
        break;
  ...
  const regionCountsByCriteria = useMemo(() => {
    switch (activeFilterCriteria) {
      case "size":
        return Object.fromEntries(
          Object.entries(regionNamesBySize).map(([k, v]) => [
            k,
            v?.length ?? 0,
          ]),
        );
      case "importance":
        return Object.fromEntries(
          Object.entries(regionNamesByImportance).map(([k, v]) => [
            k,
            v?.length ?? 0,
          ]),
        );
      case "population":
        return Object.fromEntries(
          Object.entries(regionNamesByPopulation).map(([k, v]) => [
            k,
            v?.length ?? 0,
          ]),
        );
    ...
    activeFilterCriteria,
    regionNamesBySize,
    regionNamesByImportance,
    regionNamesByPopulation,
  ]);
  ...
  const previewViewBox = useMemo(() => {
    ...
    if (containerDimensions.width > 0 && containerDimensions.height > 0) {
      const containerAspect =
        containerDimensions.width / containerDimensions.height;
      return calculateFitCropViewBox(
        originalParsed,
        cropRegion,
        containerAspect,
      );
  ...
  const { previewAddRegions, previewRemoveRegions } = useMemo(() => {
    ...
    switch (activeFilterCriteria) {
      case "size":
        currentFiltered = filterRegionsBySizes(
          baseRegions,
          includeSizes,
          "world",
        );
        break;
      case "importance":
        currentFiltered = filterRegionsByImportance(
          baseRegions,
          includeImportance,
        );
        break;
      case "population":
        currentFiltered = filterRegionsByPopulation(
          baseRegions,
          includePopulation,
        );
        break;
    ...
    switch (activeFilterCriteria) {
      case "size":
        if (sizeRangePreview) {
          ...
          previewFiltered = filterRegionsBySizes(
            baseRegions,
            previewSizes,
            "world",
          );
      ...
      case "importance":
        if (importanceRangePreview) {
          ...
          previewFiltered = filterRegionsByImportance(
            baseRegions,
            previewLevels,
          );
        }
      ...
      case "population":
        if (populationRangePreview) {
          ...
          previewFiltered = filterRegionsByPopulation(
            baseRegions,
            previewLevels,
          );
        }
  ...
  const peers = useMemo(() => {
    ...
    if (currentLevel === 1 && path[0]) {
      ...
      return CONTINENTS.filter(
        (c) => c.id !== currentContinent && c.id !== "antarctica",
      ).map((c) => ({
        id: c.id,
        label: c.name,
        emoji: c.emoji,
    ...
    >
      ...
      >
        <MapSelectorMap
          ...
          selectedGroup={
            currentLevel === 0 && selectedContinent !== "all"
              ? selectedContinent
              : null
          }
        ...
        {!fillContainer &&
          ...
          (() => {
            ...
            const backLabel = backToWorld
              ? "World"
              : (CONTINENTS.find((c) => c.id === backToContinentId)?.name ??
                "Continent");
          ...
          >
                ...
                {ALL_FILTER_CRITERIA.map((criteria) => {
                  ...
                  const buttonContent = (
                    ...
                    >
                      ...
                      {isActive && (
                        <span>{FILTER_CRITERIA_CONFIG[criteria].label}</span>
                      )}
                    </button>
                  );
            ...
            {activeFilterCriteria === "size" && (
              <RangeThermometer
                ...
                onChange={(min, max) =>
                  onRegionSizesChange(rangeToSizes(min, max))
                }
                orientation="vertical"
                isDark={isDark}
                counts={
                  regionCountsByCriteria as Partial<Record<RegionSize, number>>
                }
                showTotalCount
            ...
            {activeFilterCriteria === "importance" && (
              <RangeThermometer
                ...
                onChange={(min, max) =>
                  setIncludeImportance(rangeToImportance(min, max))
                }
                orientation="vertical"
                isDark={isDark}
                counts={
                  regionCountsByCriteria as Partial<
            ...
            {activeFilterCriteria === "population" && (
              <RangeThermometer
                ...
                onChange={(min, max) =>
                  setIncludePopulation(rangeToPopulation(min, max))
                }
                orientation="vertical"
                isDark={isDark}
                counts={
                  regionCountsByCriteria as Partial<
                    Record<PopulationLevel, number>
                  >
                }
                showTotalCount
      ...
      {!fillContainer && peers.length > 0 && (
        ...
        >
          {peers.map((peer) => {
            ...
            const planetData =
              "planetData" in peer
                ? (peer.planetData as PlanetData | null)
                : null;
            ...
            return (
              <div
                key={peer.id}
                data-element={
                  isPlanet ? `planet-${peer.id}` : `peer-${peer.id}`
                }
                onClick={
                  isPlanet ? undefined : () => handlePeerClick(peer.path)
                }
                className={css({
                  display: "flex",
                  flexDirection: "column",
