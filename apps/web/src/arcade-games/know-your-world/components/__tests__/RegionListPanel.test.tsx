...
describe("RegionListPanel", () => {
  ...
  describe("rendering", () => {
    ...
    it("renders empty list when no regions provided", () => {
      ...
      expect(
        container.querySelector('[data-element="region-list-panel"]'),
      ).toBeInTheDocument();
    });

    it("has correct data-element attribute", () => {
      const { container } = render(
        <RegionListPanel regions={defaultRegions} />,
      );
  ...
  describe("styling props", () => {
    it("accepts isDark prop", () => {
      ...
      const { container } = render(
        <RegionListPanel regions={defaultRegions} isDark={true} />,
      );
    ...
    it("accepts maxHeight prop", () => {
      // Just verify it renders without error
      const { container } = render(
        <RegionListPanel regions={defaultRegions} maxHeight="300px" />,
      );
  ...
  describe("large lists", () => {
    ...
    it("handles long region names", () => {
      ...
      expect(
        screen.getByText("Democratic Republic of the Congo"),
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          "United Kingdom of Great Britain and Northern Ireland",
