...
export function GameInfoPanel({
  ...
  const currentDifficultyLevel = useMemo(() => {
    const mapDiffConfig =
      (selectedMap === "world" ? WORLD_MAP : USA_MAP).difficultyConfig ||
      DEFAULT_DIFFICULTY_CONFIG;
    return (
      mapDiffConfig.levels.find((level) => level.id === state.difficulty) ||
      mapDiffConfig.levels[0]
    );
  ...
  const displayFlagEmoji =
    selectedMap === "world" && displayRegionId
      ? getCountryFlagEmoji(displayRegionId)
      : "";
  ...
  const [simplifiedTracerPaths, setSimplifiedTracerPaths] = useState<
    string[] | null
  >(null);
  ...
  const puzzlePieceShape = useMemo(() => {
    ...
    const region = mapData.regions.find(
      (r) => r.id === puzzlePieceTarget.regionId,
    );
  ...
  const requiresNameConfirmation =
    assistanceConfig.nameConfirmationLetters ?? 0;
  ...
  const safeScale = useMemo(() => {
    ...
    currentRegionName,
    isLearningMode,
    takeoverBounds.width,
    takeoverBounds.height,
  ]);
  ...
  const puzzlePieceSpring = useSpring({
    ...
    to: puzzlePieceTarget?.sourceRect
      ...
      : {
          ...
          x:
            typeof window !== "undefined" ? (window.innerWidth - 400) / 2 : 200,
          y:
  ...
  for (let i = 0; i < heatStops.length - 1; i++) {
    if (t >= heatStops[i].t && t <= heatStops[i + 1].t) {
      const segmentT =
        (t - heatStops[i].t) / (heatStops[i + 1].t - heatStops[i].t);
      heatColors = {
  ...
  const emberCount =
    requiresNameConfirmation > 0
      ? [1, 3, 8, 16][Math.min(confirmedLetterCount, 3)]
      : 1;
  const sparkCount =
    requiresNameConfirmation > 0
      ? [2, 8, 24, 48][Math.min(confirmedLetterCount, 3)]
      : 2;
    ...
    if (puzzlePieceTarget && !puzzlePieceTarget.sourceRect) {
      ...
      const timeoutId = setTimeout(() => {
        if (takeoverRegionShapeRef.current) {
          const sourceRect =
            takeoverRegionShapeRef.current.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const letterInfo = getNthNonSpaceLetter(
        currentRegionName,
        nextLetterIndex,
      );
      if (!letterInfo) {
              ...
              >
                ...
                {tracerPaths.map((_, pathIdx) => (
                  ...
                  >
                    ...
                    <animated.circle
                      r={tracerSpring.sizeScale.to(
                        (s) => tracerSize * 0.35 * (2 - s),
                      )}
                      fill="white"
                    >
                      <animateMotion
                      ...
                      return (
                        <animated.circle
                          ...
                          r={tracerSpring.emberScale.to(
                            (e) =>
                              tracerSize * (0.6 - (i / emberCount) * 0.35) * e,
                          )}
        ...
        {!isPuzzlePieceAnimating && (
          <animated.div
            ...
            style={{
              ...
              transform: takeoverSpring.scale.to(
                (s) => `translate(-50%, -50%) scale(${s})`,
              ),
              animation: showPulseAnimation
                ? "takeoverPulse 0.8s ease-in-out infinite"
                : "none",
            }}
          >
            {/* Region name display */}
                ...
                {displayRegionName
                  ? (() => {
                      ...
                      return displayRegionName.split("").map((char, index) => {
                        const isSpace = char === " ";
                        const currentNonSpaceIndex = isSpace
                          ? -1
                          : nonSpaceIndex;
                        ...
                        const isConfirmed =
                          currentNonSpaceIndex < confirmedLetterCount;
                        ...
                        return (
                          <span
                            ...
                            style={{
                              opacity:
                                needsConfirmation && !isConfirmed ? 0.4 : 1,
                              textDecoration: isNextToConfirm
                                ? "underline"
                                : "none",
                              textDecorationColor: isNextToConfirm
                                ? isDark
                                  ...
                                  : "#3b82f6"
                                : undefined,
                              textUnderlineOffset: isNextToConfirm
                                ? "4px"
                                : undefined,
                            }}
                          >
          ...
          >
            ...
            >
              {(() => {
                ...
                const hasLocalSessionVoted =
                  viewerId && giveUpVotes.includes(viewerId);
        ...
        {gameMode === "cooperative" &&
          ...
          giveUpVotes.includes(viewerId) && (
            ...
            >
              ...
              {activeUserIds.length - giveUpVotes.length === 1
                ? "player"
                : "players"}
              ...
            </div>
